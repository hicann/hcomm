    ...
    BifrostCncoiRankKeyT *tupleKey, void *tupleVal, void *codeBuf, int32_t bufLen)
{
    ...
    appVTbl = BkfPuberGetTableTypeUserData(bifrostCncoiPuber->puber, BIFROST_CNCOI_TABLE_TYPE_RANK);
    if (appVTbl == VOS_NULL) {
        VOS_ASSERT(0);
        goto error;
    }
    ...
    BifrostCncoiRankKeyT *tupleKey, void *codeBuf, int32_t bufLen)
{
    ...
    ret = BifrostCncoiRankDeleteStartAsRoot(builder, codeBuf, bufLen);
    if (ret < 0) {
        return BKF_PUBER_CODE_BUF_NOT_ENOUGH;
    }
    BifrostCncoiRankDeleteKeyCreate(builder, tupleKey);
    ret = BifrostCncoiRankDeleteEndAsRoot(builder);
    if (ret == 0 || ret > bufLen) {
        ret = -1;
        VOS_ASSERT(0);
        goto error;
...
uint32_t BifrostCncoiPuberRankReg(BifrostCncoiPuber *bifrostCncoiPuber, BifrostCncoiPuberRankVTbl *appVTbl)
{
    ...
    dcVTbl.tupleKeyLen = sizeof(BifrostCncoiRankKeyT);
    dcVTbl.tupleValLen = appVTbl->tupleValLen;
    dcVTbl.tupleKeyCmp = (F_BKF_CMP)BifrostCncoiRankKeyCmp;
    dcVTbl.tupleKeyGetStrOrNull = (F_BKF_GET_STR)BifrostCncoiRankKeyGetStr;
    ...
    puberVTbl.tableTypeId = BIFROST_CNCOI_TABLE_TYPE_RANK;
    puberVTbl.cookie = bifrostCncoiPuber;
    puberVTbl.tupleUpdateCode = (F_BKF_PUBER_TUPLE_UPDATE_CODE)BifrostCncoiPuberRankOnUpdateCode;
    puberVTbl.tupleDeleteCode = (F_BKF_PUBER_TUPLE_DELETE_CODE)BifrostCncoiPuberRankOnDeleteCode;
    puberVTbl.tableOnSub = (F_BKF_PUBER_TABLE_ONSUB)BifrostCncoiPuberRankOnSub;
    puberVTbl.tableOnUnsub = (F_BKF_PUBER_TABLE_ONUNSUB)BifrostCncoiPuberRankOnUnsub;
    ret = BkfPuberAttachTableTypeEx(bifrostCncoiPuber->puber, &puberVTbl, appVTbl, sizeof(BifrostCncoiPuberRankVTbl));
    if (ret != BKF_OK) {
        return ret;
    }
...
uint32_t BifrostCncoiPuberRankNotifyTableComplete(BifrostCncoiPuber *bifrostCncoiPuber)
{
    ...
    return BkfDcNotifyTableTypeComplete(bifrostCncoiPuber->argInit.dc, BIFROST_CNCOI_TABLE_TYPE_RANK);
}

uint32_t BifrostCncoiPuberRankCreateTable(BifrostCncoiPuber *bifrostCncoiPuber, BifrostCncoiSliceKeyT *sliceKey)
{
    if (bifrostCncoiPuber == VOS_NULL) {
        return BKF_ERR;
    }

    return BkfDcCreateTable(bifrostCncoiPuber->argInit.dc, sliceKey, BIFROST_CNCOI_TABLE_TYPE_RANK);
}

void BifrostCncoiPuberRankDeleteTable(BifrostCncoiPuber *bifrostCncoiPuber, BifrostCncoiSliceKeyT *sliceKey)
{
    if (bifrostCncoiPuber == VOS_NULL) {
        return;
    }

    BkfDcDeleteTable(bifrostCncoiPuber->argInit.dc, sliceKey, BIFROST_CNCOI_TABLE_TYPE_RANK);
}

void BifrostCncoiPuberRankReleaseTable(BifrostCncoiPuber *bifrostCncoiPuber, BifrostCncoiSliceKeyT *sliceKey)
{
    if (bifrostCncoiPuber == VOS_NULL) {
        return;
    }
    ...
    BifrostCncoiRankKeyT *tupleKey)
{
    ...
    BkfDcDeleteTuple(bifrostCncoiPuber->argInit.dc, sliceKey, BIFROST_CNCOI_TABLE_TYPE_RANK, tupleKey);
}
