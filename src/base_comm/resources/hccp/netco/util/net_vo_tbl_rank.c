...
char *NetTblGlobalRankKeyGetStr(NetTblGlobalRankKey *key, uint8_t *buf, int32_t bufLen)
{
    ...
    if (key == VOS_NULL || buf == VOS_NULL || bufLen <= 0) {
        return "__NetTblGlobalRankKeyGetStr";
    }
...
{
    (void)NetTblGlobalRankKeyH2N(&kvH->key, &kvN->key);
    uint32_t i;
...
{
    (void)NetTblGlobalRankKeyN2H(&kvN->key, &kvH->key);
...
char *NetTblGlobalRankGetStr(NetTblGlobalRank *kv, uint8_t *buf, int32_t bufLen)
{
    if (kv == VOS_NULL || buf == VOS_NULL || bufLen <= 0) {
        return "__NetTblGlobalRankGetStr";
    }
    int32_t fillLen = 0;
    (void)NetTblGlobalRankKeyGetStr(&kv->key, buf, bufLen);
    fillLen = (int32_t)VOS_StrLen((char *)buf);
    if (fillLen == bufLen) {
        return (char *)buf;
    }
    (void)NetTblGlobalRankValGetStr(&kv->val, buf + fillLen, bufLen - fillLen);
    return (char *)buf;
}
...
char *NetTblGlobalRankValGetStr(NetTblGlobalRankVal *val, uint8_t *buf, int32_t bufLen)
{
    if (val == VOS_NULL || buf == VOS_NULL || bufLen <= 0) {
        return "__NetTblGlobalRankValGetStr";
    }
    int32_t strLen;
    int32_t strTotalLen = 0;
    uint32_t i;
    ...
    for (i = 0; i < val->rankNum; i++) {
        strLen = snprintf_truncated_s((char *)buf + strTotalLen, bufLen - strTotalLen, "%u|%u|",
            val->rankInfo[i].deviceIp, val->rankInfo[i].serverIp);
        if (strLen <= 0 || (strTotalLen + strLen) >= bufLen) {
            goto error;
        }
        strTotalLen += strLen;
