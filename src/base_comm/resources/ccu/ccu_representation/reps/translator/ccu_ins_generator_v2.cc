...
HcclResult CcuInsGeneratorV2::CcuRepBufLocReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufLocRead* repBufLocRead, const TransDep &dep)
{
    CHK_PTR_NULL(repBufLocRead);
    CcuV2::CacheConfig config = {0};

    TransLocMemToLocMS(instr++, repBufLocRead->GetDst().Id(), repBufLocRead->GetSrc().addr.Id(), repBufLocRead->GetSrc().token.Id(), repBufLocRead->GetLen().Id(), dep.reserveXnId,
                       repBufLocRead->GetSem().Id(), repBufLocRead->GetMask(), config);

    return HcclResult::HCCL_SUCCESS;
}

HcclResult CcuInsGeneratorV2::CcuRepBufLocWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufLocWrite* repBufLocWrite, const TransDep &dep)
{
    CHK_PTR_NULL(repBufLocWrite);
    CcuV2::CacheConfig config = {0};

    TransLocMSToLocMem(instr++, repBufLocWrite->GetDst().addr.Id(), repBufLocWrite->GetDst().token.Id(), repBufLocWrite->GetSrc().Id(), repBufLocWrite->GetLen().Id(), dep.reserveXnId,
                       repBufLocWrite->GetSem().Id(), repBufLocWrite->GetMask(), config);

    return HcclResult::HCCL_SUCCESS;
}

HcclResult CcuInsGeneratorV2::CcuRepBufReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufRead* repBufRead, const TransDep &dep)
{
    ...
    TransMem(instr++, repBufRead->GetDst().Id(), constValue2VarMap.at(dep.ccuResSpaceTokenInfo).Id(),
             repBufRead->GetSrc().addr.Id(), repBufRead->GetSrc().token.Id(),
             repBufRead->GetLen().Id(), constValue2VarMap.at(channelId).Id(), notify, reduce, config,
             repBufRead->GetSem().Id(), repBufRead->GetMask());
...
HcclResult CcuInsGeneratorV2::CcuRepWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepWrite* repWrite)
{
    ...
    TransMem(instr++, repWrite->GetRem().addr.Id(), repWrite->GetRem().token.Id(), repWrite->GetLoc().addr.Id(), repWrite->GetLoc().token.Id(), repWrite->GetLen().Id(),
             constValue2VarMap.at(channelId).Id(), notify, reduce, config, repWrite->GetSem().Id(), repWrite->GetMask());
...
HcclResult CcuInsGeneratorV2::CcuRepReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRead* repRead)
{
    ...
    uint32_t channelId = channelImpl->GetChannelId();
    const auto &constValue2VarMap = ccuKernel->GetConstValue2VarMap();
    TransMem(instr++, repRead->GetLoc().addr.Id(), repRead->GetLoc().token.Id(), repRead->GetRem().addr.Id(), repRead->GetRem().token.Id(), repRead->GetLen().Id(),
             constValue2VarMap.at(channelId).Id(), notify, reduce, config, repRead->GetSem().Id(), repRead->GetMask());
...
HcclResult CcuInsGeneratorV2::CcuRepRemMemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemMem* repRemMem)
{
    ...
    CHK_PRT_RET(channelImpl->GetRmtBuffer(addr, size, tokenId, tokenValue) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuRepRemMem][%s] failed to get remote buffer, channelHandle[0x%llx].",
            __func__, repRemMem->GetChannel()), HCCL_E_INTERNAL);// 当前认为channel只持有一个buffer
...
HcclResult CcuInsGeneratorV2::CcuRepLocCpyTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepLocCpy* repLocCpy, const TransDep &dep)
{
    CHK_PTR_NULL(repLocCpy);
    if (repLocCpy->GetReduceFlag() == 0 && repLocCpy->GetUseCcuBuffer() == true) {
        CcuV2::CacheConfig cacheConfig{0x0, 0x0};
        CcuV2::TransLocMemToLocMem(instr++, repLocCpy->GetDstAddrId(), repLocCpy->GetDstTokenId(), 
                                   repLocCpy->GetSrcAddrId(), repLocCpy->GetSrcTokenId(), repLocCpy->GetLenId(),
                                   repLocCpy->GetFirstBufId(), repLocCpy->GetUsedBufNum(),
                                   repLocCpy->GetSemId(), repLocCpy->GetMask(), cacheConfig, cacheConfig);
    } else {
        // 使用旧接口或带规约场景，都走环回
        CcuV2::TransMemNotifyInfo notify = {};
        CcuV2::TransMemReduceInfo reduce = {};
        ...
        const auto &constValue2VarMap = ccuKernel->GetConstValue2VarMap();
        CcuV2::TransMem(instr++, repLocCpy->GetDstAddrId(), repLocCpy->GetDstTokenId(),
                        repLocCpy->GetSrcAddrId(), repLocCpy->GetSrcTokenId(), repLocCpy->GetLenId(),
                        constValue2VarMap.at(dep.reserveChannalId[0]).Id(), notify, reduce, config, repLocCpy->GetSemId(), repLocCpy->GetMask());
    }

    return HcclResult::HCCL_SUCCESS;
}

HcclResult CcuInsGeneratorV2::CcuRepBufWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufWrite* ccuRepBufWrite, const TransDep &dep)
{
    ...
    CHK_RET(GetRmtToken(channelImpl, rmtCcuResToken));
    notify.xntId = constValue2VarMap.at(rmtCcuResToken).Id();
    HCCL_INFO("DstTokenXnId[%u], NotifyTokenXnId[%u]", constValue2VarMap.at(dep.ccuResSpaceTokenInfo).Id(), notify.xntId);
    TransMem(instr++, ccuRepBufWrite->GetDst().addr.Id(), ccuRepBufWrite->GetDst().token.Id(),
...
HcclResult CcuInsGeneratorV2::CcuRepBufReduceTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufReduce* ccuRepBufReduce)
{
    UNUSED(ccuKernel);
    ...
    if (ccuRepBufReduce->GetOpType() == CCU_REDUCE_SUM) {
        if (ccuRepBufReduce->GetOutputDataType() == 1) { // 1是fp16
            CcuV2::ReduceAdd(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetOutputDataType(),
            ...
            CcuV2::ReduceAdd(instr++, msId, ccuRepBufReduce->GetCount(), 0, ccuRepBufReduce->GetDataType(),
        ...
        CcuV2::ReduceMax(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetDataType(),
            ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), ccuRepBufReduce->GetXnIdLength().Id());
    } else if (ccuRepBufReduce->GetOpType() == CCU_REDUCE_MIN) {
        CcuV2::ReduceMin(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetDataType(),
            ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), ccuRepBufReduce->GetXnIdLength().Id());
    } else {
...
HcclResult CcuInsGeneratorV2::CcuRepLocRecordEventTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepLocRecordEvent* ccuRepLocRecordEvent)
{
...
HcclResult CcuInsGeneratorV2::CcuRepLocWaitEventTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepLocWaitEvent* ccuRepLocWaitEvent)
{
...
HcclResult CcuInsGeneratorV2::CcuRepLocWaitNotifyTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepLocWaitNotify* ccuRepLocWaitNotify)
{
...
HcclResult CcuInsGeneratorV2::CcuRepRemWaitSemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemWaitSem* ccuRepRemWaitSem)
{
    ...
    uint32_t locCkeId{0};
    CHK_PRT_RET(channelImpl->GetLocCkeByIndex(ccuRepRemWaitSem->GetSemIndex(), locCkeId) != HcclResult::HCCL_SUCCESS,
...
HcclResult CcuInsGeneratorV2::CcuRepRemPostVarTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemPostVar* ccuRepRemPostVar)
{
    ...
    CHK_PRT_RET(channelImpl->GetRmtSignalAddrByIndex(ccuRepRemPostVar->GetSemIndex(), rmtSignalAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote signal addr, channelHandle[0x%llx].",
            __func__, ccuRepRemPostVar->GetChannel()), HCCL_E_INTERNAL);
    CHK_PRT_RET(channelImpl->GetRmtVarAddrByIndex(ccuRepRemPostVar->GetParamIndex(), rmtVarAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote var addr, channelHandle[0x%llx].",
            __func__, ccuRepRemPostVar->GetChannel()), HCCL_E_INTERNAL);
    uint64_t rmtToken{};
    ...
    SyncWtX(instr++, constValue2VarMap.at(rmtVarAddr).Id(), constValue2VarMap.at(rmtToken).Id(), 
            ccuRepRemPostVar->GetParam().Id(), constValue2VarMap.at(channelId).Id(), notify, 0, 0);

    return HcclResult::HCCL_SUCCESS;
}

HcclResult CcuInsGeneratorV2::CcuRepRemPostSemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemPostSem* ccuRepRemPostSem, const TransDep &dep)
{
    ...
    uint64_t rmtSignalAddr{0};
    uint32_t channelId = channelImpl->GetChannelId();
    CHK_PRT_RET(channelImpl->GetRmtSignalAddrByIndex(ccuRepRemPostSem->GetSemIndex(), rmtSignalAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote signal addr, channelHandle[0x%llx].",
            __func__, ccuRepRemPostSem->GetChannel()), HCCL_E_INTERNAL);
    uint64_t rmtToken{};
    CHK_RET(GetRmtToken(channelImpl, rmtToken));
...
HcclResult CcuInsGeneratorV2::CcuRepRecordSharedNotifyTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRecordSharedNotify* ccuRepRecordSharedNotify, const TransDep &dep)
{
    ...
    if (ccuRepRecordSharedNotify->GetNotify().DieId() != dep.dieId) {
        ...
        uint64_t ckeAddr = dep.xnBaseAddr[ccuRepRecordSharedNotify->GetNotify().DieId()] +
            CCU_RESOURCE_XN_V2_RESERVE_SIZE + (ccuRepRecordSharedNotify->GetNotify().Id() * CCU_RESOURCE_CKE_PER_SIZE);
        const auto &constValue2VarMap = ccuKernel->GetConstValue2VarMap();
        CcuV2::CacheConfig cacheConfig{0x0, 0x0};
        CcuV2::StoreXToMem(instr++,
        ...
        CcuV2::SetCKE(instr++, ccuRepRecordSharedNotify->GetNotify().Id(),
            ccuRepRecordSharedNotify->GetMask(), 0, 0, 1);
    }

    return HcclResult::HCCL_SUCCESS;
}

HcclResult CcuInsGeneratorV2::CcuRepAddTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepAdd* ccuRepAdd, const TransDep &dep)
{
    ...
    static constexpr struct { bool isImmed; bool isSelf; bool dstVar; bool srcAVar; bool srcBAddr; } table[] = {
    ...
    uint16_t dstId = info.isSelf ? srcAId
                    : (info.dstVar ? ccuRepAdd->GetVarC().Id() : ccuRepAdd->GetAddrC().Id());
...
HcclResult CcuInsGeneratorV2::CcuRepAssignTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepAssign* ccuRepAssign, const TransDep &dep)
{
    UNUSED(dep);
...
HcclResult CcuInsGeneratorV2::CcuRepMulTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepMul* ccuRepMul)
{
    ...
    uint16_t dstId = info.isSelf ? srcAId
                    : (info.dstVar ? ccuRepMul->GetVarC().Id() : ccuRepMul->GetAddrC().Id());
...
HcclResult CcuInsGeneratorV2::CcuRepSubTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepSub* ccuRepSub)
{
    ...
    uint16_t dstId = info.isSelf ? srcAId
                    : (info.dstVar ? ccuRepSub->GetVarC().Id() : ccuRepSub->GetAddrC().Id());
...
HcclResult CcuInsGeneratorV2::CcuRepAndTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepAnd* ccuRepAnd, const TransDep &dep)
{
   CHK_PTR_NULL(ccuRepAnd);
   switch (ccuRepAnd->GetSubType()) {
        case AndSubType::VAR_AND_VAR_TO_VAR: {
            CcuV2::And(instr++, ccuRepAnd->GetVarC().Id(), ccuRepAnd->GetVarA().Id(), ccuRepAnd->GetVarB().Id(), 0, 0);
            break;
        }
        case AndSubType::SELF_AND_VAR_VARIABLE: {
            CcuV2::And(instr++, ccuRepAnd->GetVarC().Id(), ccuRepAnd->GetVarC().Id(), ccuRepAnd->GetVarB().Id(), 0, 0);
            break;
        }  
        default: {
...
HcclResult CcuInsGeneratorV2::CcuRepNotTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepNot* ccuRepNot, const TransDep &dep)
{
...
HcclResult CcuInsGeneratorV2::CcuRepOrTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepOr* ccuRepOr, const TransDep &dep)
{
...
HcclResult CcuInsGeneratorV2::CcuRepXorTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepXor* ccuRepXor, const TransDep &dep)
{
    CHK_PTR_NULL(ccuRepXor);
    switch (ccuRepXor->GetSubType()) {
        case XorSubType::VAR_XOR_VAR_TO_VAR: {
            CcuV2::Xor(instr++, ccuRepXor->GetVarC().Id(), ccuRepXor->GetVarA().Id(), ccuRepXor->GetVarB().Id(), 0, 0);
            break;
        }
        case XorSubType::SELF_XOR_VAR_VARIABLE: {
            CcuV2::Xor(instr++, ccuRepXor->GetVarC().Id(), ccuRepXor->GetVarC().Id(), ccuRepXor->GetVarB().Id(), 0, 0);
            break;
        }
        default: {
...
HcclResult CcuInsGeneratorV2::CcuRepShLTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepShL* ccuRepShL, const TransDep &dep)
{
    ...
    if (ccuRepShL->GetShiftType() == ShiftType::LOGICAL_SHIFT) {
        switch (ccuRepShL->GetShiftSubType()) {
            case ShiftSubType::VAR_EQUALS_VAR_SHIFT_VAR: {
                CcuV2::SLL(instr++, ccuRepShL->GetVarD().Id(), ccuRepShL->GetVarN().Id(), ccuRepShL->GetVarM().Id(), 0, 0);
                break;
            }
            case ShiftSubType::VAR_SHIFT_ASSIGN_VAR: {
                CcuV2::SLL(instr++, ccuRepShL->GetVarD().Id(), ccuRepShL->GetVarD().Id(), ccuRepShL->GetVarM().Id(), 0, 0);
                break;
            }
            case ShiftSubType::ADDR_EQUALS_VAR_SHIFT_VAR: {
                CcuV2::SLL(instr++, ccuRepShL->GetAddressD().Id(), ccuRepShL->GetVarN().Id(), ccuRepShL->GetVarM().Id(), 0, 0);
                break;
            }
            case ShiftSubType::ADDR_SHIFT_ASSIGN_VAR: {
                CcuV2::SLL(instr++, ccuRepShL->GetAddressD().Id(), ccuRepShL->GetAddressD().Id(), ccuRepShL->GetVarM().Id(), 0, 0);
                break;
            }
            default: {
...
HcclResult CcuInsGeneratorV2::CcuRepShRTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepShR* ccuRepShR, const TransDep &dep)
{
    CHK_PTR_NULL(ccuRepShR);
    if (ccuRepShR->GetShiftType() == ShiftType::LOGICAL_SHIFT) {
        switch (ccuRepShR->GetShiftSubType()) {
            case ShiftSubType::VAR_EQUALS_VAR_SHIFT_VAR: {
                CcuV2::SRL(instr++, ccuRepShR->GetVarD().Id(), ccuRepShR->GetVarN().Id(), ccuRepShR->GetVarM().Id(), 0, 0);
                break;
            }
            case ShiftSubType::VAR_SHIFT_ASSIGN_VAR: {
                CcuV2::SRL(instr++, ccuRepShR->GetVarD().Id(), ccuRepShR->GetVarD().Id(), ccuRepShR->GetVarM().Id(), 0, 0);
                break;
            }
             case ShiftSubType::ADDR_EQUALS_VAR_SHIFT_VAR: {
                CcuV2::SRL(instr++, ccuRepShR->GetAddressD().Id(), ccuRepShR->GetVarN().Id(), ccuRepShR->GetVarM().Id(), 0, 0);
                break;
            }
            case ShiftSubType::ADDR_SHIFT_ASSIGN_VAR: {
                CcuV2::SRL(instr++, ccuRepShR->GetAddressD().Id(), ccuRepShR->GetAddressD().Id(), ccuRepShR->GetVarM().Id(), 0, 0);
                break;
            }
            default: {
...
HcclResult CcuInsGeneratorV2::CcuRepFuncBlockTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId,
    CcuRepFuncBlock* funcBlockPtr, const TransDep &dep, uint32_t step)
{
    ...
    if (step == 0) {
        ...
        CcuV2::RelJmp(instr, funcManager->GetFuncRet(callLayer).Id(), curInstrId + relJmpInstrNum, dep.commXn[0], dep.commXn[1]);
...
void CcuInsGeneratorV2::LoadFuncCallInArgs(CcuInstr* instr, std::vector<CcuRepArg>& inArgs,
...
void CcuInsGeneratorV2::LoadFuncCallOutArgs(CcuInstr* instr, uint32_t offset, std::vector<CcuRepArg>& outArgs,
                                            CcuRepReferenceManager* funcManager, uint16_t reserveXnId)
{
    ...
    for (uint32_t i = 0; i < outArgs.size(); i++) {
        if (outArgs[i].type == CcuArgType::VARIABLE) {
            ...
            for (uint32_t j = 0; j < outArgs[i].varList.size(); j++) {
                CcuV2::Add(instr + offset + idx, outArgs[i].varList[j].Id(), funcManager->GetFuncOut()[idx].Id(),
                           reserveXnId);
                idx++;
            }
        }
...
HcclResult CcuInsGeneratorV2::CcuRepFuncCallTranslate(CcuKernel* ccuKernel, CcuInstr *&curInstr, uint16_t &curInstrId,
    CcuRepFuncCall* funcCallPtr, const TransDep &dep)
{
    ...
    if (funcBlock != nullptr) {
        CcuV2::LoadImdToXn(instr + inArgCount + locId++, funcManager->GetFuncCall().Id(), funcBlock->StartInstrId());
    } else {
        CcuV2::Add(instr + inArgCount + locId++, funcManager->GetFuncCall().Id(), funcAddrVar.Id(), dep.reserveXnId);
    }
    CcuV2::LoadImdToXn(instr + inArgCount + locId++, funcManager->GetFuncRet(callLayer).Id(),
                        instrId + inArgCount + FUNC_CALL_RET_OFFSET); // 需要指向函数返回位置
    CcuV2::RelJmp(instr + inArgCount + locId, funcManager->GetFuncCall().Id(),
...
HcclResult CcuInsGeneratorV2::CcuRepJumpTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJump* jumpPtr, const TransDep &dep)
{
    (void)instr;
    (void)curInstrId;
    (void)dep;
    ...
    //  无条件跳转
    CcuV2::Jump(jumpPtr->GetInstr() + 1, jumpPtr->GetTargetInstrId().Id(), 0, 0, static_cast<int>(ConditionType::DEFAULT));
 
    return HcclResult::HCCL_SUCCESS;
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpTranslateV2Base(CcuInstr *&curInstr, uint16_t &curInstrId, CcuRepJumpBase* jumpBasePtr,
                                                    uint64_t expected, const Variable &condition, const Variable &expectedVar,
                                                    ConditionType condType)
{
    ...
    if (jumpBasePtr->IsComparedWithImmd()) {
        ...
        CcuV2::Jump(instr + 2, targetInstrId.Id(), condition.Id(), expectedVar.Id(), // 2: jump指令偏移
...
HcclResult CcuInsGeneratorV2::CcuRepJumpNETranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpNE* jumpNEPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpNEPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpNEPtr, jumpNEPtr->GetExpectedNum(), jumpNEPtr->GetCondition(),
                                    jumpNEPtr->GetExpectedVar(), ConditionType::NOT_EQUAL);
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpEQTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpEQ* jumpEQPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpEQPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpEQPtr, jumpEQPtr->GetExpectedNum(), jumpEQPtr->GetCondition(),
                                    jumpEQPtr->GetExpectedVar(), ConditionType::EQUAL);
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpLETranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpLE* jumpLEPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpLEPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpLEPtr, jumpLEPtr->GetExpectedNum(), jumpLEPtr->GetCondition(),
                                    jumpLEPtr->GetExpectedVar(), ConditionType::LESS_EQUAL);
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpGETranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpGE* jumpGEPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpGEPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpGEPtr, jumpGEPtr->GetExpectedNum(), jumpGEPtr->GetCondition(),
                                    jumpGEPtr->GetExpectedVar(), ConditionType::GREATER_EQUAL);
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpGTTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpGT* jumpGTPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpGTPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpGTPtr, jumpGTPtr->GetExpectedNum(), jumpGTPtr->GetCondition(),
                                    jumpGTPtr->GetExpectedVar(), ConditionType::GREATER_THAN);
}
 
HcclResult CcuInsGeneratorV2::CcuRepJumpLTTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepJumpLT* jumpLTPtr, const TransDep &dep)
{
    (void)dep;
    CHK_PTR_NULL(jumpLTPtr);
    return CcuRepJumpTranslateV2Base(instr, curInstrId, jumpLTPtr, jumpLTPtr->GetExpectedNum(), jumpLTPtr->GetCondition(),
                                    jumpLTPtr->GetExpectedVar(), ConditionType::LESS_THAN);
}


HcclResult CcuInsGeneratorV2::CcuRepLoopTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoop* loopPtr)
{
    (void)curInstrId;
    CHK_PTR_NULL(loopPtr);
    auto loopBlock = loopPtr->GetLoopBlock();
    CHK_PTR_NULL(loopBlock);
    CcuV2::Loop(instr++, loopBlock->StartInstrId(), loopBlock->StartInstrId() + loopBlock->InstrCount() - 1,
...
HcclResult CcuInsGeneratorV2::CcuRepLoopCallTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoopCall* loopCallPtr, const TransDep &dep)
{
    (void)curInstrId;
    (void)dep;
...
HcclResult CcuInsGeneratorV2::CcuRepSetLoopTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepSetLoop* setLoopPtr)
{
    (void)curInstrId;
...
HcclResult CcuInsGeneratorV2::CcuRepLoadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoad* loadPtr, const TransDep &dep)
{
    (void)curInstrId;
    ...
    CcuV2::LoadXFromMem(instr++,
                        dep.commXn[0],
                        constValue2VarMap.at(loadPtr->GetAddr()).Id(),
                        constValue2VarMap.at(dep.memTokenInfo).Id(),
                        constValue2VarMap.at(CCU_RESOURCE_XN_PER_SIZE * loadPtr->GetNum()).Id(),
                        cacheConfig, dep.commSignal, loadPtr->GetMask());
    CcuV2::SetCKE(instr++, 0, 0, dep.commSignal, loadPtr->GetMask(), 1);
 
    return HcclResult::HCCL_SUCCESS;
}
 
HcclResult CcuInsGeneratorV2::CcuRepLoadVarTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoadVar* loadVarPtr, const TransDep &dep)
{
    (void)curInstrId;
    ...
    CcuV2::LoadXFromMem(instr++,
                        dep.commXn[0],
                        loadVarPtr->GetSrc().Id(),
                        constValue2VarMap.at(dep.memTokenInfo).Id(),
                        constValue2VarMap.at(CCU_RESOURCE_XN_PER_SIZE * loadVarPtr->GetNum()).Id(),
                        cacheConfig, dep.commSignal, loadVarPtr->GetMask());
    CcuV2::SetCKE(instr++, 0, 0, dep.commSignal, loadVarPtr->GetMask(), 1);
    return HcclResult::HCCL_SUCCESS;
}
 
HcclResult CcuInsGeneratorV2::CcuRepLoadArgTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoadArg* loadArgPtr, const TransDep &dep)
{
    (void)curInstrId;
...
HcclResult CcuInsGeneratorV2::CcuRepNopTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepNop* nopPtr, const TransDep &dep)
{
    (void)curInstrId;
    (void)nopPtr;
    (void)dep;
...
HcclResult CcuInsGeneratorV2::CcuRepStoreTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepStore* storePtr, const TransDep &dep)
{
    (void)curInstrId;
    ...
    CcuV2::StoreXToMem(instr++,
                        constValue2VarMap.at(storePtr->GetAddr()).Id(),
                        constValue2VarMap.at(dep.memTokenInfo).Id(),
                        dep.commXn[0],
                        constValue2VarMap.at(CCU_RESOURCE_XN_PER_SIZE * storePtr->GetNum()).Id(),
                        cacheConfig, dep.commSignal, storePtr->GetMask());
    CcuV2::SetCKE(instr++, 0, 0, dep.commSignal, storePtr->GetMask(), 1);
 
    return HcclResult::HCCL_SUCCESS;
}
 
HcclResult CcuInsGeneratorV2::CcuRepStoreVarTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepStoreVar* storeVarPtr, const TransDep &dep)
{
    ...
    if (storeVarPtr->GetHscbFlag()) {
        CcuV2::HSCBStoreXToMem(instr++,
                                storeVarPtr->GetDst().Id(),
                                dep.commXn[0],
                                constValue2VarMap.at(CCU_RESOURCE_XN_PER_SIZE * storeVarPtr->GetNum()).Id(),
                                cacheConfig, dep.commSignal, storeVarPtr->GetMask());
    } else {
        CcuV2::StoreXToMem(instr++,
                            storeVarPtr->GetDst().Id(),
                            constValue2VarMap.at(dep.memTokenInfo).Id(),
...
HcclResult CcuInsGeneratorV2::PrepareConstValue(CcuRepBase* repPtr, const TransDep &dep, CcuKernel *ccuKernel)
{
    ...
    switch (repPtr->Type()) {
        case CcuRepType::LOAD:                 return PrepareLoadConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::LOAD_VAR:             return PrepareLoadVarConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::STORE:                return PrepareStoreConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::STORE_VAR:            return PrepareStoreVarConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::REM_POST_SEM:         return PrepareRemPostSemConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::REM_POST_VAR:         return PrepareRemPostVarConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::WRITE:                return PrepareWriteConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::READ:                 return PrepareReadConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::BUF_WRITE:            return PrepareBufWriteConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::BUF_READ:             return PrepareBufReadConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::LOCAL_CPY:            return PrepareLocCpyConstValue(repPtr, dep, ccuKernel);
        case CcuRepType::RECORD_SHARED_NOTIFY: return PrepareRecordSharedNotifyConstValue(repPtr, dep, ccuKernel);
        default: break;
    }
    return HcclResult::HCCL_SUCCESS;
}
...
HcclResult CcuInsGeneratorV2::PrepareLoadConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    ...
    CHK_PTR_NULL(loadPtr);
    std::vector<uint64_t> values = {loadPtr->GetAddr(), dep.memTokenInfo,
                                    CCU_RESOURCE_XN_PER_SIZE * loadPtr->GetNum()};
    return ccuKernel->Add2ConstValue2VarMap(values);
}

HcclResult CcuInsGeneratorV2::PrepareLoadVarConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
...
HcclResult CcuInsGeneratorV2::PrepareStoreConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    ...
    CHK_PTR_NULL(storePtr);
    std::vector<uint64_t> values = {storePtr->GetAddr(), dep.memTokenInfo,
                                    CCU_RESOURCE_XN_PER_SIZE * storePtr->GetNum()};
    return ccuKernel->Add2ConstValue2VarMap(values);
}

HcclResult CcuInsGeneratorV2::PrepareStoreVarConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    ...
    if (!storeVarPtr->GetHscbFlag()) { values.push_back(dep.memTokenInfo); }
    return ccuKernel->Add2ConstValue2VarMap(values);
}

HcclResult CcuInsGeneratorV2::PrepareRemPostSemConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    ...
    uint64_t rmtSignalAddr{0};
    CHK_PRT_RET(channelImpl->GetRmtSignalAddrByIndex(repRemPostSemPtr->GetSemIndex(), rmtSignalAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote signal addr, channelHandle[0x%llx].",
            __func__, repRemPostSemPtr->GetChannel()), HCCL_E_INTERNAL);
    uint64_t rmtToken{};
    CHK_RET(GetRmtToken(channelImpl, rmtToken));
...
HcclResult CcuInsGeneratorV2::PrepareRemPostVarConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    ...
    uint64_t rmtSignalAddr{0};
    uint64_t rmtVarAddr{0};
    CHK_PRT_RET(channelImpl->GetRmtSignalAddrByIndex(repRemPostVarPtr->GetSemIndex(), rmtSignalAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote signal addr, channelHandle[0x%llx].",
            __func__, repRemPostVarPtr->GetChannel()), HCCL_E_INTERNAL);
    CHK_PRT_RET(channelImpl->GetRmtVarAddrByIndex(repRemPostVarPtr->GetParamIndex(), rmtVarAddr) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuInsGeneratorV2][%s] failed to get remote var addr, channelHandle[0x%llx].",
            __func__, repRemPostVarPtr->GetChannel()), HCCL_E_INTERNAL);
    uint64_t rmtToken{};
    CHK_RET(GetRmtToken(channelImpl, rmtToken));
...
HcclResult CcuInsGeneratorV2::PrepareBufWriteConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
...
HcclResult CcuInsGeneratorV2::PrepareBufReadConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
...
HcclResult CcuInsGeneratorV2::PrepareRecordSharedNotifyConstValue(CcuRepBase* repPtr, const TransDep& dep, CcuKernel* ccuKernel)
{
    CcuRepRecordSharedNotify* sharedNotifyPtr =
        dynamic_cast<CcuRepRecordSharedNotify*>(repPtr);
    CHK_PTR_NULL(sharedNotifyPtr);
    uint64_t ckeAddr = dep.xnBaseAddr[sharedNotifyPtr->GetNotify().DieId()] +
        CCU_RESOURCE_XN_V2_RESERVE_SIZE + (sharedNotifyPtr->GetNotify().Id() * CCU_RESOURCE_CKE_PER_SIZE);
    std::vector<uint64_t> values = {ckeAddr, dep.memTokenInfo, CCU_RESOURCE_CKE_MASK_LENGTH, sharedNotifyPtr->GetMask()};
    return ccuKernel->Add2ConstValue2VarMap(values);
}

HcclResult CcuInsGeneratorV2::LoopConfigTranslate(CcuInstr*& instr, uint16_t& curInstrId,
...
HcclResult CcuInsGeneratorV2::LoopGroupConfigTranslate(CcuInstr*& instr, uint16_t& curInstrId,
                                                       CcuRepLoopGroupBundle* bundlePtr, const TransDep& dep,
                                                       bool isConfig, bool isCompat, uint16_t& loopGroupConfigId)
{
    ...
    if (isConfig) {
        ...
        uint64_t parallelImm = GetParallelParamV2(cfg.cloneNum, bundlePtr->GetRepeatLoopIdx(),
                                                  bundlePtr->GetTotalLoopNum());
        ...
        struct FieldMap { uint16_t srcShift; uint16_t dstShift; };
...
HcclResult CcuInsGeneratorV2::CcuRepLoopGroupBundleTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId,
    CcuRepLoopGroupBundle* bundlePtr, const TransDep &dep)
{
    UNUSED(ccuKernel);
    ...
    CcuV2::LoopGroup(instr++, curInstrId + 3, loopGroupConfigId, bundlePtr->GetOffsetParam().Id(), xnOffsetId);  // 向后3条为loop入口
    curInstrId++;
    ...
    for (const auto &loop : loops) {
        ...
        CHK_PTR_NULL(block);
        CcuV2::Loop(instr++, block->StartInstrId(), block->StartInstrId() + block->InstrCount() - 1,
                    loop.iterNumVar.Id(), loop.addrOffsetVar.Id(), loop.ctxIdVar.Id());
        curInstrId++;
    }
