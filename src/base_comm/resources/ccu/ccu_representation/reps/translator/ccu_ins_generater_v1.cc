...
template <typename T>
HcclResult LoadAddrListArg(CcuInstr*& instr, const std::vector<T>& dst, const std::vector<T>& src, const TransDep& dep)
{
...
HcclResult CcuInsGeneraterV1::CcuRepBufLocReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufLocRead* repBufLocRead, const TransDep &dep)
{
    ...
    TransLocMemToLocMSInstr(instr++, repBufLocRead->GetDst().Id(), repBufLocRead->GetSrc().addr.Id(), repBufLocRead->GetSrc().token.Id(), repBufLocRead->GetLen().Id(), dep.reserveChannalId[0],
...
HcclResult CcuInsGeneraterV1::CcuRepBufLocWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufLocWrite* repBufLocWrite, const TransDep &dep)
{
    ...
    TransLocMSToLocMemInstr(instr++, repBufLocWrite->GetDst().addr.Id(), repBufLocWrite->GetDst().token.Id(), repBufLocWrite->GetSrc().Id(), repBufLocWrite->GetLen().Id(), dep.reserveChannalId[0],
...
HcclResult CcuInsGeneraterV1::CcuRepBufReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufRead* repBufRead, const TransDep &dep)
{
    ...
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(repBufRead->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
    ...
    TransRmtMemToLocMSInstr(instr++, repBufRead->GetDst().Id(), repBufRead->GetSrc().addr.Id(), repBufRead->GetSrc().token.Id(), repBufRead->GetLen().Id(), channelImpl->GetChannelId(),
                            repBufRead->GetSem().Id(), repBufRead->GetMask(), 0, 0, 1, 1);
...
HcclResult CcuInsGeneraterV1::CcuRepWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepWrite* repWrite)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(repWrite->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", repWrite->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    TransLocMemToRmtMemInstr(instr++, repWrite->GetRem().addr.Id(), repWrite->GetRem().token.Id(), repWrite->GetLoc().addr.Id(), repWrite->GetLoc().token.Id(), repWrite->GetLen().Id(),
                             channelImpl->GetChannelId(), repWrite->GetDataType(), repWrite->GetOpType(), repWrite->GetSem().Id(), repWrite->GetMask(), 0, 0, 1, 1, repWrite->GetReduceFlag());
...
HcclResult CcuInsGeneraterV1::CcuRepReadTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRead* repRead)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(repRead->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", repRead->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    TransRmtMemToLocMemInstr(instr++, repRead->GetLoc().addr.Id(), repRead->GetLoc().token.Id(), repRead->GetRem().addr.Id(), repRead->GetRem().token.Id(), repRead->GetLen().Id(),
                             channelImpl->GetChannelId(), repRead->GetDataType(), repRead->GetOpType(), repRead->GetSem().Id(), repRead->GetMask(), 0, 0, 1, 1, repRead->GetReduceFlag());
...
HcclResult CcuInsGeneraterV1::CcuRepRemMemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemMem* repRemMem)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(repRemMem->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
    ...
    CHK_PRT_RET(channelImpl->GetRmtBuffer(addr, size, tokenId, tokenValue) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuRepRemMem][%s] failed to get remote buffer, channelHandle[0x%llx].",
            __func__, repRemMem->GetChannel()), HCCL_E_UNAVAIL);// 当前认为channel只持有一个buffer
...
HcclResult CcuInsGeneraterV1::CcuRepLocCpyTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepLocCpy* ccuRepLocCpy, const TransDep &dep)
{
    ...
    if (ccuRepLocCpy->GetReduceFlag() == 0) {
        TransLocMemToLocMemInstr(instr++, ccuRepLocCpy->GetDst().addr.Id(), ccuRepLocCpy->GetDst().token.Id(), ccuRepLocCpy->GetSrc().addr.Id(), ccuRepLocCpy->GetSrc().token.Id(), ccuRepLocCpy->GetLen().Id(),
                                dep.reserveChannalId[0], ccuRepLocCpy->GetSem().Id(), ccuRepLocCpy->GetMask(), 0, 0, 1, 1);
    } else {
        // 这个翻译需要验证
        TransLocMemToRmtMemInstr(instr++, ccuRepLocCpy->GetDst().addr.Id(), ccuRepLocCpy->GetDst().token.Id(), ccuRepLocCpy->GetSrc().addr.Id(), ccuRepLocCpy->GetSrc().token.Id(), ccuRepLocCpy->GetLen().Id(),
                                 dep.reserveChannalId[0], ccuRepLocCpy->GetDataType(), ccuRepLocCpy->GetOpType(), ccuRepLocCpy->GetSem().Id(), ccuRepLocCpy->GetMask(), 0, 0, 1, 1, ccuRepLocCpy->GetReduceFlag());
    }
...
HcclResult CcuInsGeneraterV1::CcuRepBufWriteTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufWrite* ccuRepBufWrite, const TransDep &dep)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(ccuRepBufWrite->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", ccuRepBufWrite->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    TransLocMSToRmtMemInstr(instr++, ccuRepBufWrite->GetDst().addr.Id(), ccuRepBufWrite->GetDst().token.Id(), ccuRepBufWrite->GetSrc().Id(), ccuRepBufWrite->GetLen().Id(), channelImpl->GetChannelId(),
...
HcclResult CcuInsGeneraterV1::CcuRepBufReduceTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepBufReduce* ccuRepBufReduce)
{
    ...
    if (ccuRepBufReduce->GetOpType() == CCU_REDUCE_SUM) {
        if (ccuRepBufReduce->GetOutputDataType() == 1) { // 1是fp16
            AddInstr(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetOutputDataType(), ccuRepBufReduce->GetDataType(), ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), 0, 0, 1, ccuRepBufReduce->GetXnIdLength().Id());
        } else if (ccuRepBufReduce->GetOutputDataType() == 2) { // 2是bf16
            AddInstr(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetOutputDataType(), ccuRepBufReduce->GetDataType(), ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), 0, 0, 1, ccuRepBufReduce->GetXnIdLength().Id());
        } else {
            AddInstr(instr++, msId, ccuRepBufReduce->GetCount(), 0, ccuRepBufReduce->GetDataType(), ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), 0, 0, 1, ccuRepBufReduce->GetXnIdLength().Id());
        }
    } else if (ccuRepBufReduce->GetOpType() == CCU_REDUCE_MAX) {
        MaxInstr(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetDataType(), ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), 0, 0, 1, ccuRepBufReduce->GetXnIdLength().Id());
    } else if (ccuRepBufReduce->GetOpType() == CCU_REDUCE_MIN) {
        MinInstr(instr++, msId, ccuRepBufReduce->GetCount(), ccuRepBufReduce->GetDataType(), ccuRepBufReduce->GetSem().Id(), ccuRepBufReduce->GetMask(), 0, 0, 1, ccuRepBufReduce->GetXnIdLength().Id());
    }
...
HcclResult CcuInsGeneraterV1::CcuRepRemWaitSemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemWaitSem* cuRepRemWaitSem)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(cuRepRemWaitSem->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", cuRepRemWaitSem->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    uint32_t locCkeId{0};
    CHK_PRT_RET(channelImpl->GetLocCkeByIndex(cuRepRemWaitSem->GetSemIndex(), locCkeId) != HcclResult::HCCL_SUCCESS,
...
HcclResult CcuInsGeneraterV1::CcuRepRemPostVarTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemPostVar* ccuRepRemPostVar)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(ccuRepRemPostVar->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", ccuRepRemPostVar->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    uint32_t rmtXnId{0};
    CHK_PRT_RET(channelImpl->GetRmtXnByIndex(ccuRepRemPostVar->GetParamIndex(), rmtXnId) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuRepRemPostSem][%s] failed to get remote xn id, channelHandle[0x%llx].",
            __func__, ccuRepRemPostVar->GetChannel()), HCCL_E_UNAVAIL);

    uint32_t rmtCkeId{0};
    CHK_PRT_RET(channelImpl->GetRmtCkeByIndex(ccuRepRemPostVar->GetSemIndex(), rmtCkeId) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuRepRemPostSem][%s] failed to get remote cke id, channelHandle[0x%llx].",
            __func__, ccuRepRemPostVar->GetChannel()), HCCL_E_UNAVAIL);

    SyncXnInstr(instr++, rmtXnId, ccuRepRemPostVar->GetParam().Id(), channelImpl->GetChannelId(),
                rmtCkeId, ccuRepRemPostVar->GetMask(), 0, 0, 0, 0, 1);
...
HcclResult CcuInsGeneraterV1::CcuRepRemPostSemTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRemPostSem* ccuRepRemPostSem, const TransDep &dep)
{
    ...
    void *channelPtr{nullptr};
    CHK_PRT_RET(static_cast<HcclResult>(HcommChannelGet(ccuRepRemPostSem->GetChannel(), &channelPtr)) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("failed to get ccu channel, type[%d]", ccuRepRemPostSem->Type()), HCCL_E_INTERNAL);

    auto *channelImpl = dynamic_cast<CcuUrmaChannel *>(static_cast<Channel *>(channelPtr));
    CHK_PTR_NULL(channelImpl);
    uint32_t rmtCkeId{0};
    CHK_PRT_RET(channelImpl->GetRmtCkeByIndex(ccuRepRemPostSem->GetSemIndex(), rmtCkeId) != HcclResult::HCCL_SUCCESS,
        HCCL_ERROR("[CcuRepRemPostSem][%s] failed to get remote cke id, channelHandle[0x%llx].",
            __func__, ccuRepRemPostSem->GetChannel()), HCCL_E_UNAVAIL);

    SyncCKEInstr(instr++, rmtCkeId, dep.reserveCkeId, ccuRepRemPostSem->GetMask(), channelImpl->GetChannelId(), 0,
                 0, 0, 0, 1);
...
HcclResult CcuInsGeneraterV1::CcuRepRecordSharedNotifyTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepRecordSharedNotify* ccuRepRecordSharedNotify, const TransDep &dep)
{
    ...
    if (ccuRepRecordSharedNotify->GetNotify().DieId() != dep.dieId) {
        SyncCKEInstr(instr++, ccuRepRecordSharedNotify->GetNotify().Id(), dep.reserveCkeId, ccuRepRecordSharedNotify->GetMask(),
            dep.reserveChannalId[1], 0, 0, 0, 0, 1);
    } else {
        SetCKEInstr(instr++, ccuRepRecordSharedNotify->GetNotify().Id(), ccuRepRecordSharedNotify->GetMask(), 0, 0, 1);
    }
...
HcclResult CcuInsGeneraterV1::CcuRepAddTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepAdd* ccuRepAdd, const TransDep &dep)
{
    ...
    switch (ccuRepAdd->GetSubType()) {
        case AddSubType::ADDR_PLUS_VAR_TO_ADDR: {
            LoadGSAXnInstr(instr++, ccuRepAdd->GetAddrC().Id(), ccuRepAdd->GetAddrA().Id(), ccuRepAdd->GetVarB().Id());
            break;
        }
        case AddSubType::ADDR_PLUS_ADDR_TO_ADDR: {
            LoadGSAGSAInstr(instr++, ccuRepAdd->GetAddrC().Id(), ccuRepAdd->GetAddrA().Id(), ccuRepAdd->GetAddrB().Id());
            break;
        }
        ...
        case AddSubType::SELF_ADD_ADDRESS: {
            LoadGSAXnInstr(instr++, ccuRepAdd->GetAddrA().Id(), ccuRepAdd->GetAddrA().Id(), ccuRepAdd->GetVarB().Id());
            break;
        }
...
HcclResult CcuInsGeneraterV1::CcuRepAssignTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, CcuRepAssign* ccuRepAssign, const TransDep &dep)
{
    ...
    switch (ccuRepAssign->GetSubType()) {
        ...
        case AssignSubType::ADDR_TO_ADDR: {
            LoadGSAGSAInstr(instr++, ccuRepAssign->GetAddrB().Id(), ccuRepAssign->GetAddrA().Id(), dep.reserveGsaId);
            break;
        }
        ...
        for (uint32_t i = 0; i < outArgs.size(); i++) {
            if (outArgs[i].type == CcuArgType::VARIABLE) {
                ...
                for (uint32_t j = 0; j < outArgs[i].varList.size(); j++) {
                    LoadXXInstr(instr++, funcManager->GetFuncOut()[iOutArg++].Id(),
                                outArgs[i].varList[j].Id(), dep.reserveXnId);
                    curInstrId++;
                }
            }
        }
                                            ...
                                            CcuRepReferenceManager* funcManager, uint16_t reserveXnId)
{
    ...
    for (uint32_t i = 0; i < outArgs.size(); i++) {
        if (outArgs[i].type == CcuArgType::VARIABLE) {
            LoadXXInstr(instr + offset + idx, outArgs[i].var.Id(), funcManager->GetFuncOut()[idx].Id(), reserveXnId);
            idx++;
        } else if (outArgs[i].type == CcuArgType::VARIABLE_LIST) {
            for (uint32_t j = 0; j < outArgs[i].varList.size(); j++) {
                LoadXXInstr(instr + offset + idx, outArgs[i].varList[j].Id(), funcManager->GetFuncOut()[idx].Id(),
                            reserveXnId);
                idx++;
            }
        }
    ...
    CcuRepFuncCall* funcCallPtr, const TransDep &dep)
{
    ...
    if (funcBlock != nullptr) {
        ...
        LoadXXInstr(instr + inArgCount + locId++,
                    funcManager->GetFuncCall().Id(), funcCallPtr->GetFuncAddrVar().Id(), dep.reserveXnId);
    }
 
    LoadImdToXnInstr(instr + inArgCount + locId++, funcManager->GetFuncRet(funcCallPtr->GetCallLayer()).Id(),
    ...
    CcuRepJumpNE* jumpNEPtr, const TransDep &dep)
{
    ...
    JumpInstr(jumpNEPtr->GetInstr() + 1, jumpNEPtr->GetTargetInstrId().Id(), jumpNEPtr->GetCondition().Id(),
    ...
    CcuRepJumpEQ* jumpEQPtr, const TransDep &dep)
{
    ...
    LoadImdToXnInstr(startInstr + localInstrIndex++, targetInstrId.Id(), jumpEQPtr->StartInstrId() + 4); // 需要指向NOP位置，为输入指令Id + 4
    JumpInstr(startInstr + localInstrIndex++, targetInstrId.Id(), jumpEQPtr->GetCondition().Id(), jumpEQPtr->GetExpectedNum());
...
HcclResult CcuInsGeneraterV1::CcuRepLoopTranslate(CcuKernel* ccuKernel, CcuInstr *&instr, uint16_t &curInstrId, CcuRepLoop* loopPtr)
{
    ...
    LoopInstr(instr++, loopBlock->StartInstrId(),
                loopBlock->StartInstrId() + loopBlock->InstrCount() - 1, loopPtr->GetLoopParam()->Id());
    return HcclResult::HCCL_SUCCESS;
}
    ...
    CcuRepLoopGroupBundle* bundlePtr, const TransDep &dep)
{
    ...
    if (bundlePtr->GetLayout() == CcuRepLoopGroupBundle::Layout::Config) {
        uint64_t parallelImm = GetParallelParam(bundlePtr->GetConfig().cloneNum, bundlePtr->GetRepeatLoopIdx(),
                                                bundlePtr->GetTotalLoopNum());
        LoadImdToXnInstr(instr++, bundlePtr->GetParallelVar().Id(), parallelImm);
        curInstrId++;

        uint64_t offsetImm = ::hcomm::CcuRep::GetOffsetParam(bundlePtr->GetConfig().addrOffset,
                                                             bundlePtr->GetConfig().ccuBufferOffset,
                                                             bundlePtr->GetConfig().eventOffset);
        LoadImdToXnInstr(instr++, bundlePtr->GetOffsetParam().Id(), offsetImm);
        curInstrId++;
    }

    LoopGroupInstr(instr++, curInstrId + 3, bundlePtr->GetParallelVar().Id(), bundlePtr->GetOffsetParam().Id(), 0);  // 向后3条为loop指令
    curInstrId++;
...
uint16_t CcuInsGeneraterV1::CcuRepLoopGroupBundleInstrCount(CcuRepLoopGroupBundle* bundlePtr)
{
    ...
    // 每 loop：config 1 条载入 / var 2 条；config bundle 额外 2 条(parallel+offset)；+loopgroup1 +跳过2 +每loop Loop1 +收尾1
