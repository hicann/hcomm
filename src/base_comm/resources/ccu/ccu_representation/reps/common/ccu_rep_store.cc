...
namespace CcuRep {
    ...
    bool CcuRepStore::Translate(CcuKernel* ccuKernel, CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        translated = true;
        CHK_PRT_THROW(
            insGeneratorPtr_->CcuRepStoreTranslate(ccuKernel, instr, instrId, this, dep) != HcclResult::HCCL_SUCCESS,
            HCCL_ERROR("[CcuRepStore][Translate] failed to translate for instrId[%u]", instrId), Hccl::CcuApiException,
