...
namespace CcuRep {
    ...
    bool CcuRepLoadArg::Translate(CcuKernel* ccuKernel, CcuInstr*& instr, uint16_t& instrId, const TransDep& dep)
    {
        ...
        CHK_PRT_THROW(
            insGeneratorPtr_->CcuRepLoadArgTranslate(ccuKernel, instr, instrId, this, dep) != HcclResult::HCCL_SUCCESS,
            HCCL_ERROR("[CcuRepLoadArg][Translate] failed to translate for instrId[%u]", instrId),
            Hccl::CcuApiException, "CcuRepLoadArg translate failed");
