...
void TransLocMemToLocMem(CcuInstr *instr, uint16_t dst, uint16_t dstToken, uint16_t src, uint16_t srcToken,
                         uint16_t len, uint16_t usedMSId, uint16_t usedMSNum, uint16_t setCKEId, uint16_t setCKEMask,
...
void SyncAtX(CcuInstr *instr, uint16_t dstAddr,
    uint16_t dstToken, uint16_t srcId, uint16_t channelId, uint16_t setCKEId,
             uint16_t setCKEMask)
{
...
// MSA~MSH Reduce到 MSA
inline void Reduce(CcuInstr *instr, const uint16_t *ms,
    uint16_t count, uint16_t castEn, uint16_t dataType, uint16_t setCKEId,
...
void ReduceAdd(CcuInstr *instr, const uint16_t *ms,
    uint16_t count, uint16_t castEn, uint16_t dataType, uint16_t setCKEId,
               uint16_t setCKEMask, uint16_t XnIdLength)
{
    if (instr == nullptr) {
...
void ReduceMax(CcuInstr *instr, const uint16_t *ms,
    uint16_t count, uint16_t dataType, uint16_t setCKEId, uint16_t setCKEMask,
    uint16_t XnIdLength)
{
    if (instr == nullptr) {
...
void ReduceMin(CcuInstr *instr, const uint16_t *ms,
    uint16_t count, uint16_t dataType, uint16_t setCKEId, uint16_t setCKEMask,
    uint16_t xnIdLength)
{
    if (instr == nullptr) {
...
std::string ParseLoadSqeArgsToX(const CcuInstr *instr)
{
    ...
    return Hccl::StringFormat("Load SqeArg[%u] to Xn[%u], Set CKE[%u:%04x]", sqeArgsId, xnId, setCKEId, setCKEMask);
}
...
std::string ParseLoadImdToXn(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Load immediate[%llu] to Xn[%u], Set CKE[%u:%04x]", 
        immediate, xnId, setCKEId, setCKEMask);
}
...
std::string ParseClearX(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("CleanX: xnId[%u] to xmId[%u], Set CKE[%u:%04x]", xnId, xmId, setCKEId, setCKEMask);
}
...
std::string ParseAdd(const CcuInstr *instr)
{
    ...
    if (instr->v2.operate.parMode == 0) {
        return Hccl::StringFormat("AddI:xdId[%u] = xnId[%u] + immediata[%u], Set CKE[%u:%04x], parMode[%u]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    } else {
        return Hccl::StringFormat("Add:xdId[%u] = xnId[%u] + xmId[%u], Set CKE[%u:%04x], parMode[%u]", xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    }
}
...
std::string ParseSub(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    if (instr->v2.operate.parMode == 0) {
        return Hccl::StringFormat("SubI:xdId[%u] = xnId[%u] - immediata[%u], Set CKE[%u:%04x], parMode[%u]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    } else {
        return Hccl::StringFormat("Sub:xdId[%u] = xnId[%u] - xmId[%u], Set CKE[%u:%04x], parMode[%u]", xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    }
}
...
std::string ParseMul(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    if (instr->v2.operate.parMode == 0) {
        return Hccl::StringFormat("MulI:xdId[%u] = xnId[%u] * immediata[%u], Set CKE[%u:%04x], parMode[%u]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    } else {
        return Hccl::StringFormat("Mul:xdId[%u] = xnId[%u] * xmId[%u], Set CKE[%u:%04x], parMode[%u]", xdId, xnId, xmId,
                            setCKEId, setCKEMask, parMode);
    }
}
...
std::string ParseAnd(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("And:xdId[%u] = xnId[%u] & xmId[%u], Set CKE[%u:%04x]", 
        xdId, xnId, xmId, setCKEId, setCKEMask);
}
...
std::string ParseOr(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Or:xdId[%u] = xnId[%u] | xmId[%u], Set CKE[%u:%04x]", 
        xdId, xnId, xmId, setCKEId, setCKEMask);
}
...
std::string ParseXor(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Xor:xdId[%u] = xnId[%u] ^ xmId[%u], Set CKE[%u:%04x]", 
        xdId, xnId, xmId, setCKEId, setCKEMask);
}
...
std::string ParseShl(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    if (shiftType == 0) {
        return Hccl::StringFormat("logic left shift:xdId[%u] = xnId[%u] << xmId[%u], Set CKE[%u:%04x]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask);
    } else {
        return Hccl::StringFormat("arithmetic left shift:xdId[%u] = xnId[%u] << xmId[%u], Set CKE[%u:%04x]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask);
    }
}
...
std::string ParseShr(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    if (shiftType == 0) {
        return Hccl::StringFormat("logic right shift:xdId[%u] = xnId[%u] >> xmId[%u], Set CKE[%u:%04x]", 
            xdId, xnId, xmId,
                            setCKEId, setCKEMask);
    } else {
        return Hccl::StringFormat("arithmetic right shift:xdId[%u] = xnId[%u] >> xmId[%u], Set CKE[%u:%04x]", 
            xdId, xnId,
...
std::string ParseStoreXToMem(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("StoreXToMem: xdId[%u] xdtId[%u], xsId[%u], xlId[%u], xhId[%u], srcType[%u], allocHint[%u], "
...
std::string ParseLoopGroup(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("LoopGroup From startLoopInstrId[%u] with loopGroupXn[%u], resOffsetXn[%u], xnOffset[%u]",
...
std::string ParseJump(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("When conditionXn[%u] condition with[%u] expectedXn[%u], Jump To RelInstrIdXn[%u]",
...
std::string ParseWait(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Wait conditionXn[%u] condition with[%u] expectedXn[%u]", conditionXnId, conditionType,
                        expectedXnId);
}
...
std::string ParseSetCKE(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Wait CKE[%u:%04x], Set CKE[%u:%04x], clearType[%u], userData[%llu]", 
...
std::string ParseClearCKE(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Wait CKE[%u:%04x], Clear CKE[%u:%04x], clearType[%u]", 
        waitCKEId, waitCKEMask, clearCKEId,
                        clearMask, clearType);
}
...
std::string ParseTransLocMemToLocMS(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Trans LocMem[%u:%u] To LocMS[%u] With LengthXn[%u], Offset[%u], Set CKE[%u:%04x], "
...
std::string ParseTransLocMSToLocMem(const CcuInstr *instr)
{
    ...
    // 使用 StringFormat 格式化字符串
    return Hccl::StringFormat("Trans LocMS[%u] To LocMem[%u:%u] With LengthXn[%u], Offset[%u], Set CKE[%u:%04x], "
                        "allocHint[%u], victimHint[%u]",
...
std::string ParseSyncWtX(const CcuInstr *instr)
{
    ...
    if (hasNotify) {
        ...
        return Hccl::StringFormat("Sync Xn[%u] To Mem[%u:%u] Use ChannelXn[%u], and Set CKE[%u:%04x], parMode[%u]", 
            xn, dst, dstToken, channelId, setCKEId, setCKEMask, parMode);
    }
}
...
std::string ParseSyncAtX(const CcuInstr *instr)
{
    ...
    return Hccl::StringFormat("Atomic Sync xsId[%u] To Mem[%u:%u] Use ChannelXn[%u], parMode[%u], setCKEId[%u:%04x]", 
        xsId,
...
std::string ParseReduce(const CcuInstr *instr)
{
    ...
    return Hccl::StringFormat("Reduce %s, count[%u], reduceType[%u], castEn[%u] and dataType[%u], Set CKE[%u:%04x]",
