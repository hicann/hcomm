...
public:
    ...
    CcuResult Acquire(CcuInsHandle insHandle, CcuResRepository &resRepo, CcuVarEventType type,
        uint8_t dieId, uint32_t num, uint64_t &handle);
...
private:
    ...
    CcuResult AllocAndRecord(CcuInsHandle insHandle, CcuResRepository &resRepo,
    ...
    static CcuResult AllocFromPool(std::vector<ResInfo> &pool, uint32_t num,
