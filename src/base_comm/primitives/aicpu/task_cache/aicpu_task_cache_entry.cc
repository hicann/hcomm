...
AddrRefreshInfo::AddrRefreshInfo()
    : needRefresh(false), memIdx(0), offset(0)
{
}

AddrRefreshInfo::AddrRefreshInfo(const uint32_t curMemIdx)
    : needRefresh(true), memIdx(curMemIdx), offset(0)
{
}

AddrRefreshInfo::AddrRefreshInfo(const AddrRefreshInfo& other)
    : needRefresh(other.needRefresh), memIdx(other.memIdx), offset(other.offset)
{
}
...
HcclResult AicpuTaskCacheEntry::InitCacheEntry(const uint64_t* baseAddrs, const uint64_t* memSizes, const uint32_t count)
{
    ...
    // 缓存sizes
    CHK_PRT_RET(cachedMemSizes_.size() != 0,
    ...
    const uint8_t* sqeArray, const uint32_t streamId)
{
    ...
    HCCL_INFO("[AicpuTaskCacheEntry][AddSqeArray] add %uth sqe array with sqeCount[%llu] streamId[%u]",
...
{
    CHK_PRT_RET(isReportTask != dbSqeProfInfo.isValid,
        HCCL_ERROR("[AicpuTaskCacheEntry][AddWqeArray] isReportTask[%d] != dbSqeProfInfo.isValid[%d]",
    ...
    HCCL_INFO("[AicpuTaskCacheEntry][AddWqeArray] add %uth wqe array with wqeCount[%u] dbSqeTmpInfo[%u, %d, %u] streamId[%u]",
        wqeTaskArrayInfos_.size() - 1, wqeCount, dbSqeTmpInfo.wqeArrayIdx, dbSqeTmpInfo.isReportTask, dbSqeTmpInfo.dbSqeIdx, streamId);
...
HcclResult AicpuTaskCacheEntry::SubmitCacheEntry()
{
    ...
    if (UNLIKELY(streamIdToDbSqeTmpInfoMap_.size() != 0)) {
        HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] streamIdToDbSqeTmpInfoMap_.size[%u] != 0",
            streamIdToDbSqeTmpInfoMap_.size());
        for (const auto& mapIter : streamIdToDbSqeTmpInfoMap_) {
            HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] streamId[%u] wqeArrayIdx[%u] dbSqeIdx[%u] isReportTask[%d]",
                mapIter.first, mapIter.second[0].wqeArrayIdx, mapIter.second[0].dbSqeIdx, mapIter.second[0].isReportTask);
        }
    ...
    CHK_PRT_RET(count == 0,
        HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] cachedBaseAddrs_.size is 0"),
        HCCL_E_INTERNAL);
...
inline HcclResult AicpuTaskCacheEntry::SubmitSqeAddrRefreshInfo_()
{
    ...
    for (size_t arrayIdx = 0; arrayIdx < sqeArrayInfos_.size(); arrayIdx++) {
        ...
        for (size_t sqeIdx = 0; sqeIdx < sqeCount; sqeIdx++) {
            CHK_RET(UpdateSqeAddrRefreshInfo_(sqePtr, sqeArrayInfo.srcAddrRefreshInfoArray[sqeIdx],
                sqeArrayInfo.dstAddrRefreshInfoArray[sqeIdx]));

            // 打印SQE AddrRefreshInfo
            HCCL_INFO("[AicpuTaskCacheEntry][SubmitCacheEntry] sqeArrayInfos_[%u][%u]: "
...
inline HcclResult AicpuTaskCacheEntry::SubmitWqeAddrRefreshInfoAndTokenInfo_()
{
    ...
    for (size_t arrayIdx = 0; arrayIdx < wqeTaskArrayInfos_.size(); arrayIdx++) {
        ...
        UbTransportLiteImpl* ubTransportLiteImplPtr = wqeTaskArrayInfo.ubTransportLiteImplPtr; // 注意: AddWqeArray时已校验非空
        std::unordered_map<UbTransportLiteImplHandle, vector<TokenInfo>>::iterator iter
            = tokenInfosMap_.find(ubTransportLiteImplPtr);

        CHK_PRT_RET(iter == tokenInfosMap_.end(),
            HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] ubTransportLiteImplPtr[%p] not found in tokenInfosMap_",
        ...
        for (size_t wqeIdx = 0; wqeIdx < wqeCount; wqeIdx++) {
            CHK_RET(UpdateWqeAddrRefreshInfoAndTokenInfo_(wqeTasks[wqeIdx], wqeTaskArrayInfo.locAddrRefreshInfoArray[wqeIdx],
                wqeTaskArrayInfo.rmtAddrRefreshInfoArray[wqeIdx], tokenInfos));

            // 打印WQE AddrRefreshInfo
            HCCL_INFO("[AicpuTaskCacheEntry][SubmitCacheEntry] wqeTaskArrayInfos_[%u][%u]: "
    ...
    for (std::unordered_map<DbSqeLocation, DbSqeProfAndRefreshInfo>::iterator iter = dbSqeLocInfoMap_.begin();
        ...
        switch (dbSqeProfInfo.taskParamType) {
            ...
            case TaskParamType::TASK_UB_INLINE_WRITE:
                ...
                break;
            default:
                HCCL_ERROR("[AicpuTaskCacheEntry][%s] invalid taskParamType[%u]", __func__, dbSqeProfInfo.taskParamType);
                return HCCL_E_INTERNAL;
        }
    }
    return HCCL_SUCCESS;
...
inline HcclResult AicpuTaskCacheEntry::ValidateLaunchOrder_()
{
    ...
    for (size_t launchIdx = 0; launchIdx < launchOrder_.size(); launchIdx++) {
        ...
        if (taskType == TaskArrayType::kTaskArrayTypeSqe) {
            // 校验arrayIdx
            CHK_PRT_RET(sqeArrayIdx >= sqeArrayInfos_.size(),
                HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] sqeArrayIdx[%u] >= sqeArrayInfos_.size[%u]",
                    sqeArrayIdx, sqeArrayInfos_.size()),
                HCCL_E_PARA);
            ++sqeArrayIdx;
        } else if (taskType == TaskArrayType::kTaskArrayTypeWqe) {
            // 校验arrayIdx
            CHK_PRT_RET(wqeArrayIdx >= wqeTaskArrayInfos_.size(),
                HCCL_ERROR("[AicpuTaskCacheEntry][SubmitCacheEntry] wqeArrayIdx[%u] >= wqeTaskArrayInfos_.size[%u]",
...
{
    // 校验count
    CHK_PRT_RET(cachedBaseAddrs_.size() != count || cachedMemSizes_.size() != count,
        HCCL_ERROR("[AicpuTaskCacheEntry][RefreshAndLaunch] cachedBaseAddrs_.size[%u] / cachedMemSizes_.size[%u] != count[%u]",
    ...
    for (uint32_t memIdx = 0; memIdx < count; memIdx++) {
        CHK_PRT_RET(cachedMemSizes_[memIdx] != memSizes[memIdx],
            HCCL_ERROR("[AicpuTaskCacheEntry][RefreshAndLaunch] cachedMemSizes_[%u] != memSizes[%u]",
                memIdx, memIdx),
            HCCL_E_INTERNAL);
    }
    ...
    HCCL_INFO("[AicpuTaskCacheEntry][RefreshAndLaunch] l1State[%d] enableTaskException[%d] -> needTaskParam[%d]",
        l1State, enableTaskException, needTaskParam);
...
inline HcclResult AicpuTaskCacheEntry::RefreshTokenInfos_(const uint64_t* baseAddrs, const uint64_t* memSizes, const uint32_t count)
{
    ...
    for (std::unordered_map<UbTransportLiteImplHandle, vector<TokenInfo>>::iterator iter = tokenInfosMap_.begin();
        ...
        vector<TokenInfo>& tokenInfos = iter->second;
        CHK_PRT_RET(tokenInfos.size() != count,
            HCCL_ERROR("[AicpuTaskCacheEntry][RefreshAndLaunch] tokenInfos.size[%u] != count[%u]",
        ...
        for (uint32_t memIdx = 0; memIdx < count; memIdx++) {
            ...
            if (tokenInfo.needLocTokenIdFlag) {
                ...
                CHK_RET(ubTransportLitePtr->BuildLocRmaBufferLite(reinterpret_cast<uintptr_t>(baseAddr), memSize, locRmaBuf));
...
inline HcclResult AicpuTaskCacheEntry::LaunchTasksByOrder_(const uint64_t* baseAddrs, const uint64_t* memSizes,
...
inline HcclResult AicpuTaskCacheEntry::PrintRefreshResult_(const uint64_t* baseAddrs, const uint64_t* memSizes, const uint32_t count)
{
    // 打印更新后的token信息
    for (std::unordered_map<UbTransportLiteImplHandle, vector<TokenInfo>>::iterator iter = tokenInfosMap_.begin();
        iter != tokenInfosMap_.end(); iter++) {
        ...
        for (uint32_t memIdx = 0; memIdx < count; memIdx++) {
            TokenInfo& tokenInfo = tokenInfos[memIdx];
            HCCL_INFO("[AicpuTaskCacheEntry][RefreshAndLaunch] tokenInfosMap_[0x%016llx][%u]: "
    ...
    for (size_t launchIdx = 0; launchIdx < launchOrder_.size(); launchIdx++) {
        const TaskArrayType taskType = launchOrder_[launchIdx];
        if (taskType == TaskArrayType::kTaskArrayTypeSqe) {
            HCCL_INFO("[AicpuTaskCacheEntry][RefreshAndLaunch] launchedArray[%u] is sqeArray[%u]", launchIdx, sqeArrayIdx);
            ++sqeArrayIdx;
        } else if (taskType == TaskArrayType::kTaskArrayTypeWqe) {
            HCCL_INFO("[AicpuTaskCacheEntry][RefreshAndLaunch] launchedArray[%u] is wqeArray[%u]", launchIdx, wqeArrayIdx);
            ++wqeArrayIdx;
        } else {
            HCCL_ERROR("[AicpuTaskCacheEntry][RefreshAndLaunch] invalid task array type[%u]", taskType);
            return HCCL_E_INTERNAL;
    ...
    uint8_t *newSqeArray, const size_t sqeBytes, const uint8_t *sqeArray, const uint32_t streamId)
{
    ...
    if (mapIter != streamIdToDbSqeTmpInfoMap_.end()) {
        ...
        for (size_t i = 0; i < mapIter->second.size(); ++i) {
            ...
            CHK_PRT_RET(dbSqeTmpInfo.wqeArrayIdx >= wqeTaskArrayInfos_.size(),
                HCCL_ERROR("[AicpuTaskCacheEntry][AddSqeArray_] streamIdToDbSqeTmpInfoMap_[%u][%u].wqeArrayIdx[%u] >= "
            ...
            if (dbSqeTmpInfo.isReportTask) {
                ...
                CHK_PRT_RET(dbSqeLocInfoMap_.find(dbSqeLocation) != dbSqeLocInfoMap_.end(),
                    HCCL_ERROR("[AicpuTaskCacheEntry][AddSqeArray_] DbSqeLocation[%u, %u] already exists in "
                ...
                dbSqeProfAndRefreshInfo.wqeArrayIdx = dbSqeTmpInfo.wqeArrayIdx; // 注意: 本函数已校验dbSqeTmpInfo.wqeArrayIdx
                dbSqeLocInfoMap_.emplace(dbSqeLocation, dbSqeProfAndRefreshInfo);
            }

            HCCL_INFO("[AicpuTaskCacheEntry][AddSqeArray_] update dbSqeLocation[%u, %u] isReportTask[%d]",
...
HcclResult AicpuTaskCacheEntry::UpdateWqeAddrRefreshInfoAndTokenInfo_(const WqeTask& wqeTask, AddrRefreshInfo& locAddrRefreshInfo,
...
inline HcclResult AicpuTaskCacheEntry::UpdateTokenFlagsByAddrRefreshInfo_(const AddrRefreshInfo& addrRefreshInfo,
    vector<TokenInfo>& tokenInfos, bool isLoc)
{
    if (!addrRefreshInfo.needRefresh) {
        return HCCL_SUCCESS;
    }
    CHK_PRT_RET(addrRefreshInfo.memIdx >= tokenInfos.size(),
        HCCL_ERROR("[AicpuTaskCacheEntry][UpdateWqeAddrRefreshInfo_] %s memIdx[%u] >= tokenInfos.size[%u]",
...
inline HcclResult AicpuTaskCacheEntry::RefreshSqeTasks_(
    const SqeArrayInfo &sqeArrayInfo, const uint64_t *baseAddrs)
{
    // sqe数组
    ...
    if (UNLIKELY(isTaskConfigDebug_)) {
        PLF_CONFIG_DEBUG(PLF_TASK, "[AicpuTaskCacheEntry][RefreshSqeTasks_] dump %llu cached SQEs in stream[%u]",
            sqeCount, rtsqA5Ptr->GetStreamId());
    }
    for (size_t sqeIdx = 0; sqeIdx < sqeCount; ++sqeIdx) {
        // 获取当前SQE的信息
        Rt91095StarsSqeHeader *sqeHeaderPtr = (Rt91095StarsSqeHeader *)sqeArrayPtr;
        CHK_RET(RefreshOneSqe_(sqeArrayPtr, sqeSrcAddrRefreshInfoArray[sqeIdx],
            sqeDstAddrRefreshInfoArray[sqeIdx], baseAddrs));
    ...
    if (UNLIKELY(isTaskConfigDebug_)) {
        ...
        for (size_t sqeIdx = 0; sqeIdx < sqeCount; ++sqeIdx) {
            // 打印当前SQE
            PLF_CONFIG_DEBUG(PLF_TASK, "[AicpuTaskCacheEntry][RefreshSqeTasks_] %uth cached SQE in stream[%u]",
                sqeIdx, rtsqA5Ptr->GetStreamId());
...
inline HcclResult AicpuTaskCacheEntry::RefreshOneSqe_(uint8_t *sqeArrayPtr, const AddrRefreshInfo &srcAddrRefreshInfo,
    ...
    switch (sqeType) {
        ...
        case Rt91095StarsSqeType::RT_91095_SQE_TYPE_SDMA: {
            ...
            if (srcAddrRefreshInfo.needRefresh) {
                RefreshTaskAddr_(memcpySqePtr->u.strideMode0.srcAddrLow, memcpySqePtr->u.strideMode0.srcAddrHigh,
                    srcAddrRefreshInfo, baseAddrs);
            }
            if (dstAddrRefreshInfo.needRefresh) {
                RefreshTaskAddr_(memcpySqePtr->u.strideMode0.dstAddrLow, memcpySqePtr->u.strideMode0.dstAddrHigh,
                    dstAddrRefreshInfo, baseAddrs);
            }
            break;
        }
        case Rt91095StarsSqeType::RT_91095_SQE_TYPE_WRITE_VALUE: {
...
inline void AicpuTaskCacheEntry::RefreshTaskAddr_(uint32_t &addrLow, uint32_t &addrHigh,
    const AddrRefreshInfo &addrRefreshInfo, const uint64_t *baseAddrs) const
{
...
inline void AicpuTaskCacheEntry::RefreshTaskAddr_(uint64_t &addr, const AddrRefreshInfo &addrRefreshInfo,
...
{
    PLF_CONFIG_DEBUG(PLF_TASK, "[AicpuTaskCacheEntry][RefreshWqeTasks_] dump %llu cached WQEs in jetty[%u, %u, %u]",
...
inline HcclResult AicpuTaskCacheEntry::DumpWqeTasksPerWqe_(
    size_t wqeIdx, const WqeTask &wqeTask, const UbConnLite *ubConnLitePtr) const
{
    PLF_CONFIG_DEBUG(PLF_TASK, "[AicpuTaskCacheEntry][%s] %uth cached WQE in jetty[%u, %u, %u]", __func__,
...
inline HcclResult AicpuTaskCacheEntry::RefreshWqeTasks_(WqeTaskArrayInfo &wqeTaskArrayInfo, const uint64_t* baseAddrs,
    const uint64_t* memSizes, const uint32_t count)
{
    ...
    UbTransportLiteImpl* ubTransportLitePtr = wqeTaskArrayInfo.ubTransportLiteImplPtr; // 注意: 已在AddWqeArray时校验非空
    ...
    CHK_PRT_RET(constIter == tokenInfosMap_.end(),
        HCCL_ERROR(
...
inline HcclResult AicpuTaskCacheEntry::RefreshWqeRead_(WqeTask &wqeTask, const AddrRefreshInfo &locAddrRefreshInfo,
    const AddrRefreshInfo &rmtAddrRefreshInfo, const uint64_t *baseAddrs, const vector<TokenInfo> &tokenInfos)
{
    // 如果需要刷新loc地址信息
    if (locAddrRefreshInfo.needRefresh) {
        // 刷新loc addr
        RefreshTaskAddr_(wqeTask.wqeWrite.u.sge.dataAddrLow, wqeTask.wqeWrite.u.sge.dataAddrHigh,
    ...
    if (rmtAddrRefreshInfo.needRefresh) {
        // 刷新rmt addr
        RefreshTaskAddr_(wqeTask.wqeWrite.comm.rmtAddrLow, wqeTask.wqeWrite.comm.rmtAddrHigh,
            rmtAddrRefreshInfo, baseAddrs);
        
        // 注意: rmtObjId是位域, 不能直接传引用
        uint32_t rmtObjId = wqeTask.wqeWrite.comm.rmtObjId;
        CHK_RET(RefreshWqeRmtTokenIdAndValue_(rmtObjId, wqeTask.wqeWrite.comm.rmtTokenValue, rmtAddrRefreshInfo, tokenInfos));
        wqeTask.wqeWrite.comm.rmtObjId = rmtObjId;
    }
    return HCCL_SUCCESS;
}

inline HcclResult AicpuTaskCacheEntry::RefreshWqeWrite_(WqeTask &wqeTask, const AddrRefreshInfo &locAddrRefreshInfo,
    const AddrRefreshInfo &rmtAddrRefreshInfo, const uint64_t *baseAddrs, const vector<TokenInfo> &tokenInfos)
{
    const uint32_t inlineEn = wqeTask.wqeWrite.comm.inlineEn;
    if (inlineEn) { // inline write
...
inline HcclResult AicpuTaskCacheEntry::RefreshWqeWriteWithNotify_(WqeTask& wqeTask, const AddrRefreshInfo& locAddrRefreshInfo,
    const AddrRefreshInfo& rmtAddrRefreshInfo, const uint64_t* baseAddrs, const vector<TokenInfo> &tokenInfos)
{
    // 如果需要刷新loc地址信息
    if (locAddrRefreshInfo.needRefresh) {
        // 刷新loc addr
        RefreshTaskAddr_(wqeTask.wqeWriteWithNotify.localU.sge.dataAddrLow,
    ...
    if (rmtAddrRefreshInfo.needRefresh) {
        // 刷新rmt addr
        RefreshTaskAddr_(wqeTask.wqeWriteWithNotify.comm.rmtAddrLow,
            wqeTask.wqeWriteWithNotify.comm.rmtAddrHigh,
            rmtAddrRefreshInfo, baseAddrs);

        // 注意: rmtObjId是位域, 不能直接传引用
        uint32_t rmtObjId = wqeTask.wqeWriteWithNotify.comm.rmtObjId;
        CHK_RET(RefreshWqeRmtTokenIdAndValue_(rmtObjId, wqeTask.wqeWriteWithNotify.comm.rmtTokenValue,
            rmtAddrRefreshInfo, tokenInfos));
        wqeTask.wqeWriteWithNotify.comm.rmtObjId = rmtObjId;
    }
    return HCCL_SUCCESS;
}

inline HcclResult AicpuTaskCacheEntry::RefreshWqeLocTokenId_(uint32_t& tokenId, const AddrRefreshInfo& addrRefreshInfo,
    const vector<TokenInfo> &tokenInfos) const
{
    ...
    CHK_PRT_RET(!tokenInfo.needLocTokenIdFlag,
        HCCL_ERROR("[AicpuTaskCacheEntry][RefreshWqeLocTokenId_] needLocTokenIdFlag is false"),
        HCCL_E_INTERNAL);
...
inline HcclResult AicpuTaskCacheEntry::RefreshWqeRmtTokenIdAndValue_(uint32_t& tokenId, uint32_t& tokenValue,
    const AddrRefreshInfo& addrRefreshInfo, const vector<TokenInfo> &tokenInfos) const
{
    ...
    CHK_PRT_RET(!tokenInfo.needRmtTokenIdAndValueFlag,
...
inline HcclResult AicpuTaskCacheEntry::RefreshDbSqe_(WqeTaskArrayInfo &wqeTaskArrayInfo)
{
    ...
    HCCL_INFO("[AicpuTaskCacheEntry][RefreshDbSqe_] update pi[%u] at dbSqeLocation[%u, %u]",
        pi, dbSqeLocation.sqeArrayIdx, dbSqeLocation.dbSqeIdx);

    return HCCL_SUCCESS;
}

HcclResult AicpuTaskCacheEntry::ReportDbSqeProfiling_(uint8_t *dbSqePtr, size_t arrayIdx, uint32_t dbSqeIdx, const uint64_t *baseAddrs,
    const uint64_t *memSizes, const uint32_t count, Hccl::TaskParam *taskParam, const u32 sqId)
{
    ...
    if (UNLIKELY(it == dbSqeLocInfoMap_.end())) {
        HCCL_INFO("[AicpuTaskCacheEntry][ReportDbSqeProfiling_] dbSqeLocInfoMap_[%u, %u] is not found",
            dbSqeLocation.sqeArrayIdx, dbSqeLocation.dbSqeIdx);
        return HCCL_SUCCESS;
    }

    CHK_PRT_RET(it->second.dbSqeProfInfo.isValid == false,
        HCCL_ERROR("[AicpuTaskCacheEntry][ReportDbSqeProfiling_] dbSqeLocInfoMap_[%u, %u] is invalid",
            dbSqeLocation.sqeArrayIdx, dbSqeLocation.dbSqeIdx),
        HCCL_E_INTERNAL);

    // 获取DbSqe对应的taskId
    Rt91095StarsSqeHeader *sqeHeaderPtr = (Rt91095StarsSqeHeader*)dbSqePtr; // 已在AddSqeArray校验, 无需再校验
    CHK_PRT_RET(static_cast<Rt91095StarsSqeType>(sqeHeaderPtr->type) != Rt91095StarsSqeType::RT_91095_SQE_TYPE_UBDMA,
        HCCL_ERROR("[AicpuTaskCacheEntry][ReportDbSqeProfiling_] sqeHeaderPtr->type[%u] is not RT_91095_SQE_TYPE_UBDMA", sqeHeaderPtr->type),
        HCCL_E_INTERNAL);
    const u32 taskId = (sqeHeaderPtr->taskId << 16) | (sqeHeaderPtr->rtStreamId);

    HCCL_INFO("[AicpuTaskCacheEntry][ReportDbSqeProfiling_] report %lluth DbSqe profiling in %lluth SQE array: dbSqeType[%u], taskId[%u], sqId[%u]",
    ...
    switch (it->second.dbSqeProfInfo.taskParamType) {
        ...
        case TaskParamType::TASK_WRITE_REDUCE_WITH_NOTIFY:
            ...
            break;
        default:
            HCCL_ERROR("[AicpuTaskCacheEntry][ReportDbSqeProfiling_] taskType[%u] is unsupported",
...
inline HcclResult AicpuTaskCacheEntry::RefreshDbSqeProfAddrs_(DbSqeProfAndRefreshInfo &profAndRefreshInfo,
    const uint64_t *baseAddrs, const uint64_t *memSizes, const uint32_t count)
{
    // 注意: dbSqeProfInfo中的src/dstAddr, 需要根据DbSqeProfAndRefreshInfo中的src/dstAddrRefreshInfo进行刷新, 才能构造TaskParam
    if (profAndRefreshInfo.srcAddrRefreshInfo.needRefresh) {
        RefreshTaskAddr_(profAndRefreshInfo.dbSqeProfInfo.srcAddr,
            profAndRefreshInfo.srcAddrRefreshInfo, baseAddrs);
    }
    if (profAndRefreshInfo.dstAddrRefreshInfo.needRefresh) {
        RefreshTaskAddr_(profAndRefreshInfo.dbSqeProfInfo.dstAddr,
            profAndRefreshInfo.dstAddrRefreshInfo, baseAddrs);
    }
    return HCCL_SUCCESS;
}

inline void AicpuTaskCacheEntry::ReportDbSqeCallback_(UbTransportLiteImpl *ubTransportLitePtr,
...
inline HcclResult AicpuTaskCacheEntry::FillTaskParamDma_(Hccl::TaskParam *taskParam,
...
inline HcclResult AicpuTaskCacheEntry::FillTaskParamReduce_(Hccl::TaskParam *taskParam,
    const DbSqeProfAndRefreshInfo &profAndRefreshInfo) const
{
    const DbSqeProfInfo &profInfo = profAndRefreshInfo.dbSqeProfInfo;
...
HcclResult AicpuTaskCacheEntry::ReportSqeProfiling_(uint8_t *sqePtr, size_t arrayIdx, uint32_t sqeIdx, const uint64_t *baseAddrs,
    const uint64_t *memSizes, const uint32_t count, u64 beginTime, const u32 sqId)
{
    ...
    if (sqeType != Rt91095StarsSqeType::RT_91095_SQE_TYPE_UBDMA) {
        std::function<HcclResult(u32, u32, const Hccl::TaskParam&, u64)> callback = sqeArrayInfo.aicpuTsThreadPtr->GetCallback();
        if (callback != nullptr) {
...
inline HcclResult AicpuTaskCacheEntry::FillTaskParamNotify_(Hccl::TaskParam &taskParam, const uint8_t *sqePtr, u64 beginTime) const
{
    Hccl::Rt91095StarsNotifySqe *notifySqe = (Hccl::Rt91095StarsNotifySqe *)sqePtr;
    Rt91095StarsSqeHeader *sqeHeaderPtr = (Rt91095StarsSqeHeader*)sqePtr;
    const Rt91095StarsSqeType sqeType = static_cast<Rt91095StarsSqeType>(sqeHeaderPtr->type);
    taskParam.taskType =
        ((sqeType == Rt91095StarsSqeType::RT_91095_SQE_TYPE_NOTIFY_RECORD) ? TaskParamType::TASK_NOTIFY_RECORD : TaskParamType::TASK_NOTIFY_WAIT);
...
inline HcclResult AicpuTaskCacheEntry::FillTaskParamSdma_(Hccl::TaskParam &taskParam, const uint8_t *sqePtr, u64 beginTime) const
{
    Hccl::Rt91095StarsMemcpySqe *sdmaSqe = (Hccl::Rt91095StarsMemcpySqe *)sqePtr;
    taskParam.beginTime = beginTime;
    if (sdmaSqe->opcode == 0) {
        taskParam.taskType = Hccl::TaskParamType::TASK_SDMA;
        taskParam.taskPara.DMA.src = reinterpret_cast<void *>(((static_cast<uint64_t>(sdmaSqe->u.strideMode0.srcAddrHigh)) << 32) | sdmaSqe->u.strideMode0.srcAddrLow);
        taskParam.taskPara.DMA.dst = reinterpret_cast<void *>(((static_cast<uint64_t>(sdmaSqe->u.strideMode0.dstAddrHigh)) << 32) | sdmaSqe->u.strideMode0.dstAddrLow);
        ...
        taskParam.taskPara.Reduce.src = reinterpret_cast<void *>(((static_cast<uint64_t>(sdmaSqe->u.strideMode0.srcAddrHigh)) << 32) | sdmaSqe->u.strideMode0.srcAddrLow);
        taskParam.taskPara.Reduce.dst = reinterpret_cast<void *>(((static_cast<uint64_t>(sdmaSqe->u.strideMode0.dstAddrHigh)) << 32) | sdmaSqe->u.strideMode0.dstAddrLow);
...
HcclResult AicpuTaskCacheEntry::ReportSqeArrayProfiling_(size_t arrayIdx, const uint64_t *baseAddrs,
