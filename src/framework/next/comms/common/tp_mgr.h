...
using GetTpInfoParam = struct GetTpInfoParamDef {
    ...
    /// CCU 设备环回 GetTpInfo：与通信域 hcclQos 解耦，SL 来自 GetTpAttr.slBitmap；写回 SL 经 HrtRaSetTpAttrAsync（HDC）
...
/*
 * TP 信息：当前仅申请 TpHandle，不感知具体 TP 详情，当前仅支持 TP 与 CTP
 * tpHandle: 对应管控面的 TPID 与相关资源，URMA 通过引用计数管理申请与销毁 TP
 */
...
public:
    ...
    // unimport jetty 会由 URMA 销毁 tp 资源，hccl 配套删除记录
...
private:
    ...
    /*
     * Request 上下文，保存查询 TP 信息相关异步接口出参
     * handle: 异步接口调用 handle，用于查询处理结果
     * tpInfoNum: 查询到的 TP 信息个数，当前为复用 TP，只会申请 1 个
     * dataBuffer: 查询到的 TP 信息数据，原始数据保留缓冲区
     */
    ...
    /// 三级索引：先按本端 IP，再按对端 IP，最后按 QoS 档（0–255，与 GetTpInfo/TP-SL 策略里用的档位一致）
...
private:
    ...
    HcclResult PollGetTpInfoReqCtx(const GetTpInfoParam &param, TpInfo &tpInfo);
    ...
    HcclResult CommitTpInfoToCache(const GetTpInfoParam &param, TpInfo tmpTpInfo, TpInfo &tpInfo);
