    ...
    hccl::hcclComm *hcclComm)
{
    ...
    HCCL_INFO("[%s] channelProtocol[%d] ub comm-domain qos applied in HcommChannelDesc::qos when converting "
        "(HcclChannelDesc has no qos field)",
...
static void LogCcuTaskInfo(const std::vector<hcomm::CcuTaskParam> &ccuParams, const CcuKernelHandle kernelHandle)
{
    ...
    for (u32 idx = 0; idx < ccuParams.size(); idx++) {
        ...
        HCCL_INFO("[LaunchCcuTasks] start ccu task, dieId[%u] missionId[%u] instStartId[%u] instCnt[%u], "
            "argSize[%u], timeout[%u]s", param.dieId, param.missionId,
            param.instStartId, param.instCnt, param.argSize, execTimeOutSec);
        constexpr std::size_t TOKEN_VALUE_INDEX = 2;
        for (std::size_t i = 0; i < param.argSize; i++) {
            if (i == TOKEN_VALUE_INDEX) { continue; }
            HCCL_INFO("[LaunchCcuTasks] arg[%lu] = %lu", i, param.args[i]);
        }
        HCCL_INFO("[ConstructCcuTaskParam]dieId[%u], missionId[%u], execMissionId[%u], instrId[%u], "
            "executeId[0x%llx], ccuKernelHandle[0x%llx]",
            param.dieId, param.missionId, param.missionId,
            param.instStartId, kernelHandle, kernelHandle);
    }
}

std::shared_ptr<std::vector<Hccl::CcuProfilingInfo>> ConstructCcuDetailInfo(
...
Hccl::TaskParam ConstructCcuTaskParam(const hcomm::CcuTaskParam &ccuParam, const CcuKernelHandle kernelHandle,
    const std::shared_ptr<std::vector<Hccl::CcuProfilingInfo>> &ccuDetailInfo, u64 beginTime, u64 endTime, bool isMaster)
{
...
HcclResult ConstructProfilingInfo(const hcomm::CcuTaskArg &arg, hcomm::CcuKernel *kernel, const HcclComm comm,
    std::vector<hcomm::CcuProfilingInfo> &allCcuProfilingInfo, bool isOpBase)
{
    ...
    // 没开profiling 而且是单算子模式
    if (!isProfilingEnabledL1 && !isProfilingEnabledL0 && isOpBase) {
        return HCCL_SUCCESS;
    }
    ...
    const CcuKernelHandle kernelHandle, void *taskArgs)
{
    ...
    CHK_PTR_NULL(hcclCommDfx);
    bool isOpBase = false;
    CHK_RET(hcclCommDfx->IsOpBase(isOpBase));

    std::vector<hcomm::CcuProfilingInfo> allCcuProfilingInfo;
    CHK_RET(ConstructProfilingInfo(*ccuTaskArgs, kernel, comm, allCcuProfilingInfo, isOpBase));
    LogCcuTaskInfo(ccuParams, kernelHandle);
    auto ccuDetailInfo = ConstructCcuDetailInfo(allCcuProfilingInfo);
    for (u32 idx = 0; idx < ccuParams.size(); idx++) {
        u64 beginTime = Hccl::DlProfFunction::GetInstance().dlMsprofSysCycleTime();
        CHK_RET(LaunchCcuTasks(ccuParams[idx], streamPtr));
        u64 endTime = Hccl::DlProfFunction::GetInstance().dlMsprofSysCycleTime();
        Hccl::TaskParam taskParam = ConstructCcuTaskParam(ccuParams[idx], kernelHandle, ccuDetailInfo,
