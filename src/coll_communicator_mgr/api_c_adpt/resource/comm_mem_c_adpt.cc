...

{
    ...
    CHK_PRT_RET(
        ...
        HCCL_E_PARA);
    CHK_PRT_RET(
        memTagStr.compare(0, strlen(HCCL_TEAM_USERMEM_TAG_PREFIX), HCCL_TEAM_USERMEM_TAG_PREFIX) == 0,
        HCCL_ERROR(
            "[HcclCommMemReg]memTag[%s] uses reserved team usermem prefix[%s]", memTag, HCCL_TEAM_USERMEM_TAG_PREFIX),
        HCCL_E_PARA);
