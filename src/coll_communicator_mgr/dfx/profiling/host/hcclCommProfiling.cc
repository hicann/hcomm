    ...
    uint64_t beginTime, const std::string &commTag, const std::string &kernelName, uint32_t threadId, bool cachedReq)
{
    ...
    EXCEPTION_CATCH(
        ...
        return HCCL_E_PTR);
    EXCEPTION_CATCH(Hccl::ProfilingHandler::GetInstance().ReportNodeBasicInfo(endTime, cmdItemId, threadId, cachedReq),
        return HCCL_E_PTR);
    HCCL_INFO(
...
{
    profilingReporter_->CallReportMc2CommInfo(mc2CommInfo.FreeStreamId, mc2CommInfo.streamsId, mc2CommInfo.groupname,
        mc2CommInfo.myRankId, mc2CommInfo.rankSize, mc2CommInfo.parentRankId);
}
