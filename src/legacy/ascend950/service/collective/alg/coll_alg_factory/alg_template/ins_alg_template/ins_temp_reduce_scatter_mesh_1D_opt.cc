...
namespace Hccl {
InsTempReduceScatterMesh1DOpt::InsTempReduceScatterMesh1DOpt(const RankId virtualRank, const u32 tempRankSize,
...
u64 InsTempReduceScatterMesh1DOpt::CalcScratchMultiple(const BufferType &inBuffType, const BufferType &outBuffType) const
{
    (void)inBuffType;
    (void)outBuffType;
...
HcclResult InsTempReduceScatterMesh1DOpt::GenExtIns(const TempFuncs &tempFuncs, const TemplateDataParams &tempAlgParams,
...
HcclResult InsTempReduceScatterMesh1DOpt::PostCopy(const TemplateDataParams &tempAlgParams, std::vector<InsQuePtr> &tempInsQues)
{
...
HcclResult InsTempReduceScatterMesh1DOpt::RunReduceScatter(const ResLinks &tempLinks, std::vector<InsQuePtr> &tempInsQues, 
                                                        const TemplateDataParams &tempAlgParams)
{
    ...
    for (u32 queIdx = 1; queIdx < queNum_; queIdx++) {
        ...
        HCCL_DEBUG("[InsTempReduceScatterMesh1DOpt][RunReduceScatter] myRank[%d], toRank[%d], fromRank[%d]",
        ...
        u64 recvSlice = ((myAlgRank == tempRankSize_ - 1) && (tempAlgParams.tailSize != 0)) ? tempAlgParams.tailSize : tempAlgParams.sliceSize;
        ...
        for (u32 repeatIdx = 0; repeatIdx < tempAlgParams.repeatNum; repeatIdx++) {
            DataSlice rxSrcSlice = DataSlice(tempAlgParams.buffInfo.inBuffType, tempAlgParams.buffInfo.inBuffBaseOff + 
                repeatIdx * tempAlgParams.inputRepeatStride + myAlgRank * tempAlgParams.inputSliceStride, recvSlice); // 接收源
            DataSlice rxDstSlice = DataSlice(tempAlgParams.buffInfo.scratBuffType, tempAlgParams.buffInfo.scratchBuffBaseOff + 
                repeatIdx * tempAlgParams.outputRepeatStride + nextRank * recvSlice, recvSlice); // 接收目标
