...
public:
    explicit CcuCtxArgAllGatherNHR1D(
...
public:
    explicit CcuTaskArgAllGatherNHR1D(
...
public:
    ...
    void Init(
        uint32_t rankId, uint64_t inputAddr, uint64_t outputAddr, uint64_t axisId, uint32_t axisSize, uint64_t die0Size,
        uint64_t die1Size, uint64_t repeatNum, uint64_t inputSliceStride, uint64_t outputSliceStride,
        uint64_t inputRepeatStride, uint64_t outputRepeatStride, std::vector<NHRStepInfo> stepInfoVector,
    ...
    {
        HCCL_INFO("CcuInstructionAllGatherNHR1D instype is CCU_ALLGATHER_NHR_1D_MEM2MEM.");
        return instType_;
    }
    ...
    {
        return StringFormat(
            "CcuInstructionAllGatherNHR1D rankId [%u], instType[%s]", rankId_, instType_.Describe().c_str());
    }

    std::unique_ptr<CcuCtxArg> GetCtxArg() const override
    {
        return std::make_unique<CcuCtxArgAllGatherNHR1D>(
    ...
    {
        return std::make_unique<CcuTaskArgAllGatherNHR1D>(
            inputAddr_, outputAddr_, token_, die0Size_, die1Size_, repeatNum_, inputSliceStride_, outputSliceStride_,
