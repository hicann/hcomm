...
void TopoInfo::Deserialize(const nlohmann::json &topoInfoJson)
{
    ...
    HCCL_DEBUG("[TopoInfo::%s] deserialize summary: peers[%zu], edges[%zu], PEER2PEER[%u], PEER2NET[%u]",
        __func__, peers.size(), edges.size(), peer2PeerEdgeCount, peer2NetEdgeCount);
}
...
void TopoInfo::VerifyEdges(EdgeInfo &edge)
{
    ...
    if (find(edges.begin(), edges.end(), edge) != edges.end()) {
        HCCL_DEBUG("[TopoInfo::%s] ignore duplicate physical edge, linkType[%s], topoType[%s], "
                   "topoInstanceId[%u], localA[%u], localB[%u].",
            __func__, edge.linkType.Describe().c_str(), edge.topoType.Describe().c_str(), edge.topoInstId,
            edge.localA, edge.localB);
        return;
    }
...
TopoInfo::TopoInfo(BinaryStream &binaryStream)
{
    ...
    HCCL_INFO("[TopoInfo] version is [%s], peerCount is [%u], edgeCount is [%u], peers size is [%zu], "
...
void TopoInfo::GetBinStream(BinaryStream &binaryStream) const
{
    ...
    binaryStream << edgeGroupCount;
    HCCL_INFO("[TopoInfo::GetBinStream] version is [%s], peerCount is [%u], edgeCount is [%u], peers size is [%zu], "
              "edges size is [%zu]",
