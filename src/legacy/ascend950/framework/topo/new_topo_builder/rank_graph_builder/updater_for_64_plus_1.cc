...
bool HasLayer0Peer2PeerLink(const shared_ptr<Graph<PhyTopo::Node, PhyTopo::Link>> &phyTopoGraph,
                            const shared_ptr<NetInstance::Peer> &peer, LocalId replacedLocalId)
{
    if (phyTopoGraph == nullptr || peer == nullptr) {
        return false;
    }
    bool matched = false;
    phyTopoGraph->TraverseEdge(PhyTopo::Peer::GetId(peer->GetLocalId()), PhyTopo::Peer::GetId(replacedLocalId),
                                            ...
                                            NetInstance* netInstance, const RankTableInfo* rankTable) const
{
    ...
    if (backupPeer == nullptr || peer == nullptr ||
        !backupPeer->TryGetLayer0Address(backD2PlanePort, backD2PlaneAddr) ||
...
std::shared_ptr<PhyTopo::Link> UpdaterFor64Plus1::GetPeer2PlaneEdges(u32 backupPlaneId,
    shared_ptr<NetInstance::Peer> peer, std::shared_ptr<Graph<PhyTopo::Node, PhyTopo::Link>> phyTopoGraph,
    u32 expectedPortNum, const std::set<LinkProtocol> &expectedProtocols) const
{
    ...
    for (const auto &edge : peer2AllPlaneEdges) {
        ...
        if (edge->GetTopoInstId() == backupPlaneId &&
            HasLayer0Port(peer, edge->GetSourceIFace()->GetPorts())) {
            if (peer2PlaneEdges != nullptr) {
                THROW<InvalidParamsException>("[UpdaterFor64Plus1][%s] BackupPlane[%u] is ambiguous", __func__,
                    backupPlaneId);
            }
