...
const std::map<OpType, std::string> PROF_OP_NAME_V2 = {{OpType::INVALID, "hcom_invalid_"},
    {OpType::ALLREDUCE, "hcom_allReduce_"}, {OpType::BROADCAST, "hcom_broadcast_"}, {OpType::REDUCE, "hcom_reduce_"},
    {OpType::SEND, "hcom_send_"}, {OpType::RECV, "hcom_receive_"}, {OpType::ALLGATHER, "hcom_allGather_"},
    ...
    {OpType::BATCHPUT, "hccl_batchPut_"}, {OpType::BATCHGET, "hccl_batchGet_"}, {OpType::DEBUGCASE, "hccl_debugCase_"},
    {OpType::BARRIER, "hccl_barrier_"}, {OpType::HALFALLTOALLV, "hccl_halfAlltoallv_"}};
...
public:
    ...
    void ReportHcclTaskApi(TaskParamType taskType, uint64_t beginTime, uint64_t endTime, bool isMasterStream,
        bool cachedReq, bool ignoreLevel = false);
    ...
    inline bool GetOpBaseFlag() const
    {
        return isOpbase_;
    }
    inline bool GetCachedFlag() const
    {
        return isCached_;
    }
    ...
    {
        return cachedAlgTypeHashId_;
    }

private:
    explicit ProfilingHandler();

    void ReportAclApi(uint32_t cmdType, uint64_t beginTime, uint64_t endTime, uint64_t cmdItemId, uint32_t threadId,
        bool cachedReq);
    ...
    void InitLog() const;
    void ReportHcclMC2CommInfoLog(const Stream &stream, const std::vector<Stream *> &aicpuStreams) const;
    ...
    void LogCcuTaskInfo(const CcuProfilingInfo &info, const TaskInfo &taskInfo, uint64_t itemId, uint64_t groupName,
        u32 rankId, u32 ranksize) const;
    void LogCcuWaitSignalInfo(const CcuProfilingInfo &info, const TaskInfo &taskInfo, uint64_t itemId,
        uint64_t groupName, u32 rankId, u32 ranksize) const;
    void LogCcuGroupInfo(const CcuProfilingInfo &info, const TaskInfo &taskInfo, uint64_t itemId, uint64_t groupName,
        u32 rankId, u32 ranksize) const;
