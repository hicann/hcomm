...
/// 数值一致； 本头文件不 include env_config，避免经 base_config_legacy.h 拉入 dma_mode.h 导致 platform
/// 等目标缺头编译失败。
...
enum class DeviceIdType { DEVICE_ID_TYPE_PHY_ID = 0, DEVICE_ID_TYPE_SDID };
...
JfcHandle HrtRaUbCreateJfc(RdmaHandle handle, CqCreateInfo& cqInfo, HrtUbJfcMode mode, u32 cqDepth = 0);
...
MAKE_ENUM(TpProtocol, CTP, TP, UBOE, UB_RTP);
