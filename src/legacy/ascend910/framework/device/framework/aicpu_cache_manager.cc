...
HcclResult AicpuCacheManager::LookupOpUnfoldCache(const std::string &algName, const OpParam &param,
    const AlgResourceResponse &algResource, bool &needExecute, bool &isCacheMiss, Stream &mainStream,
    std::vector<Stream> &slaveStreams, void *dispatcherPtr, const bool isDeviceMode, const HcclTopoInfo &topoinfo,
    ...
    const DeviceMem &tinySendRecvMem, std::function<HcclResult()> setProfStartCallback)
{
    ...
    for (size_t i = 0; i < slaveStreams.size(); ++i) {
        ...
        HCCL_INFO("[AicpuCacheManager][LookupOpUnfoldCache] %uth slaveStream with streamId[%u] and id[%u]", i,
    ...
    if (needCache) {
        ...
        HCCL_INFO("[AicpuCacheManager][LookupOpUnfoldCache] prepare key[%s] for op-unfold cache",
        ...
        CHK_RET(PrepareUserMemRanges(param, algResource, userInputMemRanges, userOutputMemRanges, topoinfo,
            zeroCopyExchangerPtr, workflowMode, tinySendRecvMem));

        // 查找算子展开的动态缓存
        HCCL_INFO("[AicpuCacheManager][LookupOpUnfoldCache] look up op-unfold cache for key %s",
            opUnfoldKey.GetKeyString().c_str());
        ...
        if (entryPtr != nullptr) { // Cache hit
            ...
            if (IsAlltoallvType(param.opType)) { // alltoallv类算子, 需要额外的offset信息
                ...
                CHK_RET((reinterpret_cast<DispatcherAiCpu *>(dispatcherPtr))
                            ->LaunchNewTask(entryPtr, userInputMemRanges, userOutputMemRanges, mainStream, slaveStreams,
            ...
            // 注意: AicpuCacheManager下dispatcher一定是DispatcherAicpu
            CHK_RET((reinterpret_cast<DispatcherAiCpu *>(dispatcherPtr))
                        ->SetLaunchContext(opUnfoldKey, opUnfoldCachePtr_, userInputMemRanges, userOutputMemRanges,
                            isAlltoallv, alltoallvMetadataPtr));
    ...
    const AlgOpContext &algContext, const HcclWorkflowMode workflowMode)
{
    ...
    if (IsAlltoallvType(param.opType)) {
        ...
        if (!isInitAlltoallvMetadata_) {
            ...
            // 所以使用的一定是CollRunAlltoAllDirectFullmesh executor
            CHK_PTR_NULL(executor.get());
            HCCL_INFO("[AicpuCacheManager][PostProcessForCacheMiss] get hcclOffset-dstRank mapping of key[%s] for "
    ...
    bool &needCache)
{
    ...
    if (param.aicpuCacheEnable == 0) {
        HCCL_INFO("[AicpuCacheManager][NeedOpUnfoldCache] disable aicpu cache for aicpuCacheEnable[%u]",
    ...
    for (std::unordered_map<u32, bool>::const_iterator map_iter = isUsedRdmaMap.cbegin();
        ...
        if (map_iter->second) {
            HCCL_INFO("[AicpuCacheManager][NeedOpUnfoldCache] rank[%u] uses RDMA -> not supported for operator "
                      "unfolding cache",
                map_iter->first);
            return HCCL_SUCCESS;
        }
    ...
    if (opType == HcclCMDType::HCCL_CMD_BROADCAST || opType == HcclCMDType::HCCL_CMD_REDUCE
        ...
        if (algName != "RunAlltoAllDirectFullmesh") {
            HCCL_INFO("[AicpuCacheManager][NeedOpUnfoldCache] algName[%s] is not supported for alltoallv's cache",
                algName.c_str());
            return HCCL_SUCCESS;
        }
        ...
        if (!isSmallData) {
            HCCL_INFO("[AicpuCacheManager][NeedOpUnfoldCache] large-data alltoallv[%u] is not supported for operator "
                      "unfolding cache",
                opType);
            return HCCL_SUCCESS;
        }
...
HcclResult AicpuCacheManager::HasRemoteDataAlltoallv(
    const OpParam &param, bool &hasRemoteData, const HcclTopoInfo &topoinfo)
{
...
HcclResult AicpuCacheManager::IsInplace(const OpParam &param, bool &isInplace, const HcclTopoInfo &topoinfo)
{
    ...
    if (inputSize == 0 || outputSize == 0) {
        isInplace = false;
        HCCL_INFO("[AicpuCacheManager][IsInplace] inputSize[%u] is not overlapping with outputSize[%u]", inputSize,
            outputSize);
        return HCCL_SUCCESS;
    }
    ...
    if (inputStart <= outputEnd && outputStart <= inputEnd) {
        isInplace = true;
        HCCL_INFO("[AicpuCacheManager][IsInplace] input[0x%016llx, 0x%016llx] is overlapping with output[0x%016llx, "
                  "0x%016llx]",
            inputStart, inputEnd, outputStart, outputEnd);
    } else {
        isInplace = false;
        HCCL_INFO("[AicpuCacheManager][IsInplace] input[0x%016llx, 0x%016llx] is not overlapping with "
...
HcclResult AicpuCacheManager::IsSmallDataAlltoallv(
    const OpParam &param, bool &isSmallData, const HcclTopoInfo &topoinfo)
{
    ...
    const AlgOpContext &algContext)
{
    ...
    CHK_PRT_RET(links.size() != rankSize,
        HCCL_ERROR("[AicpuCacheManager][CalcMetadataForFirstAlltoallv] links.size[%u] != rankSize[%u]", links.size(),
            rankSize),
        HCCL_E_INTERNAL);
    for (uint32_t tmpRank = 0; tmpRank < rankSize; ++tmpRank) {
        if (tmpRank == curRank) {
...
HcclResult AicpuCacheManager::IsBigCountForAlltoallv(
    const OpParam &param, const HcclTopoInfo &topoinfo, bool &isBigCount)
{
    isBigCount = false;
    ...
    const HcclWorkflowMode workflowMode, const DeviceMem &tinySendRecvMem)
{
    ...
    // 参考aicpu_communicator.cc中的SetAlltoAllInputAndOutPutMem
    const HcclCMDType opType = param.opType;
    CHK_PRT_RET((inputSize == 0 || outputSize == 0) && opType != HCCL_CMD_ALLTOALL && opType != HCCL_CMD_ALLTOALLV
                    && opType != HCCL_CMD_ALLTOALLVC,
        HCCL_ERROR("[AicpuCacheManager][PrepareUserMemRanges] opType[%u] inputSize[%llu] outputSize[%llu]", opType,
    ...
    if (param.isZeroCopy) {
        ...
        for (size_t planeIdx = 0; planeIdx < algResource.opTransportResponse.size(); ++planeIdx) {
            ...
            for (size_t commIdx = 0; commIdx < subCommTransport.size(); ++commIdx) {
                ...
                HCCL_INFO("[AicpuCacheManager][PrepareUserMemRanges] planeIdx[%u] commIdx[%u] links.size[%u]", planeIdx,
                    commIdx, links.size());
                ...
                for (size_t reqIdx = 0; reqIdx < transportRequests.size(); ++reqIdx) {
                    ...
                    if (curReq.isValid) {
                        if (curReq.remoteUserRank == curRank) { // 本rank无需从link获取user memory range
                            continue;
                        } else if (curReq.remoteUserRank
                                   == INVALID_VALUE_RANKID) { // 本rank无需从link获取user memory range
                            continue;
                        }
    ...
    if (UNLIKELY(HcclCheckLogLevel(HCCL_LOG_INFO))) {
        for (size_t rankId = 0; rankId < userInputMemRanges.size(); ++rankId) {
            ...
            HCCL_INFO("[AicpuCacheManager][PrepareUserMemRanges] userInputMemRanges[%u] isValid: %d, baseAddr: "
                      "0x%016llx, memSize: %llu, endAddr: 0x%016llx",
            ...
            HCCL_INFO("[AicpuCacheManager][PrepareUserMemRanges] userOutputMemRanges[%u] isValid: %d, baseAddr: "
                      "0x%016llx, memSize: %llu, endAddr: 0x%016llx",
    ...
    uint64_t &inputSize, uint64_t &outputSize, const HcclTopoInfo &topoinfo)
{
    ...
    if (opType == HcclCMDType::HCCL_CMD_ALLTOALL) { // alltoall算子
        ...
        if (opType == HcclCMDType::HCCL_CMD_ALLTOALLV) { // alltoallv算子
            ...
            const uint32_t curRank = topoinfo.userRank;
            HCCL_INFO("[AicpuCacheManager][ParseOpParamForCache] sum %u-size sendCountMatrix for input/output size",
                rankSize);
            for (uint32_t tmpRank = 0; tmpRank < rankSize; ++tmpRank) {
                // curRank发送到tmpRank的数据量
                const uint64_t curSendCounts = *(static_cast<const u64 *>(param.All2AllDataDes.sendCountMatrix)
    ...
    HCCL_DEBUG("[AicpuCacheManager][ParseOpParamForCache] opType[%u] rankSize[%u] sendType[%u] recvType[%u] "
