...
ZeroCopyAddressMgr::AddLocalIpc2RemoteAddr(u32 devicePhyId, void* localIpcBase, void* remoteAddrBase, u64 length)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ZeroCopyAddressMgr][AddLocalIpc2RemoteAddr] dev[%u] remote addr %p had set", devicePhyId, remoteAddrBase),
        HCCL_E_PARA);
...
HcclResult ZeroCopyAddressMgr::DelLocalIpc2RemoteAddr(u32 devicePhyId, void* remoteAddrBase)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR("[ZeroCopyAddressMgr][DelLocalIpc2RemoteAddr] dev[%u] addr %p not set", devicePhyId, remoteAddrBase),
        HCCL_E_PARA);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ZeroCopyAddressMgr][DelLocalIpc2RemoteAddr] dev[%u] remoteAddr %p localAddr 0x%lx still have activate "
            "memory [0x%lx, 0x%lx)",
...
HcclResult ZeroCopyAddressMgr::ActivateCommMemoryAddr(void* startPtr, u64 length)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ZeroCopyAddressMgr][ActivateCommMemoryAddr] overlap address exist:[0x%lx, 0x%lx) valid:[0x%lx, 0x%lx)",
...
HcclResult ZeroCopyAddressMgr::DeactivateCommMemoryAddr(void* startPtr)
{
    ...
    HCCL_INFO(
        "[ZeroCopyAddressMgr][DeactivateCommMemoryAddr] deactivate address [0x%lx, 0x%lx) success", it->start, it->end);
