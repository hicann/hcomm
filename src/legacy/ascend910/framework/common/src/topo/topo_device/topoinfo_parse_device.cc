...
// nicIdx不只是校验，还有修改
HcclResult TopoInfoParse::ParseAndCheck(std::vector<u32>& nicIdx) { return HCCL_E_NOT_SUPPORT; }
...
HcclResult TopoInfoParse::CheckAndAssignNicInfo(std::vector<u32>& nicIdx) { return HCCL_E_NOT_SUPPORT; }

// nicIdx做填充，nicIdx也是deviceId，deviceId做过的校验这里不再重复
HcclResult TopoInfoParse::CheckRankTableNicInfo(std::vector<u32>& nicIdx) { return HCCL_E_NOT_SUPPORT; }

// 校验server内4p场景下deivce选取是否合法，2p与标卡场景重合
HcclResult TopoInfoParse::CheckServerInnerRankInfo() { return HCCL_E_NOT_SUPPORT; }

HcclResult TopoInfoParse::IsAllRankSamePlane(bool& isAllRankSamePlane) { return HCCL_E_NOT_SUPPORT; }

HcclResult TopoInfoParse::IsSingleMeshAggregation(bool& isSingleMeshAggregation) { return HCCL_E_NOT_SUPPORT; }

HcclResult TopoInfoParse::IsAllRankConnectedWithHCCS(bool& isAllRankConnectedWithHCCS) { return HCCL_E_NOT_SUPPORT; }
