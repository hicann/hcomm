...
HcclResult CheckAverageDev(u32 uDeviceNum, u32 uServerNum)
{
    ...
    if ((uDeviceNum % uServerNum)
        ...
        HCCL_ERROR(
            "[Check][AverageDev]errNo[0x%016llx] device / server is invalid, uDeviceNum[%u], uServerNum[%u]",
...
HcclResult TopoInfoRanktableParser::ReadFile(const std::string& readFile)
{
    ...
    if (!infile) {
        ...
        try {
            ...
            HCCL_ERROR(
                "[%s][%s]errNo[0x%016llx] load file[%s] to json fail. please check json file!",
...
HcclResult TopoInfoRanktableParser::LoadFile(const std::string& file)
{
    if (file.empty()) {
        HCCL_ERROR(
            "[%s][%s]errNo[0x%016llx] json file length is zero", LOG_KEYWORDS_INIT_GROUP.c_str(),
    ...
    if (!CheckFilePath(file, fileType)) {
        HCCL_ERROR(
            "[%s][%s]errNo[0x%016llx] path %s is not a valid %s file", LOG_KEYWORDS_INIT_GROUP.c_str(),
    ...
    do {
        if ((std::chrono::steady_clock::now() - startTime) >= TIMEOUT) {
            HCCL_ERROR(
                "[%s][%s]errNo[0x%016llx] Load ranktable file[%s] timeout[%lld]s", LOG_KEYWORDS_INIT_GROUP.c_str(),
                LOG_KEYWORDS_RANKTABLE_CONFIG.c_str(), HCOM_ERROR_CODE(HCCL_E_TIMEOUT), strFilePath.c_str(), TIMEOUT);
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[%s][%s]read file[%s] error", LOG_KEYWORDS_INIT_GROUP.c_str(), LOG_KEYWORDS_RANKTABLE_CONFIG.c_str(),
                strFilePath.c_str()),
            HCCL_E_PARA);
...
HcclResult TopoInfoRanktableParser::LoadString(const std::string& string)
{
    ...
    if (string.empty()) {
        ...
        HCCL_ERROR(
            "[%s][%s]errNo[0x%016llx] json string length is zero", LOG_KEYWORDS_INIT_GROUP.c_str(),
            LOG_KEYWORDS_RANKTABLE_CONFIG.c_str(), HCOM_ERROR_CODE(HCCL_E_PARA));
        return HCCL_E_PARA;
    }
    ...
    RPT_INPUT_ERR(
        ret != HCCL_SUCCESS, "EI0004", std::vector<std::string>({"ranktable_path", "error_reason"}),
        std::vector<std::string>({string, "failed to parse rank table JSON content, the JSON format is incorrect"}));
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[%s][%s]Failed to read the string %s into JSON.", LOG_KEYWORDS_INIT_GROUP.c_str(),
            LOG_KEYWORDS_RANKTABLE_CONFIG.c_str(), string.c_str()),
        ret);
    return HCCL_SUCCESS;
}
...
HcclResult TopoInfoRanktableParser::Init()
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR("[Init][TopoInfoRanktableParser]load string[%s] error", fileName_.c_str()),
        ret);
...
HcclResult TopoInfoRanktableParser::LoadFileInit(std::string& rankTableM)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[%s][%s]load file[%s] error", LOG_KEYWORDS_INIT_GROUP.c_str(), LOG_KEYWORDS_RANKTABLE_CONFIG.c_str(),
            rankTableFile_.c_str()),
        ret);
    ...
    const nlohmann::json& obj, const char* propName, std::string& propValue, bool optionalProp) const
{
    ...
    if (obj.find(propName) == obj.end()) {
        if (optionalProp) {
            ...
            RPT_INPUT_ERR(
                true, "EI0017", std::vector<std::string>({"config"}),
                std::vector<std::string>({std::string(propName)}));
            HCCL_ERROR(
                "[%s][%s]json object has no property called %s", LOG_KEYWORDS_INIT_GROUP.c_str(),
    ...
    if (obj[propName].is_string()) {
        ...
        HCCL_ERROR(
            "[Get][JsonProperty]errNo[0x%016llx] json object property value of Name[%s] is not string!",
    ...
    const nlohmann::json& obj, const char* propName, nlohmann::json& propValue, bool optionalProp) const
{
    ...
    propValue = obj[propName];
    CHK_PRT_RET(
        propValue.size() == 0, HCCL_ERROR("[Get][JsonProperty]get property[%s] size is zero", propName), HCCL_E_PARA);
    return HCCL_SUCCESS;
}
    ...
    const nlohmann::json& obj, const u32 index, const char* propName, std::string& propValue, bool optionalProp) const
{
    if (!obj.is_array() || index >= obj.size()) {
        HCCL_ERROR(
            "[Get][JsonArrayMemberProperty]errNo[0x%016llx] index[%u] is out of json object range",
    ...
    bool optionalProp) const
{
    ...
    CHK_PRT_RET(
        propValue.size() == 0,
        HCCL_ERROR(
            "[Get][JsonArrayMemberProperty]get index[%u] property[%s] size is "
            "zero",
            index, propName),
        HCCL_E_PARA);
    return HCCL_SUCCESS;
    ...
    const JsonUniqueInfoType& type, const std::string& value, string& strType) const
{
    ...
    CHK_PRT_RET(
        it == JsonInfoTypeNameMap.end(),
        HCCL_ERROR(
            "[Get][JsonArrayMemberProperty][Check][UniquePara]errNo[0x%016llx] jsonUniqueInfoType[%d] "
            "is not in JsonInfoTypeNameMap",
    ...
    switch (type) {
        ...
        case JsonUniqueInfoType::UNIQUE_INFO_TYPE_ETH_IP: {
            ...
            if (ip.IsInvalid()) {
                ...
                HCCL_ERROR(
                    "[%s][%s]errNo[0x%016llx] ip[%s] is not a valid ip address", LOG_KEYWORDS_INIT_GROUP.c_str(),
        ...
        case JsonUniqueInfoType::UNIQUE_INFO_TYPE_POD_NAME: {
            if (value.length() > GROUP_NAME_MAX_LEN) {
                HCCL_ERROR(
                    "[Check][UniquePara]errNo[0x%016llx] Name[%s] length[%lu] is too long",
        ...
        default: {
            HCCL_ERROR(
                "[Check][UniquePara]errNo[0x%016llx] invalid unique info type[%d]", HCOM_ERROR_CODE(HCCL_E_PARA), type);
            return HCCL_E_PARA;
        } break;
    }
    return HCCL_SUCCESS;
    ...
    const JsonUniqueInfoType& type, const std::string& value, const JsonCheckOpType& opType)
{
    ...
    if (opType == JsonCheckOpType::CHECK_OP_TYPE_INSERT) {
        ...
        if (srvIt == uniqueInfoCheckPool_[static_cast<u32>(type)].end()) {
            HCCL_ERROR(
                "[Check][UniqueAndInsertPool]errNo[0x%016llx] [%s]:[%s] is not exist", HCOM_ERROR_CODE(HCCL_E_PARA),
                strUniqueInfoType.c_str(), value.c_str());
            return HCCL_E_NOT_FOUND;
        }
    } else {
        HCCL_ERROR(
            "[Check][UniqueAndInsertPool]errNo[0x%016llx] invalid json check op Type[%d]", HCOM_ERROR_CODE(HCCL_E_PARA),
            opType);
        return HCCL_E_PARA;
    }
    return HCCL_SUCCESS;
...
HcclResult TopoInfoRanktableParser::ConvertIpAddress(const std::string& ipStr, HcclIpAddress& ipAddr)
{
    ...
    RPT_INPUT_ERR(
        ret != HCCL_SUCCESS, "EI0014", std::vector<std::string>({"value", "variable", "expect"}),
        std::vector<std::string>({ipStr, "IP", "a valid IP address"}));
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[%s][%s]ipStr[%s] convert failed", LOG_KEYWORDS_INIT_GROUP.c_str(), LOG_KEYWORDS_RANKTABLE_CHECK.c_str(),
            ipStr.c_str()),
        ret);
    return ret;
}
