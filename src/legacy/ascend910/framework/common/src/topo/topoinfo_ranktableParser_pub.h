...
enum class ServerHostNicIndex { HOST_NIC_IP_INDEX = 0, HOST_NIC_PORT_INDEX = 1, HOST_NIC_INDEX_NUM };

enum class JsonCheckOpType { CHECK_OP_TYPE_FIND = 0, CHECK_OP_TYPE_INSERT = 1, CHECK_OP_TYPE_NUM };
...
public:
    ...
    virtual HcclResult GetClusterInfo(hccl::HcclCommParams& params, hccl::RankTable_t& rankTable);
...
protected:
    ...
    HcclResult GetJsonProperty(
        const nlohmann::json& obj, const char* propName, std::string& propValue, bool optionalProp = true) const;
    HcclResult GetJsonProperty(
        const nlohmann::json& obj, const char* propName, nlohmann::json& propValue, bool optionalProp = true) const;
    // json数组中索引为index的一个名为prop_name的属性
    // 数返回这个属性的值prop_value，属性值重载了string和json类型
    HcclResult GetJsonArrayMemberProperty(
        const nlohmann::json& obj, const u32 index, const char* propName, std::string& propValue,
    ...
    HcclResult
    CheckUniqueAndInsertPool(const JsonUniqueInfoType& type, const std::string& value, const JsonCheckOpType& opType);
