    ...
    [[maybe_unused]] const AicpuOpTiling opTilingInfo, u64 opTilingDataSize, bool isCustom)
{
    ...
    if (opParam.isCapture) { // 非主流下发时，acl graph场景，capture从流
        ...
        CHK_PRT_RET(
            ret != ACL_SUCCESS, HCCL_ERROR("[%s]aclrtRecordEvent failed, ret[%d]", __func__, ret), HCCL_E_RUNTIME);
        ...
        HCCL_INFO(
            "[HcclCommunicator][%s]tag[%s], capture kfcOpStream, mainStream[%d] recordEvent, kfcOpStream[%d] "
            "waitEvent.",
    ...
    if (opParam.isCapture) {
        ...
        HCCL_INFO(
            "[HcclCommunicator][%s]tag[%s], capture kfcOpStream, mainStream[%d] waitEvent, kfcOpStream[%d] "
            "recordEvent.",
            __func__, opParam.tag.c_str(), opParam.stream.id(), kfcOpStream.id());
    }
