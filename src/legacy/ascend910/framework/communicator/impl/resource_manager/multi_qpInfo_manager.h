...
struct MulQpInfoCacheBase {
    ...
    // 获取ip->ip port个数
    virtual HcclResult
    GetSpecialSourcePortsByIpPair(MulQpSourcePorts& sourcePorts, const KeyPair& ipPair = KeyPair()) const;
...
struct DevCfgMulQpInfoCache : MulQpInfoCacheBase {
    ...
    // 获取ip->ip port个数
    HcclResult
    GetSpecialSourcePortsByIpPair(MulQpSourcePorts& sourcePorts, const KeyPair& ipPair = KeyPair()) const override;
