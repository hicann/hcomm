...
namespace hccl {
GatherMesh::GatherMesh(const HcclDispatcher dispatcher) : AlgTemplateBase(dispatcher), round_(0) {}
...
// 非root rank发送数据
HcclResult GatherMesh::RunSendGather(const u32 dstRank, const Slice& slice, const std::vector<LINK>& links)
{
    ...
    HCCL_DEBUG(
        "root rank[%u] tx input[%p] offset[%llu] to srcrank size[%llu] ", dstRank, src.ptr(), slice.offset, slice.size);
...
// Gather的入口函数
HcclResult
GatherMesh::RunAsync(const u32 rank, const u32 rankSize, const std::vector<std::shared_ptr<Transport>>& links)
{
    CHK_SMART_PTR_NULL(dispatcher_);
    CHK_PTR_NULL(stream_.ptr());
    HCCL_INFO(
        "GatherMesh run: rank[%u] rankSize[%u] inputMem[%p] to outputMem[%p] count[%llu]", rank, rankSize,
    ...
    if (rankSize == 1) {
        if (inputMem_ != outputMem_) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[GatherMesh][RunAsync]rank[%u] copy input[%p] to output[%p] "
                    "failed",
    ...
    if (links.size() < rankSize) {
        HCCL_ERROR(
            "[GatherMesh][RunAsync]rank[%u] linksize[%llu] is less than rankSize[%u]", rank, links.size(), rankSize);
        return HCCL_E_INTERNAL;
    }
    ...
    if (rank == root_) {
        ...
        for (u32 srcRank = 0; srcRank < rankSize; srcRank++) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[GatherMesh][RunAsync]srcrank[%u] send gather to "
...
HcclResult GatherMesh::AddMainSteamSubStreamSyncPost(u32 rank, u32 rankSize)
{
    for (u32 streamIndex = 0; streamIndex < rankSize - 2; streamIndex++) { // rankSize - 2 stream num
        ...
        HCCL_DEBUG("rank[%u] streamindex[%u] record signal[%p]", rank, streamIndex, (*meshSignal_)[streamIndex]->ptr());
