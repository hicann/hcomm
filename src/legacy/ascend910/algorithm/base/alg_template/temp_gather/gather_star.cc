...
namespace hccl {
// Gather的入口函数
GatherStar::GatherStar(const HcclDispatcher dispatcher) : AlgTemplateBase(dispatcher) {}
...
HcclResult
GatherStar::RunAsync(const u32 rank, const u32 rankSize, const std::vector<std::shared_ptr<Transport>>& links)
{
    ...
    if (rankSize == 1) {
        if (inputMem_ != outputMem_) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[GatherStar][RunAsync]rank[%u] copy input[%p] to output[%p] "
                    "failed",
    ...
    if (links.size() < rankSize) {
        HCCL_ERROR(
            "[GatherStar][RunAsync]rank[%u] linksize[%llu] is less than rankSize[%u]", rank, links.size(), rankSize);
    ...
    if (rank == root_) {
        ...
        for (u32 srcRank = 0; srcRank < rankSize; srcRank++) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[GatherStar][RunAsync] srcrank[%u] recv broadcast from"
                    "root rank[%u] run failed!",
                    srcRank, root_),
                ret);
        }
    } else {
...
HcclResult GatherStar::RunRecvGather(const u32 srcRank, const Slice& slice, const std::vector<LINK>& links)
{
    ...
    if (slice.size > 0 && srcRank != root_) {
        if (srcRank >= links.size()) {
            HCCL_ERROR(
                "[Run][GatherStar][RecvGather]SrcRank[%u] is out of range, linkSize[%llu]", srcRank, links.size());
            return HCCL_E_INTERNAL;
        }

        dst = outputMem_.range(slice.size * srcRank, slice.size);
        HCCL_DEBUG(
            "rank[%u] will rcv with output's offset[%llu], size[%llu] dstmem[%p]", root_, slice.offset, slice.size,
            dst.ptr());
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Run][GatherStar][RecvGather]rank[%u] rx async with output's offset[%llu] failed", root_,
                slice.offset),
            ret);
        ...
        if (!links[srcRank]->IsTransportRoce()) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR("[Run][GatherStar][SendGather]srcRank[%u] gather mesh run executor barrier failed", srcRank),
                ret);

            ret = links[srcRank]->RxWaitDone(stream_);
            CHK_PRT_RET(
                ret != HCCL_SUCCESS, HCCL_ERROR("[Run][GatherStar][GatherStar][SendGather]RxWaitDone failed"), ret);
        }
    } else if (srcRank == root_) {
        CHK_RET(HcclMemcpyAsync(
            dispatcher_, static_cast<u8*>(outputMem_.ptr()) + slice.offset, outputMem_.size() - slice.offset,
            inputMem_.ptr(), slice.size, HcclRtMemcpyKind::HCCL_RT_MEMCPY_KIND_DEVICE_TO_DEVICE, stream_,
...
HcclResult GatherStar::RunSendGather(const u32 dstRank, const Slice& slice, const std::vector<LINK>& links)
{
    // 非root发送数据
    if (slice.size > 0) {
        HCCL_DEBUG(
            "root rank[%u] tx input[%p] offset[%llu] to srcrank size[%llu] ", dstRank, inputMem_.ptr(), slice.offset,
            slice.size);
        if (dstRank >= links.size()) {
            HCCL_ERROR(
                "[Run][GatherStar][SendGather]DstRank[%u] is out of range, link Size[%llu]", dstRank, links.size());
            return HCCL_E_INTERNAL;
        }
        ...
        HcclResult ret
            = links[dstRank]->TxAsync(UserMemType::OUTPUT_MEM, slice.offset, inputMem_.ptr(), slice.size, stream_);
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Run][GatherStar][SendGather]srcrank[%u] tx async to root"
                "rank[%u] run failed",
                dstRank, root_),
            ret);
        ...
        if (!links[dstRank]->IsTransportRoce()) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[Run][GatherStar][SendGather]dstRank[%u] gather mesh run"
                    "executor barrier failed",
                    dstRank),
                ret);
