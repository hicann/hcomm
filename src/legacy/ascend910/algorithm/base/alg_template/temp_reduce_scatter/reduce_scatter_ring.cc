...
namespace hccl {
ReduceScatterRing::ReduceScatterRing(const HcclDispatcher dispatcher) : AlgTemplateBase(dispatcher) {}
...
HcclResult ReduceScatterRing::RunVectorSourceReducer(
...
HcclResult ReduceScatterRing::RunVectorDestRducer(
    const LINK& link, const std::vector<Slice>& rxSlices, const std::vector<Slice>& rxSlicetemp)
{
    ...
    for (u32 i = 0; i < rxSlices.size(); i++) {
        ...
        HCCL_DEBUG(
            "rcv offset[%llu], size[%llu] ,then reduce with offset[%llu] size[%llu] ", rxSlicetemp[i].offset,
            rxSlicetemp[i].size, rxSlices[i].offset, rxSlices[i].size);
...
HcclResult ReduceScatterRing::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    if (links.size() < rankSize) {
        HCCL_ERROR(
            "[ReduceScatterRing][RunAsync]rank[%u] link size[%llu] is less than rank size[%u]", rank, links.size(),
            rankSize);
...
HcclResult ReduceScatterRing::RunReduceScatter(
    const u32 rank, const u32 rankSize, const std::vector<Slice>& inputSlices, const std::vector<Slice>& outputSlices)
{
    ...
    HCCL_DEBUG(
        "rank[%u] outputslice[%u] size[%llu] will tx_ack to left", rank, rxSliceIndex, outputSlices[rxSliceIndex].size);
    ...
    for (u32 i = 0; i < (rankSize - 2); i++) { // 中间rank_size - 2次传输
        // reduce目的操作
        HCCL_DEBUG(
            "rank[%u] round[%u] use sliceindex[%u] inputSlicesoffset[%llu] "
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS, HCCL_ERROR("[Run][ReduceScatter]rank[%u] round[%u] tx ack failed", rank, i), ret);
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS, HCCL_ERROR("[Run][ReduceScatter]rank[%u] round[%u] rx ack run failed", rank, i), ret);
...
HcclResult ReduceScatterRing::RunReduceScatterChunk(
    const u32 rank, const u32 rankSize, const std::vector<Slice>& inputSlices, const std::vector<Slice>& outputSlices)
{
    bool bRetSize = (inputSlices.size() < rankSize);
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[Run][ReduceScatterChunk]rank[%u] inputslice size[%llu] is less than rank "
            "size[%u]",
    ...
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[Run][ReduceScatterChunk]rank[%u] outputslice size[%llu] is less than rank "
            "size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
    ...
    if (sendSliceLen >= chunkSize) {
        ...
        for (u32 midRankIdx = 1; midRankIdx < sendSliceLen - 1; midRankIdx++) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[Run][ReduceScatterChunk]rank[%u] run mid[%u] ReduceScatter chunk failed", rank, midRankIdx),
                HCCL_E_INTERNAL);
        }
    }
    ...
    if (iterNic != nicRankList_.end()) {
        ...
        if (outputSlices[rxSliceIndex].size > 0) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[Run][ReduceScatterChunk]rank[%u] rxSlice[%u] Reducer dst run "
                    "failed",
                    rank, rxSliceIndex),
                ret);
        }
        for (u32 chunkIdx = 1; chunkIdx < chunkSize; chunkIdx++) {
            ...
            if (outputSlices[rxSliceIndex].size > 0) {
                ...
                CHK_PRT_RET(
                    ret != HCCL_SUCCESS,
                    HCCL_ERROR(
                        "[Run][ReduceScatterChunk]rank[%u] rxSlice[%u] Reducer dst "
                        "run failed",
                        rank, rxSliceIndex),
                    ret);
            }
...
HcclResult ReduceScatterRing::HeadReduceScatterChunk(
    u32 rank, u32 rankSize, const std::vector<Slice>& inputSlices, const std::vector<Slice>& outputSlices)
{
    bool bRetSize = (inputSlices.size() < rankSize);
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][HeadReduceScatterChunk]rank[%u] inputslice size[%llu] is "
    ...
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][HeadReduceScatterChunk]rank[%u] outputslice size[%llu] is "
            "less than rank size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
    ...
    if (iterSlice != preRankSlices.end()) {
        if (outputSlices[rxSliceIndex].size > 0) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[ReduceScatterRing][HeadReduceScatterChunk]rank[%u] rxSlice[%u] "
                    "Reducer dst run failed",
                    rank, rxSliceIndex),
                ret);
        }
    }
...
HcclResult ReduceScatterRing::MidReduceScatterChunk(
    u32 rank, u32 rankSize, u32 sliceIdx, const std::vector<Slice>& inputSlices, const std::vector<Slice>& outputSlices)
{
    bool bRetSize = (inputSlices.size() < rankSize);
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][MidReduceScatterChunk]rank[%u] inputslice size[%llu] is "
            "less than rank size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][MidReduceScatterChunk]rank[%u] outputslice size[%llu] is "
            "less than rank size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
    ...
    if (iterSlice != preRankSlices.end()) {
        ...
        if (outputSlices[rxSliceIndex].size > 0) {
            ...
            CHK_PRT_RET(
                ret != HCCL_SUCCESS,
                HCCL_ERROR(
                    "[ReduceScatterRing][MidReduceScatterChunk]rank[%u] rxSlice[%u] "
                    "Reducer dst run failed",
                    rank, rxSliceIndex),
                ret);
        }
...
HcclResult ReduceScatterRing::TailReduceScatterChunk(
    u32 rank, u32 rankSize, u32 sliceIdx, const std::vector<Slice>& inputSlices, const std::vector<Slice>& outputSlices)
{
    bool bRetSize = (inputSlices.size() < rankSize);
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][TailReduceScatterChunk]rank[%u] inputslice size[%llu] is "
            "less than rank size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
    ...
    CHK_PRT_RET(
        bRetSize,
        HCCL_ERROR(
            "[ReduceScatterRing][TailReduceScatterChunk]rank[%u] outputslice size[%llu] is "
            "less than rank size[%u]",
            rank, outputSlices.size(), rankSize),
        HCCL_E_INTERNAL);
...
HcclResult ReduceScatterRing::GetNslbAdjInfo(
