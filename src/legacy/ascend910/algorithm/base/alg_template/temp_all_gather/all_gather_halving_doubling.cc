...
HcclResult AllGatherHalvingDoubling::CalculateSlices(
...
HcclResult AllGatherHalvingDoubling::CalculateSlices(u64 size, u32 sliceNum, std::vector<Slice>& sliceOut) const
{
    ...
    for (u32 i = 0; i < sliceNum; i++) {
        ...
        HCCL_DEBUG("Slice Info: slices[%u].offset=%llu, slices[%u].size=%llu", i, slices[i].offset, i, slices[i].size);
    }
...
HcclResult AllGatherHalvingDoubling::Rx(const LINK& link, const Slice& rxSlice)
{
    ...
    // 接收数据到output
    HcclResult ret = link->RxAsync(hdOutputMemType_, rxSlice.offset + baseOffset_, rxMem.ptr(), rxSlice.size, stream_);
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[AllGatherHalvingDoubling][Rx]rank[%u] rx_async with rxMem[%p] Failed", interRank_, rxMem.ptr()),
        ret);
    ret = link->DataReceivedAck(stream_);
    CHK_PRT_RET(
        ret != HCCL_SUCCESS, HCCL_ERROR("[AllGatherHalvingDoubling][Rx]rank[%u] data_received_ack Failed", interRank_),
        ret);
    return HCCL_SUCCESS;
}
...
HcclResult AllGatherHalvingDoubling::Tx(const LINK& link, const Slice& txSlice)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[AllGatherHalvingDoubling][Tx]rank[%u] txSlice.size[%llu]Link rx_ack "
            "Failed",
            interRank_, txSlice.size),
        ret);
    DeviceMem txMem = inputMem_.range(txSlice.offset, txSlice.size);
    // input的数据发送
    ret = link->TxAsync(hdOutputMemType_, txSlice.offset + baseOffset_, txMem.ptr(), txSlice.size, stream_);
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[AllGatherHalvingDoubling][Tx]rank[%u] tx_async txMem[%p] Failed", interRank_, txMem.ptr()), ret);
    return HCCL_SUCCESS;
}
...
HcclResult AllGatherHalvingDoubling::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    HCCL_INFO(
        "AllGatherHD Run: rank[%u] totalrank[%u] inputMem[%p] outputMem[%p] count[%llu]", rank, rankSize,
...
HcclResult AllGatherHalvingDoubling::RunAllGather(u32 rank, u32 stepNum, const std::vector<LINK>& links)
{
    HcclResult ret = HCCL_SUCCESS;
    ...
    for (u32 step = 0; step < stepNum; step++) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR("[Run][AllGather]rank[%u] step[%u] tx ack to dstrank[%u] Failed", rank, step, peerRank), ret);
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS, HCCL_ERROR("[Run][AllGather]Rank[%u] rx from PeerRank[%u] failed", rank, peerRank),
            ret);
