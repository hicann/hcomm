...
HcclResult AllGatherMesh::Prepare(
...
HcclResult AllGatherMesh::Tx(const LINK& link, const Slice& txSlice, const Slice& dstSlice, Stream stream)
{
    ...
    HCCL_DEBUG(
        "rank[%u] tx srcMem[%p] output's offset[%llu] size[%llu] to dstrank's offset[%llu]", interRank_, srcMem.ptr(),
        txSlice.offset, txSlice.size, dstSlice.offset);
    HcclResult ret
...
HcclResult AllGatherMesh::Rx(const LINK& link, const Slice& srcSlice, const Slice& rxSlice, Stream stream)
{
    ...
    HCCL_DEBUG(
        "rank[%u] rx rcvMem[%p] output's offset[%llu] size[%llu] rcv data from srcrank's"
        "offset[%llu] ",
...
HcclResult AllGatherMesh::RunAllGather(
    const std::vector<LINK>& links, const std::vector<Slice>& outputSlices, const std::vector<Slice>& inputSlices)
{
    ...
    for (u32 round = 1; round < interRankSize_; round++) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS, HCCL_ERROR("[Run][AllGather]rank[%u] tx ack to rank[%u] Failed", interRank_, dstRank),
            ret);
        CHK_SMART_PTR_NULL(links[dstRank]);
        ...
        HCCL_DEBUG(
            "rank[%u] tx to rank[%u] inputslice offset[%llu] size[%llu] outputSlices "
            "offset[%llu] size[%llu] ",
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS, HCCL_ERROR("[Run][AllGather]rank[%u] tx to rank[%u] failed", interRank_, dstRank),
            ret);
        HCCL_DEBUG(
            "rank[%u] will rcv from rank[%u] inputSlices offset[%llu] size[%llu] outputSlices"
            "offset[%llu] size[%llu]",
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Run][AllGather]destrank[%u] AllGather mesh run tempAlg barrier "
                "failed",
                dstRank),
            ret);
...
HcclResult AllGatherMesh::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    if (meshStreams_.size() < subStreamSize || (*meshSignal_).size() < subStreamSize
        ...
        HCCL_ERROR(
            "[AllGatherMesh][RunAsync]AllGatherMesh stream size error: "
    ...
    for (u32 i = 0; i < interRankSize_; i++) {
        HCCL_DEBUG(
            "[AllGatherMesh][Outputslice]: size[%llu] offset[%llu] inputslice: size[%llu] offset[%llu]",
    ...
    if (inputMem_ != outputMem_) {
        ...
        HCCL_INFO(
            "inputMem != outputMem: rank[%u] copy src[%p] offset[%llu] size[%llu] to dst[%p] offset[%llu]"
            "size[%llu]",
    ...
    for (u32 streamIndex = 0; streamIndex < rankSize - 2; streamIndex++) { // rankSize-2: stream num
        HCCL_DEBUG(
            "rank[%u] streamindex[%u] wait signalaux[%p]", rank, streamIndex, (*meshSignalAux_)[streamIndex]->ptr());
    ...
    for (u32 streamIndex = 0; streamIndex < rankSize - 2; streamIndex++) { // rankSize - 2 stream num
        HCCL_DEBUG("rank[%u] streamindex[%u] wait signal[%p] ", rank, streamIndex, (*meshSignal_)[streamIndex]->ptr());
