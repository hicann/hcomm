...
HcclResult AllReduceChunkMesh::Prepare(
...
HcclResult AllReduceChunkMesh::PrepareAllreduceSliceData()
{
    ...
    for (u32 rank = 0; rank < localRankSize_; rank++) {
        ...
        CHK_PRT_RET(ret != HCCL_SUCCESS, HCCL_ERROR("[AllReduceChunkMesh][PrepareSlice]rank[%u] failed", rank), ret);
    }
    return HCCL_SUCCESS;
}
...
HcclResult AllReduceChunkMesh::RunAsync(const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    HCCL_INFO(
        "AllReduceChunkMesh run: rank[%u] ranksize[%u] inputMem[%p] outputMem[%p] count[%llu]", rank, rankSize,
        inputMem_.ptr(), outputMem_.ptr(), count_);

    if (links.size() < rankSize) {
        HCCL_ERROR(
            "[AllReduceChunkMesh][RunAsync]rank[%u] linksize[%llu] is less than rankSize[%u]", rank, links.size(),
            rankSize);
    ...
    if (rankSize == 1) {
        if (opInfo_->outputAddr != opInfo_->inputAddr) {
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[AllReduceRing][RunAsync]rank[%u] count[%llu] failed in PrepareSliceData step", rank, count_), ret);
...
{
    HCCL_INFO(
        "ReduceScatterMeshAtomicOpbase run: rank[%u] totalrank[%u] inputMem[%p] outputMem[%p] count[%llu]", rank,
    ...
    for (u32 round = 1; round < rankSize; round++) {
        ...
        for (u32 peer = 1; peer < rankSize; peer++) {
            ...
            CHK_RET(HcclReduceAsync(
                dispatcher_, static_cast<void*>(src.ptr()), sliceMap[rank][dstSlice].size / unitSize, dataType_,
                reductionOp_, subStream, static_cast<void*>(dst.ptr()), links[dstRank]->GetRemoteRank(),
