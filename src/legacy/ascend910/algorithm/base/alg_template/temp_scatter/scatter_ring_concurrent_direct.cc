...
namespace hccl {
ScatterRingConcurrentDirect::ScatterRingConcurrentDirect(const HcclDispatcher dispatcher) : AlgTemplateBase(dispatcher)
{}

ScatterRingConcurrentDirect::~ScatterRingConcurrentDirect() {}

HcclResult ScatterRingConcurrentDirect::Prepare(
...
HcclResult
ScatterRingConcurrentDirect::CheckParameters(const u32 rank, const u32 rankSize, const std::vector<LINK>& links)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ScatterRingConcurrentDirect] ringsOrder size[%u] is not equal to rank size[%u]", ringsOrder_.size(),
            rankSize),
        HCCL_E_PARA);
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[ScatterRingConcurrentDirect] userMemInputSlices size[%u] is not equal to rank size[%u]",
            userMemInputSlices_.size(), rankSize),
        HCCL_E_PARA);
...
HcclResult ScatterRingConcurrentDirect::OneRankMemcpy()
{
    ...
    if (opInfo_->outputAddr != nullptr) {
        ...
        HCCL_DEBUG(
            "[OneRankMemcpy]Memcpy operation: stream[main], rank[%u] starts to rcv offset[%llu], size[%llu] at "
            "userMemOut_",
...
HcclResult ScatterRingConcurrentDirect::GetInitializedNeighborLinks(
...
HcclResult ScatterRingConcurrentDirect::SetSlices(const u32 rank, const u32 rankSize)
{
    if (slices_.size() == 0) {
        ...
        for (u32 i = 0; i < rankSize; i++) {
            ...
            HCCL_DEBUG(
                "[SetSlices]rank[%u], slices[%u].offset=[%llu], slices[%u].size=[%llu]", rank, i, slices_[i].offset, i,
                slices_[i].size);
        }
    }
    if (UNLIKELY(HcclCheckLogLevel(DLOG_DEBUG))) {
        for (u32 i = 0; i < slices_.size(); i++) {
            HCCL_DEBUG(
                "[ScatterRingConcurrentDirect][SetSlices]rank[%u], slices[%u].offset=[%llu], slices[%u].size=[%llu]",
...
HcclResult ScatterRingConcurrentDirect::RunMainStream(
    const u32 stepsFromRank2Root, const u32 step, const Slice& txSlice, const Slice& rxSlice, const u32 rankSize)
{
    ...
    if (needSend) {
        ...
        CHK_RET(rightLink_->TxAsync(
            UserMemType::INPUT_MEM, txSlice.offset + baseOffset_, src.ptr(), txSlice.size, stream_));
    }
    // 不同的rank会在不同的step开始持续发送操作，距离root节点越近，越早step开始发送操作
    if (needReceive) {
        HCCL_DEBUG(
            "MemcpyAsync operation: step[%u] stream[main], src rank[%u] starts to send offset[%llu] size[%llu] "
            "from leftMem_",
...
HcclResult ScatterRingConcurrentDirect::RunSubStream(
