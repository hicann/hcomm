...
public:
    ...
    SumByPairs(__gm__ T* cclGMSelf, int64_t x, int64_t count, int32_t tag, int64_t flagOffsetBase, __gm__ T* outputGM);
    ...
    template <typename T>
    __aicore__ inline void
    ProcessSingleRanksizeCore(GM_ADDR input, GM_ADDR output, uint64_t len, int32_t tag, uint64_t bufferSize);
};
...
template <typename T>
__aicore__ inline void AivReduceScatterDeterMid910B::CPGM2GMAccordingFlag(
    __gm__ T* cclGMSelf, __gm__ T* cclGMOther, uint64_t count, __gm__ int32_t* ctrlFlagGMSelf,
...
template <typename T>
__aicore__ inline void AivReduceScatterDeterMid910B::SumByPairs(
    __gm__ T* cclGMSelf, int64_t x, int64_t count, int32_t tag, int64_t flagOffsetBase, __gm__ T* outputGM)
{
    ...
    if (target == 0) {
        ...
        ReduceWithFlagWrap(
            cclGMSelf + target * count, cclGMSelf + x * count, count, tag, flagCntDonePre, flagCntSelf,
            flagCntDoneSelf);
    }
}

template <typename T>
__aicore__ inline void AivReduceScatterDeterMid910B::ReduceWithFlagWrap(
    __gm__ T* cclGMSelf, __gm__ T* cclGMOther, uint64_t count, int32_t tag, __gm__ int32_t* flagCntDoneBase,
    __gm__ int32_t* flagCntSelf, __gm__ int32_t* flagCntDoneSelf)
{
    uint64_t processedBatchCount = 0;
...
template <typename T>
__aicore__ inline void
AivReduceScatterDeterMid910B::Process(GM_ADDR input, GM_ADDR output, uint64_t len, int32_t tag, uint64_t bufferSize)
{
    int64_t count = len;
    ...
    if (blockIdx_ < blockNumPerGroup) {
        CpGM2GMWithFlagWrap(
            cclGMSelf + x * count, inputGM + x * count, count, (__gm__ int32_t*)(GM_OUT[rank_] + flagOffset1stCount), 8,
            tag);
        return;
    }
    // 第二组 拷贝cclbuffer前半部分到cllbuffer后半部分
    else if (blockNumPerGroup <= blockIdx_ && blockIdx_ < 2 * blockNumPerGroup) {
        ...
        if (x == 0) {
            ...
            CPGM2GMAccordingFlag(outputGM, cclGMOther + rank_ * count, count, flagCntSelf, flagCntDoneOtner, tag, 8);
        } else {
            CPGM2GMAccordingFlag(
                cclGMSelf + allCount + x * count, cclGMOther + rank_ * count, count, flagCntSelf, flagCntDoneOtner, tag,
                8);
        }
        return;
    }
    // 第三组 进行reduce操作
        ...
        if (x != 0) {
            if (rankSize_ >= DETERMINISTIC_RANKSIZE) {
                ...
                ReduceWithFlagWrap(
                    outputGM, cclGMSelf + allCount + x * count, count, tag, flagCntDonePre, flagCntSelf,
                    flagCntDoneSelf);
            }
        }
    }
...
template <typename T>
__aicore__ inline void AivReduceScatterDeterMid910B::ProcessSingleRanksizeCore(
    GM_ADDR input, GM_ADDR output, uint64_t len, int32_t tag, uint64_t bufferSize)
{
    int64_t count = len;
    int64_t allCount = count * rankSize_;
    int64_t blockNumPerGroup = rankSize_;
    ...
    if (x != 0) {
        if (rankSize_ >= DETERMINISTIC_RANKSIZE) {
            ...
            __gm__ int32_t* flagCntDoneSelf = (__gm__ int32_t*)(GM_OUT[rank_] + flagOffset3stCount);
            ReduceWithFlagWrap(
                outputGM, cclGMSelf + allCount + x * count, count, tag, flagCntDonePre, flagCntSelf, flagCntDoneSelf);
        }
    }
}

template <typename T>
__aicore__ inline void AivReduceScatterDeterMid910B::ProcessProxy(
    GM_ADDR input, GM_ADDR output, uint64_t len, int32_t tag, uint64_t bufferSize)
{
