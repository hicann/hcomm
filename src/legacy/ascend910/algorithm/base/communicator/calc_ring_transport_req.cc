...
HcclResult CalcRingTransportReq::CalcTransportRequest(
    ...
    for (u32 ringIndex = 0; ringIndex < ringSize; ringIndex++) {
        ...
        for (u32 rankIndex = 0; rankIndex < rankSize; rankIndex++) {
            ...
            if (rankIndex == (rank + rankSize + HCCL_RANK_OFFSET) % rankSize
                ...
                HCCL_INFO(
                    "[CommFactory][CalcRingCommInfo] param_.tag[%s] ringIndex[%u], localRank[%u], "
...
HcclResult
CalcRingTransportReq::CalcDstRanks(const u32 rank, const std::vector<u32> commGroups, std::set<u32>& dstRanks)
{
    CHK_PRT_RET(
...
REGISTER_AHC_COMM_CALC_FUNC(
    AHCTemplateType::AHC_TEMPLATE_RING, CalcRingTransportReq, CalcRingTransportReq::CalcDstRanks);
} // namespace hccl
