...
CommP2P::CommP2P(
    ...
    const bool isUsedRdmaLevel0, const std::string& tag, const u32 dstUserRank, const NICDeployment nicDeployInner,
...
HcclResult CommP2P::CalcLink()
{
    ...
    if (paraVector_.size() <= dstRank) {
        HCCL_ERROR(
            "[Calc][Link]comm p2p dstRank[%u] is bigger than para_vector size[%llu]", dstRank, paraVector_.size());
        return HCCL_E_PARA;
    }
    ...
    if (!isSupportP2P) {
        HCCL_ERROR(
            "[Calc][Link]comm p2p rank[%u] devid[%d] serverId[%s] and dstRank[%u] devid[%d] serverId[%s] is "
            "not support P2P",
            rank_, paraVector_[rank_].devicePhyId, paraVector_[rank_].serverId.c_str(), dstRank,
            paraVector_[dstRank].devicePhyId, paraVector_[dstRank].serverId.c_str());
        return HCCL_E_PARA;
    }
    ...
    if (rank_ < dstRank) {
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Calc][Link]comm p2p calc links num failed, type[%d], dstRank[%u]",
