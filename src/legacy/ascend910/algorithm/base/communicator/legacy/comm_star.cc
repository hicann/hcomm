...
CommStar::CommStar(
    ...
    const bool isUsedRdmaLevel0, const std::string& tag, const NICDeployment nicDeployInner, const u32 subUserRankRoot,
    bool isHaveCpuRank)
    : CommBase(
          collectiveId, userRank, userRankSize, rank, rankSize, paraVector, topoFlag, dispatcher, notifyPool,
          netDevCtxMap, exchanger, inputMem, outputMem, isUsedRdmaLevel0, tag, nicDeployInner, false, false, false,
          subUserRankRoot, isHaveCpuRank)
{
...
HcclResult CommStar::CalcLink()
{
    if (rank_ == subUserRankRoot_) {
        for (u32 dstRank = 0; dstRank < rankSize_; dstRank++) {
            if (dstRank != rank_) {
                ...
                CHK_PRT_RET(
                    ret != HCCL_SUCCESS,
                    HCCL_ERROR(
                        "[Calc][Link]comm Star calc links num failed, type[%d], dstRank[%u]",
        ...
        CHK_PRT_RET(
            ret != HCCL_SUCCESS,
            HCCL_ERROR(
                "[Calc][Link]comm Star calc links num failed, type[%d], Root[%u]",
                static_cast<int32_t>(MachineType::MACHINE_CLIENT_TYPE), subUserRankRoot_),
            ret);
    }
...
HcclResult CommStar::CreateInterLinks()
{
    ...
    if (paraVector_[rank_].devicePhyId == HOST_DEVICE_ID && isHostUseDevNic_) {
        ...
        for (auto phyNicInfo : rankDevicePhyIdNicInfoMap_[hostIp]) {
            ...
            HCCL_DEBUG(
                "[Create][InterLinks] dstInterServerMap size[%u], dstInterClientMap size[%u]",
            ...
            for (auto& serverInfo : dstInterServerMap_) {
                if (targetDevicePhyId == serverInfo.second.devicePhyId) {
                    HCCL_DEBUG(
                        "[Create][InterLinks] targetDevicePhyId[%u], phyNicInfo.second[%s] serverInfo "
                        "dstRank[%u] serverInfo.second.devicePhyId[%u]",
                        targetDevicePhyId, phyNicInfo.second.GetReadableAddress(), serverInfo.first,
                        serverInfo.second.devicePhyId);
                    ret = pyhIdResourseSockets_[targetDevicePhyId]->CreateSockets(
                        tag_, true, netDevCtxMap_[phyNicInfo.second], dstInterServerMap_, dstInterClientMap_,
        ...
        HCCL_INFO(
            "[Create][InterLinks] dstInterServerMap size[%u], dstInterClientMap size[%u]", dstInterServerMap_.size(),
            dstInterClientMap_.size());
...
HcclResult CommStar::GetDevIP(const HcclIpAddress& hostIp, const u32& devicePhyId, HcclIpAddress& ip)
{
    std::string hostIpStr(hostIp.GetReadableAddress());
    CHK_PRT_RET(
        rankDevicePhyIdNicInfoMap_.find(hostIpStr) == rankDevicePhyIdNicInfoMap_.end()
            || rankDevicePhyIdNicInfoMap_[hostIpStr].find(devicePhyId) == rankDevicePhyIdNicInfoMap_[hostIpStr].end(),
        HCCL_ERROR(
            "Get available device nic info fail,"
            "hostIp[%s] devicePhyId[%u]",
            hostIpStr.c_str(), devicePhyId),
        HCCL_E_PARA);
    ip = rankDevicePhyIdNicInfoMap_[hostIpStr][devicePhyId];
    HCCL_DEBUG(
        "Get available device nic info success, hostIp[%s] devicePhyId[%u] device ip[%s]", hostIpStr.c_str(),
        devicePhyId, ip.GetReadableAddress());

    return HCCL_SUCCESS;
}

HcclResult CommStar::SetMachinePara(
    ...
    const std::vector<std::shared_ptr<HcclSocket>>& sockets, MachinePara& machinePara)
{
    ...
    if (paraVector_[rank_].devicePhyId == HOST_DEVICE_ID && paraVector_[dstRank].devicePhyId != HOST_DEVICE_ID
        && rankDevicePhyIdNicInfoMap_.find(localHostIpStr) != rankDevicePhyIdNicInfoMap_.end()) {
        CHK_PRT(GetDevIP(paraVector_[rank_].hostIp, paraVector_[dstRank].devicePhyId, machinePara.localIpAddr));
    ...
    if (paraVector_[dstRank].devicePhyId == HOST_DEVICE_ID && paraVector_[rank_].devicePhyId != HOST_DEVICE_ID
        && rankDevicePhyIdNicInfoMap_.find(remoteHostIpStr) != rankDevicePhyIdNicInfoMap_.end()) {
        CHK_PRT(GetDevIP(paraVector_[dstRank].hostIp, paraVector_[rank_].devicePhyId, machinePara.remoteIpAddr));
    }

    HCCL_INFO(
        "selfIp[%s] selfPort[%u] peerIp[%s] peerPort[%u] deviceLogicId[%d].",
...
void CommStar::SetTransportParam(TransportPara& para, MachinePara& machinePara)
{
    ...
    HCCL_INFO(
        "SetTransportParam proxyDevLogicId[%u] deviceType is %u", para.proxyDevLogicId, paraVector_[rank_].deviceType);
    ...
    if (paraVector_[rank_].deviceType == DevType::DEV_TYPE_NOSOC) {
        ...
        para.isESPs = true;
        HCCL_INFO(
            "selfIp[%s] selfPort[%u] peerIp[%s] peerPort[%u] qpMode[%d].", para.selfIp->GetReadableAddress(),
    ...
    if (paraVector_[rank_].devicePhyId == HOST_DEVICE_ID) {
        ...
        if (paraVector_[rank_].deviceType == DevType::DEV_TYPE_910B) {
            para.qpMode
                = (GetWorkflowMode() == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ? OPBASE_QP_MODE_EXT :
                                                                                       OFFLINE_QP_MODE_EXT);
        } else {
            para.qpMode
                = (GetWorkflowMode() == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ? OPBASE_QP_MODE :
                                                                                       OFFLINE_QP_MODE);
        }
    ...
    para.isHdcMode = true;
    HCCL_INFO(
        "selfIp[%s] selfPort[%u] peerIp[%s] peerPort[%u] qpMode[%d].", para.selfIp->GetReadableAddress(), para.selfPort,
        para.peerIp->GetReadableAddress(), para.peerPort, para.qpMode);
}
