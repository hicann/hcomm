...
public:
    explicit CommBase(
        ...
        const NICDeployment nicDeployInner = NICDeployment::NIC_DEPLOYMENT_DEVICE, bool isAlltoAllCommMesh = false,
        const bool useOneDoorbell = false, const bool isAicpuModeEn = false, const u32 rankRoot = INVALID_UINT,
        const bool isHaveCpuRank = false, const bool useSuperPodMode = false, DeviceMem expMem = DeviceMem());
    virtual ~CommBase();

    inline const std::string& CollectiveId() const { return collectiveId_; }

    inline const std::vector<LINK>& TransportInfo() const { return transportInfo_; }

    inline u32 UserRank() const { return userRank_; }

    inline u32 UserRankSize() const { return userRankSize_; }

    inline u32 Rank() const { return rank_; }

    inline u32 RankSize() const { return rankSize_; }

    inline const std::string& Tag() const { return tag_; }

    inline void SetHeterogP2PType() { isNeedHeterogP2P_ = true; }

    inline void SetHostUseDevNic() { isHostUseDevNic_ = true; }
    ...
    HcclResult CreateIntraThread(
        const u32 role, u32 dstRank,
        const std::vector<std::shared_ptr<HcclSocket>>& sockets); // 节点内建链起线程
    HcclResult CreateInterThread(
        const u32 role, u32 dstRank,
...
protected:
    ...
    virtual HcclResult SetMachinePara(
    ...
    HcclResult TransportBuildAsync(
...
protected:
    ...
    // 超节点server间HCCS场景使用
    HcclResult GetSuperNodeIntraRankIPInfo(
        std::map<u32, HcclSocketRole>& rankRole, HcclIpAddress& localIP, std::map<u32, HcclRankLinkInfo>& dstServerMap,
        std::map<u32, HcclRankLinkInfo>& dstClientMap);
