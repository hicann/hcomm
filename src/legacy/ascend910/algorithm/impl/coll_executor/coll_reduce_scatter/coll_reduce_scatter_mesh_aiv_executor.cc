...
namespace hccl {
CollReduceScatterMeshAivExecutor::CollReduceScatterMeshAivExecutor(
...
HcclResult CollReduceScatterMeshAivExecutor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollReduceScatterMeshAivExecutor][CalcStreamNum] tag[%s] streamNum[%u].", tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
...
CollReduceScatterMeshAivExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType)
{
    ...
    HCCL_INFO(
        "[CollReduceScatterMeshAivExecutor][CalcTransportMemType] tag[%s] inputType[%d],"
        " outputType[%d].",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollReduceScatterMeshAivExecutor::CalcLevel0CommInfo(
...
HcclResult
CollReduceScatterMeshAivExecutor::CalNumBlocks(u32& numBlocks, u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_WARNING(
            "[CollReduceScatterMeshAivExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].", numBlocks_,
            rankSize),
        HCCL_E_PARA);

    CHK_PRT_RET(
        numBlocks_ < bestNumBlocks && (topoAttr_.deviceType != DevType::DEV_TYPE_910_93 || isOpBase),
        HCCL_WARNING(
            "[CollReduceScatterMeshAivExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].", numBlocks_,
            bestNumBlocks),
        HCCL_E_PARA);
...
HcclResult CollReduceScatterMeshAivExecutor::GetAivExecParam(
    const OpParam& param, AlgResourceResponse& algRes, AivSuperKernelArgs& args)
{
    ...
    HCCL_DEBUG(
        "[CollReduceScatterMeshAivExecutor][GetAivExecParam] userRank [%d] localRank [%d]", topoAttr_.userRank,
        localRank);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollReduceScatterMeshAivExecutor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel "
            "run failed",
...
HcclResult CollReduceScatterMeshAivExecutor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    u32 localRankSize = level0CommInfo.localRankSize;
    HCCL_DEBUG(
        "[CollReduceScatterMeshAivExecutor][KernelRun] userRank [%d] localRank [%d]", topoAttr_.userRank, localRank);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollReduceScatterMeshAivExecutor][KernelRun]ReduceScatter aiv failed, return[%d]", ret), ret);
