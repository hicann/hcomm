...
HcclResult
CollReduceScatterAivDeterSmallExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType)
{
...
CollReduceScatterAivDeterSmallExecutor::CalNumBlocks(u32& numBlocks, u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    ...
    HCCL_INFO(
        "[CollReduceScatterAivDeterSmallExecutor][CalNumBlocks] numBlocks is set to [%u], limit[%u], recommanded[%u]",
    ...
    const OpParam& param, AlgResourceResponse& algRes, AivSuperKernelArgs& args)
{
    ...
    HCCL_INFO(
        "SPK [CollReduceScatterAivDeterSmallExecutor][GetAivExecParam], rank[%llu], rankSize[%llu], "
