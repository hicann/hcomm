    ...
    TransportMemType& inputType, TransportMemType& outputType)
{
    ...
    HCCL_INFO(
        "[CollReduceScatterVMeshAivSmallCountExecutor][CalcTransportMemType] tag[%s] inputType[%d],"
        " outputType[%d].",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollReduceScatterVMeshAivSmallCountExecutor::CalcLevel0CommInfo(
...
HcclResult CollReduceScatterVMeshAivSmallCountExecutor::CalNumBlocks(
    u32& numBlocks, u32 rankSize, u64 dataSize, HcclCMDType cmdType)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_WARNING(
            "[CollReduceScatterVMeshAivSmallCountExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].",
            numBlocks_, numBlocks),
        HCCL_E_PARA);
...
HcclResult CollReduceScatterVMeshAivSmallCountExecutor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollReduceScatterVMeshAivSmallCountExecutor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel "
            "run failed.",
...
HcclResult CollReduceScatterVMeshAivSmallCountExecutor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    HCCL_DEBUG(
        "[CollReduceScatterVMeshAivSmallCountExecutor][KernelRun] userRank [%d] localRank [%d]", topoAttr_.userRank,
        localRank);
    ...
    for (u32 i = 0; i < localRankSize; i++) {
        ...
        HCCL_INFO(
            "[CollReduceScatterVMeshAivSmallCountExecutor][KernelRun]rank[%u], sendCount[%llu], sendDispl[%llu]", i,
            extraArgs.sendCounts[i], extraArgs.sendDispls[i]);
    }
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollReduceScatterVMeshAivSmallCountExecutor][KernelRun]ReduceScatterV aiv failed, return[%d].", ret),
        ret);
...
REGISTER_EXEC(
    "ReduceScatterVMeshAivSmallCountExecutor", ReduceScatterVMeshAivSmallCount,
