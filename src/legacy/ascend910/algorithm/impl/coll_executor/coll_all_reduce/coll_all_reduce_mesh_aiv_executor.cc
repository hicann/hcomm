...
CollAllReduceMeshAivExecutor::CollAllReduceMeshAivExecutor(
...
HcclResult CollAllReduceMeshAivExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType)
{
    ...
    HCCL_INFO(
        "[CollAllReduceMeshAivExecutor][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d].", tag_.c_str(),
        inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshAivExecutor::CalcLevel0CommInfo(
...
HcclResult CollAllReduceMeshAivExecutor::CalNumBlocks(
    u32& numBlocks, u32 rankSize, [[maybe_unused]] u64 dataSize, [[maybe_unused]] HcclCMDType cmdType)
{
    ...
    CHK_PRT_RET(
        numBlocks_ < rankSize,
        HCCL_WARNING(
            "[CollAllReduceMeshAivExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].", numBlocks_,
            rankSize),
        HCCL_E_PARA);
    CHK_PRT_RET(
    ...
    HCCL_INFO(
        "[CollAllReduceMeshAivExecutor][CalNumBlocks] numBlocks is set to [%u], limit[%u], recommanded[%u]", numBlocks,
...
HcclResult CollAllReduceMeshAivExecutor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllReduceMeshAivExecutor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel run failed",
...
HcclResult CollAllReduceMeshAivExecutor::GetAdjInfo(
    [[maybe_unused]] AlgResourceResponse& algRes, [[maybe_unused]] AdjInfo& adjInfo)
{
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshAivExecutor::GetAivExecParam(
    const OpParam& param, AlgResourceResponse& algRes, AivSuperKernelArgs& args)
{
    ...
    HCCL_DEBUG(
        "[CollAllReduceMeshAivExecutor][GetAivExecParam] userRank [%d] localRank [%d]", topoAttr_.userRank, localRank);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllReduceMeshAivExecutor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel "
            "run failed",
            HCCL_ERROR_CODE(ret), param.tag.c_str()),
        ret);
