...
HcclResult CollAllReduceFastDoubleRingFor91093Executor::DoubleRingReduceScatter(
    ...
    const std::vector<std::vector<Slice>> multRingsUserMemSlice, const bool disableDMAReduce)
{
    ...
    std::unique_ptr<AlgTemplateBase> tempAlg
        = AlgTemplateRegistry::Instance().GetAlgTemplate(TemplateType::TEMPLATE_REDUCESCATTER_DB_RING_SLC, dispatcher_);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllReduceFastDoubleRingFor91093Executor][DoubleRingReduceScatter] Double ring "
    ...
    const HcomCollOpInfo* opInfo, const std::vector<std::vector<Slice>> multRingsUserMemSlice)
{
    (void)tag;
    HCCL_CONFIG_INFO(
        HCCL_ALG, "[CollAllReduceFastDoubleRingFor91093Executor][DoubleRingAllGather] DoubleRingAllGather starts");
    HcclResult ret = HCCL_SUCCESS;
    u32 ringNum = multRingsSliceZero.size();
    CHK_RET(CheckCommSize(COMM_LEVEL0, ringNum));
    // 拿到ring环映射关系
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllReduceFastDoubleRingFor91093Executor][DoubleRingAllGather]Double ring "
...
REGISTER_EXEC(
    "AllReduceFastDoubleRingFor91093Executor", AllReduceFastDoubleRingFor91093,
