...
CollAllReduceMeshAivFor91093Executor::CollAllReduceMeshAivFor91093Executor(
...
HcclResult CollAllReduceMeshAivFor91093Executor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollAllReduceMeshAivFor91093Executor][CalcStreamNum] tag[%s] streamNum[%u].", tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
...
HcclResult
CollAllReduceMeshAivFor91093Executor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType)
{
    inputType
        = (workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ? TransportMemType::CCL_INPUT :
                                                                           TransportMemType::SCRATCH);
    outputType = TransportMemType::AIV_OUTPUT;
    HCCL_INFO(
        "[CollAllReduceMeshAivFor91093Executor][CalcTransportMemType] tag[%s] inputType[%d],"
        " outputType[%d].",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshAivFor91093Executor::CalcLevel0CommInfo(
    ...
    u32& numBlocks, u32 rankSize, [[maybe_unused]] u64 dataSize, [[maybe_unused]] HcclCMDType cmdType)
{
    ...
    CHK_PRT_RET(
        ...
        HCCL_ERROR(
            "[CollAllReduceMeshAivFor91093Executor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].",
            numBlocks_, minNumBlocks),
        HCCL_E_PARA);

    HCCL_INFO(
        "[CollAllReduceMeshAivFor91093Executor][CalNumBlocks] numBlocks is set to [%u], limit[%u], recommanded[%u]",
        numBlocks, numBlocks_, bestNumBlocks);
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshAivFor91093Executor::GetAivExecParam(
    const OpParam& param, AlgResourceResponse& algRes, AivSuperKernelArgs& args)
{
    ...
    HCCL_DEBUG(
        "[CollAllReduceMeshAivFor91093Executor][GetAivExecParam] userRank [%d] localRank [%d]", topoAttr_.userRank,
        localRank);
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllReduceMeshAivFor91093Executor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel "
            "run failed",
            HCCL_ERROR_CODE(ret), param.tag.c_str()),
        ret);

    HCCL_INFO(
        "tag[%s], AllReduce executor getalgexecparam success, take time [%lld]us.", param.tag.c_str(),
        DURATION_US(TIME_NOW() - startut));
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshAivFor91093Executor::CopyAivCommInfoToDevice(
...
HcclResult CollAllReduceMeshAivFor91093Executor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    execMem.inputMem
        = (workflowMode_ != HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE ? algRes.scratchMem : algRes.cclInputMem);
    execMem.outputMem = algRes.aivOutputMem;
...
HcclResult CollAllReduceMeshAivFor91093Executor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    u32 localRankSize = level0CommInfo.localRankSize;
    HCCL_DEBUG(
        "[CollAllReduceMeshAivFor91093Executor][KernelRun] userRank [%d] localRank [%d]", topoAttr_.userRank,
        localRank);
    ...
    AivOpArgs opArgs{
        ...
        0,
        isOpbase};
    AivTopoArgs topoArgs{localRank,           localRankSize,        MAX_RANK_SIZE,       0,
                         topoAttr_.serverNum, topoAttr_.deviceType, algoAttr_.identifier};

    u32 numBlocks;
    CHK_PRT_RET(
        CalNumBlocks(numBlocks, localRankSize, opArgs.count * SIZE_TABLE[opArgs.dataType]) != HCCL_SUCCESS,
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAllReduceMeshAivFor91093Executor][KernelRun]allreduce aiv failed, return[%d]", ret), ret);
