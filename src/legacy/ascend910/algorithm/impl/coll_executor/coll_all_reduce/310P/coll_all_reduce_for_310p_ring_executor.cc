...
namespace hccl {
CollAllReduceFor310PRingExecutor::CollAllReduceFor310PRingExecutor(
...
HcclResult CollAllReduceFor310PRingExecutor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    HCCL_CONFIG_INFO(
        HCCL_ALG, "[CollAllReduceFor310PRingExecutor][KernelRun] userRank[%u] starts with isInlineReduce[%d]",
...
HcclResult CollAllReduceFor310PRingExecutor::CalcLevel0CommInfo(
...
CollAllReduceFor310PRingExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType) const
{
    ...
    HCCL_INFO(
        "[CollAllReduceFor310PRingExecutor][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]", tag_.c_str(),
