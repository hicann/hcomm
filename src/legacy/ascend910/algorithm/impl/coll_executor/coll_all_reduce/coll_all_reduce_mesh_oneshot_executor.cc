...
CollAllReduceMeshOneshotExecutor::CollAllReduceMeshOneshotExecutor(
...
HcclResult CollAllReduceMeshOneshotExecutor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollAllReduceMeshOneshotExecutor][CalcStreamNum] tag[%s] streamNum[%u]", tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
...
HcclResult
CollAllReduceMeshOneshotExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType) const
{
    ...
    HCCL_INFO(
        "[CollAllReduceMeshOneshotExecutor][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d]", tag_.c_str(),
        inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshOneshotExecutor::CalcLevel0CommInfo(
...
bool CollAllReduceMeshOneshotExecutor::IsSmallData(const u64 totalSize, [[maybe_unused]] const u64 curSize)
{
...
{
    HCCL_CONFIG_INFO(
        HCCL_ALG, "[CollAllReduceMeshOneshotExecutor][KernelRun] userRank[%u] starts.", topoAttr_.userRank);
