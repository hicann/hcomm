...
CollAllReduceMeshOpbaseExecutor::CollAllReduceMeshOpbaseExecutor(
...
HcclResult CollAllReduceMeshOpbaseExecutor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollAllReduceMeshOpbaseExecutor][CalcStreamNum] tag[%s] streamNum[%u].", tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
...
HcclResult
CollAllReduceMeshOpbaseExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType) const
{
    ...
    HCCL_INFO(
        "[CollAllReduceMeshOpbaseExecutor][CalcTransportMemType] tag[%s] inputType[%d], outputType[%d].", tag_.c_str(),
        inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllReduceMeshOpbaseExecutor::CalcLevel0CommInfo(
...
{
    HCCL_CONFIG_INFO(HCCL_ALG, "[CollAllReduceMeshOpbaseExecutor][KernelRun] userRank[%u] starts.", topoAttr_.userRank);
