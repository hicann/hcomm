...
namespace hccl {
CollAllGatherMeshAivExecutor::CollAllGatherMeshAivExecutor(
...
HcclResult CollAllGatherMeshAivExecutor::CalcStreamNum(u32& streamNum)
{
    ...
    HCCL_INFO("[CollAllGatherMeshAivExecutor][CalcStreamNum] tag[%s] streamNum[%u].", tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}
...
HcclResult CollAllGatherMeshAivExecutor::CalcTransportMemType(TransportMemType& inputType, TransportMemType& outputType)
{
    ...
    HCCL_INFO(
        "[CollAllGatherMeshAivExecutor][CalcTransportMemType] tag[%s] inputType[%d],"
        " outputType[%d].",
        tag_.c_str(), inputType, outputType);
    return HCCL_SUCCESS;
}

HcclResult CollAllGatherMeshAivExecutor::CalcLevel0CommInfo(
...
HcclResult CollAllGatherMeshAivExecutor::CalNumBlocks(
    u32& numBlocks, u32 rankSize, [[maybe_unused]] u64 dataSize, [[maybe_unused]] HcclCMDType cmdType)
{
    ...
    CHK_PRT_RET(
        numBlocks_ < rankSize,
        HCCL_WARNING(
            "[CollAllGatherMeshAivExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].", numBlocks_,
            rankSize),
        HCCL_E_PARA);
    CHK_PRT_RET(
        isOpBase && numBlocks_ < bestNumBlocks,
        HCCL_WARNING(
            "[CollAllGatherMeshAivExecutor][CalNumBlocks]aivCore[%u] is invalid, at least need [%u].", numBlocks_,
            bestNumBlocks),
        HCCL_E_PARA);
...
HcclResult CollAllGatherMeshAivExecutor::Orchestrate(OpParam& param, AlgResourceResponse& algRes)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR(
            "[CollAllGatherMeshAivExecutor][Orchestrate]errNo[0x%016llx] tag[%s] executor kernel "
            "run failed",
...
HcclResult CollAllGatherMeshAivExecutor::GetAivExecParam(
    const OpParam& param, AlgResourceResponse& algRes, AivSuperKernelArgs& args)
{
    ...
    HCCL_DEBUG(
        "[CollAllGatherMeshAivExecutor][GetAivExecParam] userRank [%d] localRank [%d]", topoAttr_.userRank, localRank);
...
HcclResult CollAllGatherMeshAivExecutor::KernelRun(const OpParam& param, ExecMem& execMem)
{
    ...
    CHK_PRT_RET(
        ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollAllGatherMeshAivExecutor][KernelRun]AllGather aiv failed, return[%d]", ret), ret);
