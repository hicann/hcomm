...
TransportDeviceIbverbs::TransportDeviceIbverbs(DispatcherPub *dispatcher, const std::unique_ptr<NotifyPool> &notifyPool,
    MachinePara &machinePara, std::chrono::milliseconds timeout, const TransportDeviceIbverbsData &transDevIbverbsData)
...
HcclResult TransportDeviceIbverbs::Init()
{
    ...
    for (u32 i = 1, j = 0; i < qpInfoSize; i++, j++) {
        ...
        HCCL_DEBUG("TransportDeviceIbverbs Init multiQp[%u], aiQpAddr[%llu] sqIndex[%u] dbIndex[%u]", j,
    ...
    for (u32 i = 0; i < transDevIbverbsData_.qpsPerConnection; ++i) {
        CHK_PRT_RET(transDevIbverbsData_.userLocalNotify[i].empty() && transDevIbverbsData_.qpsPerConnection > 1,
            HCCL_ERROR("[TransportDeviceIbverbs][Init]userLocalNotify[%u] is empty, qpsPerConnection[%u]", i,
    ...
    for (u32 i = 0; i < transDevIbverbsData_.qpsPerConnection; ++i) {
        ...
        if (transDevIbverbsData_.qpsPerConnection > 1) {
            ...
            HCCL_INFO("multiQpDataNotifyRemoteMemMsg_[%u] addr[0x%llx] notifyId[%u] lkey[%u]", i,
...
HcclResult TransportDeviceIbverbs::AddWrList(void *dstMemPtr, const void *srcMemPtr, u64 srcMemSize, u32 srcKey,
    u32 dstKey, WqeType wqeType, WrAuxInfo &aux, std::vector<WrInformation> &wrInfoVec, bool isLast)
{
    ...
    HCCL_DEBUG("wrInfoTmp dst_addr[0x%llx] memList addr[0x%llx] len[%llu] isLast[%u]", wrInfoTmp.wrData.dstAddr,
...
HcclResult TransportDeviceIbverbs::GetMemInfo(
    UserMemType memType, void **dstMemPtr, unsigned int *dstKey, u64 &dstMemSize)
{
...
HcclResult TransportDeviceIbverbs::ConstructPayLoadWqe(void *dstMemPtr, u32 dstKey, const void *src, u32 srcKey,
    u64 len, WqeType wqeType, WrAuxInfo &aux, std::vector<WrInformation> &wrInfoVec, u32 txSendDataTimes, bool isLast)
{
    HcclResult ret;
    // 发送数据Wqe
...
HcclResult TransportDeviceIbverbs::TxAsync(
    UserMemType dstMemType, u64 dstOffset, const void *src, u64 len, Stream &stream)
{
...
HcclResult TransportDeviceIbverbs::TxSendDataAndNotify(
    std::vector<WrInformation> &wrInfoVec, Stream &stream, bool useOneDoorbell)
{
    u32 maxLength = 0;
        ...
        *completeNum += completeNumLocal;
        ...
        if (ret == HCCL_E_AGAIN || *completeNum < sendNum) {
            ...
            CHK_PRT_RET(bTimeout,
                HCCL_ERROR("[Aicpu][Send][Wr]errNo[0x%016llx] wrlist send async timeout[%d]ms. "
    ...
    WrInformation wrList[], u32 sendNum, struct SendWrRsp opRsp[], unsigned int *completeNum, u32 multiQpIndex)
{
    ...
    for (; i < sendNum; i++) {
        ...
        if (wrList[i].wrData.op == RA_WR_RDMA_ATOMIC_WRITE) {
        ...
        } else if (wrList[i].wrData.op == RA_WR_RDMA_WRITE_WITH_NOTIFY || wrList[i].wrData.op == RA_WR_RDMA_REDUCE_WRITE
            ...
            HCOMM_DSB();
            HCCL_DEBUG("ibv_exp_post_send, op = [0x%x], remote_addr = [0x%llx], size = [%d]", wrList[i].wrData.op,
...
HcclResult TransportDeviceIbverbs::RdmaSendAsync(
    struct SendWr &wr, Stream &stream, WqeType wqeType, u64 notifyAddr, u32 notifyId)
{
    HcclResult ret;
    WrInformation wrInfoTmp;
...
HcclResult TransportDeviceIbverbs::RxAsync(UserMemType srcMemType, u64 srcOffset, void *dst, u64 len, Stream &stream)
{
    ...
    if (UseMultiQp() && actualMultiQpNum != 1 && actualMultiQpNum <= qpsPerConnection_ && len != 0) {
        for (u32 i = 0; i < actualMultiQpNum; i++) {
            CHK_RET(dispatcher_->SignalWait(multiQpDataNotify_[i]->ptr(), stream, machinePara_.localUserrank,
                machinePara_.remoteWorldRank, INVALID_VALUE_STAGE, false, multiQpDataNotify_[i]->notifyId_));
        }
    } else {
...
{
    CHK_RET(dispatcher_->SignalWait(ackNotify_->ptr(), stream, machinePara_.localUserrank, machinePara_.remoteWorldRank,
        INVALID_VALUE_STAGE, false, ackNotify_->notifyId_));
    return HCCL_SUCCESS;
}
...
HcclResult TransportDeviceIbverbs::TxData(
    UserMemType dstMemType, u64 dstOffset, const void *src, u64 len, Stream &stream)
{
    CHK_SMART_PTR_NULL(stream);
    std::vector<WrInformation> wrInfoVec;
    ...
    const void *remoteAddr, const void *localAddr, u64 length, Stream &stream, WqeType wqeType, struct WrAuxInfo &aux)
{
    ...
    if (localAddr != nullptr) {
        ...
        CHK_RET(ConstructPayLoadWqe(resolve.transRemoteAddr, resolve.dstKey, resolve.transLocalAddr, resolve.srcKey,
...
HcclResult TransportDeviceIbverbs::ResolveTransferDesc(const HcommBatchTransferDesc &desc, const void *&remoteAddr,
    ...
    const HcommBatchTransferDesc *transferDescs, uint32_t descNum, Stream &stream)
{
    ...
    for (uint32_t i = 0; i < descNum; i++) {
        ...
        HCCL_DEBUG("[BatchTransferImpl] index[%u] localAddr[%p] remoteAddr[%p] len[%llu] wqeType[%d]", i, localAddr,
        ...
        if (localAddr != nullptr) {
            ...
            CHK_RET(ResolveRdmaAddrsAndKeys(resolve));
            CHK_RET(ConstructPayLoadWqe(resolve.transRemoteAddr, resolve.dstKey, resolve.transLocalAddr, resolve.srcKey,
                length, wqeType, aux, wrInfoVec, txSendDataTimes, isLast));
        }
    }
...
HcclResult TransportDeviceIbverbs::Post(u32 notifyIdx, Stream &stream)
{
    ...
    CHK_PRT_RET(bRet,
        HCCL_ERROR("[TransportDeviceIbverbs][Post]notifyNum[%u], notifyIdx[%u] out of range[0, %u]", notifyNum_,
            notifyIdx, notifyNum_ - 1),
        HCCL_E_INTERNAL);

    // 每个QP发送一个指定idx的notify
    for (u32 i = 0; i < qpsPerConnection_; i++) {
        CHK_RET(TxSendWqe(userMultiQpRemoteNotifyMsg_[i][notifyIdx].addr,
            userMultiQpRemoteNotifyMsg_[i][notifyIdx].lkey, memMsg_[static_cast<u32>(MemType::NOTIFY_SRC_MEM)].addr,
            memMsg_[static_cast<u32>(MemType::NOTIFY_SRC_MEM)].lkey, notifySize_, stream,
...
HcclResult TransportDeviceIbverbs::Wait(u32 notifyIdx, Stream &stream, const u32 timeOut)
{
    ...
    bool bRet = (notifyIdx >= notifyNum_);
    CHK_PRT_RET(bRet,
        HCCL_ERROR("[TransportDeviceIbverbs][Wait]notifyNum[%u], notifyIdx[%u] out of range[0, %u]", notifyNum_,
            notifyIdx, notifyNum_ - 1),
        HCCL_E_INTERNAL);
    ...
    for (u32 i = 0; i < qpsPerConnection_; i++) {
        CHK_RET(dispatcher_->SignalWait(userMultiQpLocalNotify_[i][notifyIdx]->ptr(), stream,
            machinePara_.localUserrank, machinePara_.remoteWorldRank, INVALID_VALUE_STAGE, false,
    ...
    struct MemDetails *remoteMems, u32 memNum, HcclWrOpCode opCode, u64 &dbInfo, bool fence)
{
    ...
    for (uint32_t index = 0; index < memNum; index++) {
        ...
        sendWr[index].send_flags = (index == last) ? (fence ? (IBV_SEND_SIGNALED | IBV_SEND_FENCE) : IBV_SEND_SIGNALED)
                                                   : 0;                      // 最后一个WR才需要回复CQE
