    ...
    if (!tmpIp.IsInvalid()) {
        ...
        RPT_ENV_ERR(
            ...
            std::vector<std::string>(
                {tmpIp.GetReadableAddress(), "HCCL_SOCKET_IFNAME",
        ...
        if (ret != HCCL_SUCCESS) {
            ...
            RPT_ENV_ERR(
                true, "EI0001", std::vector<std::string>({"value", "env", "expect"}),
                std::vector<std::string>(
                    {hcclSocketIfnameStr, "HCCL_SOCKET_IFNAME",
                     "a valid network interface name (e.g., eth0, bound0) present on this host"}));
            for (auto& ifInfo : ifInfos) {
                HCCL_ERROR(
                    "[%s][%s]get host ip fail by socket Ifname. name[%s] ip[%s]", LOG_KEYWORDS_INIT_GROUP.c_str(),
                    LOG_KEYWORDS_ENV_CONFIG.c_str(), ifInfo.first.c_str(), ifInfo.second.GetReadableAddress());
            }
            return HCCL_E_NOT_FOUND;
        }
    } else {
