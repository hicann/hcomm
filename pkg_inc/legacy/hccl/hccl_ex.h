...
extern int
HcclIsend(void* buffer, int count, HcclDataType dataType, int dstRank, int tag, HcclComm comm, HcclRequest* request);
...
extern int
HcclTestSome(int count, HcclRequest requestArray[], int* compCount, int compIndices[], HcclStatus compStatus[]);
 ...
 * @param [in] conn    connection handle
 * @param [in] backLog    max peer can be queued in parallel when listening
...
extern HcclResult
HcclGetAicpuOpStreamAndNotify(HcclComm comm, rtStream_t* opstream, u8 aicpuNotifyNum, void** aicpuNotify);
