...
| HCCL_CHANNEL_CONFIG_TYPE_IS_SHARED_QUEUE | 0 | 是否启用共享队列模式（bool/int，默认0=false）。<br>设为true时，创建的多个Channel共享一个Jetty。<br>仅支持AIV引擎的UB网络语义协议（UB_CTP）。 | [HcclChannelConfigSetInt](../control_plane_api/comms_domain_resource_mgmt/HcclChannelConfigSetInt.md) |
