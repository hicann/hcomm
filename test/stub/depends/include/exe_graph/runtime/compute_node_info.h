...
public:
    AnchorInstanceInfo() { (void)memset_s(reserved_, sizeof(reserved_), 0, sizeof(reserved_)); };
     ...
     * @return 实例化个数
     */
    size_t GetInstanceNum() const { return instantiation_num_; }

    /**
     * 获取本输入/输出首个实例化的Anchor的index
     * @return 首个实例化的Anchor的index
     */
    size_t GetInstanceStart() const { return instance_start_; }

    /**
     * 设置本输入/输出首个实例化Anchor的index
     * @param instance_start 首个实例化Anchor的index
     */
    void SetInstanceStart(const uint32_t instance_start) { instance_start_ = instance_start; }

    /**
     * 设置本输入/输出实例化的个数
     * @param instantiation_num 实例化的个数
     */
    void SetInstantiationNum(const uint32_t instantiation_num) { instantiation_num_ = instantiation_num; }
...
public:
    CompileTimeTensorDesc() { (void)memset_s(reserved_, sizeof(reserved_), 0, sizeof(reserved_)); }
    /**
     * 获取DataType
     * @return DataType
     */
     ...
     * @return format
     */
    const StorageFormat& GetFormat() const { return storage_format_; }
    /**
     * 获取原始format
     * @return 原始format
     */
    ge::Format GetOriginFormat() const { return storage_format_.GetOriginFormat(); }
    /**
     * 获取运行时format
     * @return 运行时format
     */
    ge::Format GetStorageFormat() const { return storage_format_.GetStorageFormat(); }
    /**
     * 获取原始format向运行时format转换时的补维规则
     * @return 补维规则
     */
    ExpandDimsType GetExpandDimsType() const { return storage_format_.GetExpandDimsType(); }
    /**
     * 设置 data type
     ...
     */
    void SetStorageFormat(const ge::Format format) { storage_format_.SetStorageFormat(format); }
    /**
     * 设置原始format
     * @param format 原始format
     */
    void SetOriginFormat(const ge::Format format) { storage_format_.SetOriginFormat(format); }
    /**
...
public:
     ...
     * @return node type
     */
    const ge::char_t* GetNodeType() const { return node_type_; }
    /**
     * 获取计算节点的node name
     * @return node name
     */
    const ge::char_t* GetNodeName() const { return node_name_; }
    /**
     * 获取算子IR原型定义中的输入个数
     * @return IR原型中定义的输入个数
     */
    size_t GetIrInputsNum() const { return ir_inputs_num_; }
    /**
     * 获取算子IR原型定义中的输出个数
     * @return IR原型中定义的输出个数
     */
    size_t GetIrOutputsNum() const { return ir_outputs_num_; }
    /**
     * 获取计算节点的输入个数
     * @return 计算节点的输入个数
     */
    size_t GetInputsNum() const { return inputs_num_; }
    /**
     * 获取计算节点的输出个数
     * @return 计算节点的输出个数
     */
    size_t GetOutputsNum() const { return outputs_num_; }
    /**
    ...
    {
        return reinterpret_cast<const RuntimeAttrs*>(
            reinterpret_cast<const uint8_t*>(&place_holder) + sizeof(AnchorInstanceInfo) * ir_inputs_num_
     ...
     */
    void SetNodeType(const ge::char_t* node_type) { node_type_ = node_type; }
    /**
     * 设置计算节点的node name
     * @param node_name 计算节点的node name
     */
    void SetNodeName(const ge::char_t* node_name) { node_name_ = node_name; }
    /**
     * 根据IR原型中的输入index，获取对应的实例化信息
     * @param ir_index IR原型定义中的输入index
     * @return 输入的实例化信息
    ...
    CalcSize(const size_t ir_inputs_num, const size_t inputs_num, const size_t outputs_num, size_t& total_size);
    void Init(
        const size_t ir_inputs_num, const size_t inputs_num, const size_t outputs_num, const ge::char_t* node_name,
        const ge::char_t* node_type);
    static ge::graphStatus CalcSize(
        const size_t ir_inputs_num, const size_t ir_outputs_num, const size_t inputs_num, const size_t outputs_num,
        size_t& total_size);
    void Init(
        const size_t ir_inputs_num, const size_t ir_outputs_num, const size_t inputs_num, const size_t outputs_num,
