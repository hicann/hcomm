...
public:
     ...
     */
    StorageFormat(
        const ge::Format origin_format, const ge::Format storage_format, const ExpandDimsType& expand_dims_type)
     ...
     * @return 原始format
     */
    ge::Format GetOriginFormat() const { return origin_format_; }
    /**
     * 设置原始format
     * @param origin_format 原始format
     */
    void SetOriginFormat(const ge::Format origin_format) { origin_format_ = origin_format; }
    /**
     * 获取运行时format
     * @return 运行时format
     */
    ge::Format GetStorageFormat() const { return storage_format_; }
    /**
     * 设置运行时format
     * @param storage_format 运行时format
     */
    void SetStorageFormat(const ge::Format storage_format) { storage_format_ = storage_format; }
    /**
     * 获取补维规则
     * @return 补维规则
     */
    ExpandDimsType GetExpandDimsType() const { return expand_dims_type_; }
    /**
     * 设置补维规则
     * @param expand_dims_type 补维规则
     */
    void SetExpandDimsType(const ExpandDimsType& expand_dims_type) { expand_dims_type_ = expand_dims_type; }
    /**
     * 获取可写的补维规则
     * @return 补维规则引用
     */
    ExpandDimsType& MutableExpandDimsType() { return expand_dims_type_; }
    /**
     * 判断格式是否相等
     * @param other 另一个格式
     * @return true代表相等
     ...
     * @return true代表不相等
     */
    bool operator!=(const StorageFormat& other) const { return !(*this == other); }
