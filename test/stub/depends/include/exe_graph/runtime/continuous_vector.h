...
public:
     ...
     * @return 首个元素的指针地址
     */
    const void* GetData() const { return elements; }
    /**
     * 获取首个元素的指针地址，[GetData(), GetData() + GetSize()) 中的数据即为当前容器中保存的数据
     * @return 首个元素的指针地址
     */
    void* MutableData() { return elements; }
...
public:
     ...
     * @return 首个元素的指针地址
     */
    T* MutableData() { return static_cast<T*>(ContinuousVector::MutableData()); }
    /**
     * 获取首个元素的指针地址，[GetData(), GetData() + GetSize()) 中的数据即为当前容器中保存的数据
     * @return 首个元素的指针地址
     */
    const T* GetData() const { return static_cast<const T*>(ContinuousVector::GetData()); }
};
