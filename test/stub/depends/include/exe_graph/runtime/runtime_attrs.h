...
public:
     ...
     * @return 指向属性值的指针
     */
    const int64_t* GetInt(const size_t index) const { return GetAttrPointer<int64_t>(index); }
    /**
     * 获取list int类型属性
     * @param index 属性index
     * @return 指向属性值的指针
     ...
     * @return 指向属性值的指针
     */
    const char* GetStr(const size_t index) const { return GetAttrPointer<char>(index); }
    /**
     * 获取tensor类型属性
     * @param index 属性index
     * @return 指向属性值的指针
     */
    const Tensor* GetTensor(const size_t index) const { return GetAttrPointer<Tensor>(index); }
    /**
     * 获取float类型属性
     * @param index 属性index
     * @return 指向属性值的指针
     */
    const float* GetFloat(const size_t index) const { return GetAttrPointer<float>(index); }
    /**
     * 获取bool类型属性
     * @param index 属性index
     * @return 指向属性值的指针
     */
    const bool* GetBool(const size_t index) const { return GetAttrPointer<bool>(index); }
