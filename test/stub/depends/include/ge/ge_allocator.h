...
public:
    ...
    size_t GetSize() const { return block_size_; }
    void SetSize(const size_t mem_size) { block_size_ = mem_size; }
    void Free()
    {
        if (GetCount() > 0U) {
            if (SubCount() == 0U) {
    ...
    size_t AddCount() { return ++count_; }
    size_t SubCount() { return --count_; }
