...
namespace optiling {
struct CharPtrCmp {
    bool operator()(const char* strLeft, const char* strRight) const { return strcmp(strLeft, strRight) < 0; }
};
...
public:
    ...
    size_t GetStructSize(const char* structType) { return structSizeInfo.at(structType); }
...
#define BEGIN_TILING_DATA_DEF(class_name)                                                                             \
    ...
    public:                                                                                                           \
        ...
        size_t FieldHandler(                                                                                          \
            const char* dtype, const char* name, const char* structType, size_t structSize, void* ptr,                \
            const char* namePh)                                                                                       \
        {                                                                                                             \
...
public:                                                                                                               \
    ...
    {                                                                                                                 \
        ...
        const auto err_t                                                                                              \
            = memcpy_s(data_ptr_ + offset, data_size_ - offset, field_name, (arr_size) * sizeof(arr_type));           \
        if (err_t != EOK) {                                                                                           \
            GeLogError("tilingdata_base.h TILING_DATA_FIELD_DEF_ARR memcpy is failed !");                             \
        }                                                                                                             \
    }                                                                                                                 \
    arr_type* get_##field_name() { return (arr_type*)(data_ptr_ + field_name##_offset_); }                            \
                                                                                                                      \
private:                                                                                                              \
...
public:                                                                                                     \
    struct_type field_name{nullptr};                                                                        \
                                                                                                            \
private:                                                                                                    \
    size_t field_name##_offset_ = FieldHandler(                                                             \
        "struct", #field_name, #struct_type, StructSizeInfoBase::GetInstance().GetStructSize(#struct_type), \
        ...
        public:                                                                                                     \
            ...
            {                                                                                                       \
                CTilingDataClassFactory::GetInstance().RegisterTilingData(                                          \
                    #op_type, op_type##class_name##Helper::CreateTilingDataInstance);                               \
            }                                                                                                       \
