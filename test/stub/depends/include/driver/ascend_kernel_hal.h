 ...
 */
int hal_kernel_send_ipc_to_lp_async(
    unsigned int devid, unsigned char cmd_type0, unsigned char cmd_type1, unsigned char* data, unsigned int data_len);
 ...
 * @        -2 item not set
 */
int hal_kernel_get_user_config(
    unsigned int dev_id, const char* name, unsigned char* data, unsigned int in_len, unsigned int* out_len);
 ...
 * @return   0 Success, others for fail
 */
int hal_kernel_dms_get_pg_info(
    unsigned int dev_id, HAL_DMS_PG_INFO_TYPE info_type, char* data, unsigned int size, unsigned int* ret_size);
...
enum sensorhub_pps_source_type { PPS_FROM_XGMAC = 0x0, PPS_FROM_CHIP = 0x1 };

enum sensorhub_ssu_ctrl_type { SSU_SW = 0x0, PATH_SW = 0x1, SSU_BUT };
...
enum sensorhub_polarity_cfg_type { POLARITY_PPS_CONFIG = 0x0, POLARITY_CFG_BUT };
...
enum sensorhub_fault_check_type { EXPO_CHECK_PRE_THRESHOLD = 0x0, EXPO_CHECK_POST_THRESHOLD = 0x1 };
 ...
 */
int32_t
hal_kernel_qos_get_all_bandwidth(uint32_t devid, struct qos_bw_result_t* res, uint32_t in_size, uint32_t* out_size);
...
enum esched_cq_type { ESCHED_CQ_TYPE_PHASE, ESCHED_CQ_TYPE_PTR, ESCHED_CQ_TYPE_MAX };
...
enum esched_raw_data_type { RAW_DATA_TYPE_CQ, RAW_DATA_TYPE_MAX };
 ...
 * @return   other  fail
 */
int hal_kernel_prof_sample_register(
    unsigned int dev_id, unsigned int chan_id, struct prof_kernel_sample_register_para* para);
...
int hal_kernel_prof_sample_data_report(
    unsigned int dev_id, unsigned int chan_id, unsigned int sub_chan_id, struct prof_kernel_data_report_para* para);
 ...
 * @return 0: success, else: fail
 */
int devdrv_create_functional_sqcq(
    u32 devid, u32 tsid, struct devdrv_func_sqcq_alloc_para_in* para_in,
 ...
 * @return 0: success, else: fail
 */
typedef enum { ADDR_INDEPENDENT = 0, ADDR_UNIFIED, ADDR_MODE_MAX } HAL_KERNEL_ADDR_MODE;
...
enum txatu_user_module_type { TXATU_USER_MODULE_RTS = 0x0, TXATU_USER_MODULE_MAX };
...
int hal_kernel_agentdrv_update_tx_atu(
    u32 udevid, enum txatu_user_module_type type, u64 host_phy_addr, u64 host_addr_size);
/**
 * @ingroup driver
