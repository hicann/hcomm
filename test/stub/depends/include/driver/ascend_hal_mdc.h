...
halHdcRegisterMem(signed int devid, enum drvHdcMemType mem_type, void* va, unsigned int len, unsigned int flag);
/**
 * @ingroup driver
 * @brief unregister va
 * @attention null
 ...
 */
hdcError_t
halHdcWaitMemReleaseEx(HDC_SESSION session, struct drvHdcWaitMsgInput* input, struct drvHdcFastSendFinishMsg* msg);
...
halLidarDpEncode(struct lidar_encoder_matrix* encoder_matrix_info, unsigned int timeout, unsigned int* encoder_result);

struct adspc_solve_info {
    unsigned char solve_type; /* 0:SolveArrowHead, 1:AX=B */
    unsigned char
        send_to_peer; /* 发送到对端标志.0:不发送; 1:跨片使用求解器时，最后一帧求解结果通过stars串接发送到对片 */
...
struct adspc_solve_result {
    unsigned char
        solve_status; /* 求解任务完成状态:0 正常完成；其他 表示出现异常(包括ECC硬件异常、数据异常、强行终止) */
    unsigned char task_id;       /* 任务ID，配置范围0~255 */
