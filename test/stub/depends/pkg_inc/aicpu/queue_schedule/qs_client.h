...
public:
     ...
     */
    static BqsClient*
    GetInstance(const char_t* const serverProcName, const uint32_t procNameLen, const ExeceptionCallback fn);
     ...
     */
    uint32_t
    BindQueue(const std::vector<BQSBindQueueItem>& bindQueueVec, std::vector<BQSBindQueueResult>& bindResultVec) const;
     ...
     */
    uint32_t
    UnbindQueue(const std::vector<BQSQueryPara>& bqsQueryParaVec, std::vector<BQSBindQueueResult>& bindResultVec) const;
     ...
     * @return Number of get bind relation success
     */
    uint32_t GetPagedBindQueue(
        const uint32_t offset, const uint32_t limit, std::vector<BQSBindQueueItem>& bindQueueVec,
    ...
    uint32_t BindQueueMbufPool(
        const std::vector<BQSBindQueueMbufPoolItem>& bindQueueVec,
        std::vector<BQSBindQueueResult>& bindResultVec) const;

    uint32_t UnbindQueueMbufPool(
        const std::vector<BQSUnbindQueueMbufPoolItem>& bindQueueVec,
        std::vector<BQSBindQueueResult>& bindResultVec) const;
