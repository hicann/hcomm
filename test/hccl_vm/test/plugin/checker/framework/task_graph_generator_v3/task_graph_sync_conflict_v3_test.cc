 ...
 * Please refer to the License for details. This software is distributed on an "AS IS" BASIS,
...
protected:
    ...
    {
        return MakePosition(static_cast<RankId>(lane / 4U), static_cast<StreamId>(lane % 4U),
    ...
    static AivPipeEvent MakeEvent(int32_t eventId, RankId rankId = 0, uint64_t launchIdx = 1,
        uint32_t blockId = 2, uint32_t srcPipe = 3, uint32_t dstPipe = 4)
    {
        AivPipeEvent event;
        event.rankId = rankId;
    ...
    static AivFlagSync MakeFlag(uint64_t commInfoOffset, RankId flagOwnerRank = 0, uint64_t launchIdx = 1,
        int32_t value = 0)
    {
        AivFlagSync flag;
        flag.currentRank = 0;
...
TEST_F(SyncConflictTest, CcuOrderedReuseAcrossMultiplePairsIsAllowed)
{
    ...
    TaskRecordCCU *post1 = AddNode(std::make_unique<TaskRecordCCU>(
        CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU), lane);
    TaskWaitCCU *wait1 = AddNode(std::make_unique<TaskWaitCCU>(
        CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU), lane);
    TaskRecordCCU *post2 = AddNode(std::make_unique<TaskRecordCCU>(
        CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU), lane);
    TaskWaitCCU *wait2 = AddNode(std::make_unique<TaskWaitCCU>(
        CcuNotify{INVALID_CHANNEL_ID, 0, 0, 0, 1, 1}, ProtocolType::CCU), lane);
    AddEdge(start, post1);
    AddEdge(post1, wait1);
    AddEdge(wait1, post2);
...
TEST_F(SyncConflictTest, AivEventFieldsCreateIndependentBuckets)
{
    const std::vector<AivPipeEvent> resources{
        MakeEvent(1, 0, 1, 2, 3, 4),
        MakeEvent(1, 1, 1, 2, 3, 4),
        MakeEvent(1, 0, 2, 2, 3, 4),
        MakeEvent(1, 0, 1, 3, 3, 4),
        MakeEvent(1, 0, 1, 2, 4, 4),
        MakeEvent(1, 0, 1, 2, 3, 5),
