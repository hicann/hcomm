...
void TransMemExecutor::Run()
{
    ...
    if (rmtRankId < 0 || rmtDieId < 0) {
        HCCL_VM_ERROR("RemoteCCU not exist. rankId:[{}], dieId:[{}], xcValue:[{}]",
    ...
    if (dmaOpCode_ == 3 || dmaOpCode_ == 5) {
        ...
        if (msIdmode_ == 1) {
            ...
            ccuResMgr.TransMemToMem(reinterpret_cast<void *>(srcAddr), reinterpret_cast<void *>(dstAddr), length, udfEnable_, reduceOpCode_, reduceDataType_);
        }
...
{
    return HcclSim::StringFormat("[TransMemExecutor] xdId[%u] xdtId[%u] xsId[%u] xstId[%u] "
        ...
        udfType_, reduceDataType_, reduceOpCode_, dmaOpCode_,
        order_, fence_, cqe_, nf_, udfEnable_, splitMode_,
        se_, rmtJettyType_, srcMode_, dstMode_, msIdmode_,
        targetHint_, setCKEId_, setCKEMask_);
}
