...
{
    return HcclSim::StringFormat("[SyncXnWtExecutor] xdId[%u] xdtId[%u] xsId[%u] xcId[%u] "
                                  "xnId[%u] xntId[%u] value[0x%08x] notifyValid[%u] parMode[%u] "
                                  "setCKEId[%u] setCKEMask[0x%04x]\n",
        xdId_, xdtId_, xsId_, xcId_, xnId_, xntId_, value_, notifyValid_, parMode_,
        setCKEId_, setCKEMask_);
}
