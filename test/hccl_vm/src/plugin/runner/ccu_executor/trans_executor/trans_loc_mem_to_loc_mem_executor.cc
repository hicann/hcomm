...
void TransLocMemToLocMemExecutor::RunV2()
{
    ...
    HCCL_VM_DEBUG("Trans data from srcMemAddr[0x{:x}] to dstMemAddr[0x{:x}], length[{}].",
        srcLocMemAddr, dstLocMemAddr, len);

    ccuResMgr.TransMemToMem(reinterpret_cast<void *>(srcLocMemAddr), reinterpret_cast<void *>(dstLocMemAddr), len, false, 0, 0);
...
std::string TransLocMemToLocMemExecutor::Describe()
{
    if (version_ == RunnerCcuVersion::CCU_V1) {
        ...
        return HcclSim::StringFormat("[TransLocMemToLocMemExecutor] xdId[%u] xdtId[%u] xsId[%u] xstId[%u] "
