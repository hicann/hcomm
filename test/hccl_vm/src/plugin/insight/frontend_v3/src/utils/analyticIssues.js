...
import { resolveMemoryAddressDisplay } from './msMemory.js'
  ...
  return [
    ...
    .map(([label, slice]) => {
      const addressField = resolveMemoryAddressDisplay({
        bufferType: typeof slice.buffer_type === 'string' ? slice.buffer_type : '',
        bufferId: Number.isInteger(slice.buffer_type) ? slice.buffer_type : null,
        offset: slice.offset,
        msId: slice.ms_id ?? slice.msId,
      })
      return {
        label,
        fields: [
          ['buffer_type', formatValue(slice.buffer_type)],
          [addressField.label, addressField.value],
