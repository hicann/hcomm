...
function parseGraphGroupData(rawGroup = {}) {
  ...
  return {
    ...
    isCheckerV3: Boolean(group.is_checker_v3),
  }
}
...
function parseGraphStageStatsData(rawStats = {}) {
  ...
  const inputGraph = parseGraphStageStatData(stats.input_graph)
  const v3FinalGraph = parseGraphStageStatData(stats.v3_final_graph)

  return {
    inputGraph: inputGraph ?? v3FinalGraph,
