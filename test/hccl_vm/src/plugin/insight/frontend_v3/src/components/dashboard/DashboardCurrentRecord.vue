    ...
    <template v-if="selectedDataset">
      <div class="dashboard-record-card">
        ...
        <div class="dashboard-tag-row">
          ...
          <el-tag type="info">算子 {{ summaryRows.find(([label]) => label === '算子')?.[1] ?? '--' }}</el-tag>
          <el-tag type="primary">
            已选 Rank {{ selectedRankCount }}<template v-if="totalRankCount > 0"> / {{ totalRankCount }}</template>
          </el-tag>
      ...
      <section class="dashboard-card-group">
        ...
        <div class="dashboard-file-grid">
          ...
          <div class="dashboard-file-card">
            <strong>{{ fileStats.snapshots }}</strong>
            <span>快照文件</span>
          </div>
          <div class="dashboard-file-card">
            <strong>{{ fileStats.validation }}</strong>
            <span>校验文件</span>
          </div>
        </div>
      </section>
