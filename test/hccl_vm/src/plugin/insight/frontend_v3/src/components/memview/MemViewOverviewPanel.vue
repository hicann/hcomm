...
const visibleBufferSlots = computed(() => {
  ...
  if (!selectedRankIds.value.length) return []
  ...
  if (presenceMap instanceof Map) {
    for (const rankId of selectedRankIds.value) {
...
watch(
  () => [props.datasetName, props.enabled, selectedRankIds.value.join(',')],
  async () => {
    await loadManifests()
  },
  { immediate: true },
...
async function loadManifests() {
  ...
  if (!props.enabled || !datasetName || !rankIds.length) {
    rankBundles.value = []
