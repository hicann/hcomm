...
struct CcuQueueGraph {
    std::vector<u32> entryQueueIds; // 保持 ccuNode->children 原顺序去重后的入口 queue
    std::unordered_map<u32, TaskNode*> queueId2Head;
    std::unordered_map<u32, std::vector<u32>> queueAdj;
    std::unordered_map<TaskNode*, uint8_t> nodeIsQueueHead;
};
...
private:
    ...
    void GenFragQueueInOneQueueOnly(TaskNode* head);                                         // for ccu memcheck
    HcclResult CollectCcuQueueGraph(TaskNode* ccuNode, RankId rankId, CcuQueueGraph& graph); // for ccu memcheck
