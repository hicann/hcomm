...
HcclResult CheckLoopGroupNotSupport(LoopGroupParamA6* loopGroupParam)
{
    if (loopGroupParam != nullptr) {
        HCCL_VM_ERROR("{} This A6 instruction cannot be used inside a loop expansion.",
...
// 将立即数 写到第n个X寄存器中
HcclResult TransformLoadImdToXInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
...
HcclResult CheckXnValid(uint16_t xnId, uint16_t xnIdMin, uint16_t xnIdMax) {
    if (xnId < xnIdMin || xnId > xnIdMax) {
        HCCL_VM_ERROR("{} Xn register id is out of the valid range, xnId={}, validMin={}, validMax={}",
...
HcclResult TransformLoadXInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    ...
    if (mode == 0) {
        ...
        HCCL_VM_DEBUG("LoadX Xn[{}]({}) + immediate[{}] -> Xn[{}]({}), + immediate[{}] -> Xn[{}]({})",
        ...
        HCCL_VM_DEBUG("LoadX Xn[{}]({}) + Xn[{}]({}) -> Xn[{}]({}), + Xn[{}]({}) -> Xn[{}]({})",
            xsId, xsValue, xsSoId, xsSoValue, xsTmpId, xsTmpValue, xsDoId, xsDoValue, xdId, xdValue);
    } else {
        HCCL_VM_ERROR("{} This instruction mode is not supported by CheckerV3 graph expansion, "
            "instruction=LoadX, mode={}", MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(), mode);
        return HCCL_E_PARA;
    }
...
// StoreX指令
HcclResult TransformStoreXInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    uint8_t mode = instr->v2.loadStoreX.oMode;
    ...
    if (mode == 0) {
        ...
        HCCL_VM_DEBUG("StoreX Xn[{}]({}) + immediate[{}] -> Xn[{}], Xn[{}]({}) + immediate[{}] -> Xn[{}]({})",
        ...
        HCCL_VM_DEBUG("StoreX Xn[{}]({}) + Xn[{}]({}) -> Xn[{}], Xn[{}]({}) + Xn[{}]({}) -> Xn[{}]({})",
            xdId, xdValue, xsDoId, xsDoValue, newId, xsId, xsValue, xsSoId, xsSoValue, newId, newValue);
    } else {
        HCCL_VM_ERROR("{} This instruction mode is not supported by CheckerV3 graph expansion, "
            "instruction=StoreX, mode={}", MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(), mode);
        return HCCL_E_PARA;
    }
...
HcclResult TransformClearXInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    // 清除范围为Xn~Xm  Xn的有效值？
...
// Nop指令，空指令，用于排指令流水
HcclResult TransformNopInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    (void) instr;
    (void) curCcuTask;
...
HcclResult TransformLoadInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId{0};
    ...
    if (dstType != 0) {
        // 目前只支持Xn寄存器
        HCCL_VM_ERROR("{} Load only supports writing into Xn registers, actualDstType={}",
    ...
    if ((xsStartAddr & 0x7) != 0) {
        HCCL_VM_ERROR("{} Load source address must be 8-byte aligned, sourceAddress={}",
    ...
    if ((length & 0x7) != 0) {
        HCCL_VM_ERROR("{} Load length must be a multiple of 8 bytes, length={}",
    ...
    if (AllRankParamRecorder::Global()->GetHBM(rankId, dieId, xsStartAddr, entry) != HCCL_SUCCESS) {
        ...
        HCCL_VM_ERROR("{} Failed to read HBM content before it was initialized, rankId={}, dieId={}, "
            "instrId={}, hbmAddr={}", MakeErrorCodeText(ErrorCode::GRAPH_REGISTER_UNINITIALIZED).c_str(),
            static_cast<uint32_t>(rankId), dieId, instrId, xsStartAddr);
        return HCCL_E_PARA;
    }
    if (length/8 > entry.size()) {
        HCCL_VM_ERROR("{} Load length exceeds the number of 64-bit entries available in HBM, "
...
HcclResult TransformStoreInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    (void) isContinue;
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId{0};
    ...
    if (srcType != 0) {
        // 目前只支持Xn寄存器
        HCCL_VM_ERROR("{} Store only supports reading from Xn registers, actualSrcType={}",
    ...
    CHK_GET_XN_V3(curCcuTask, queId, xlId, length);
    if ((length & 0x7) != 0) {
        HCCL_VM_ERROR("{} Store length must be a multiple of 8 bytes, length={}",
            MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(), length);
        return HCCL_E_PARA;
    }
    std::vector<uint64_t> entry;
    ...
    if ((entry.size() % 8) != 0) {
        const uint32_t instrId = curCcuTask->microCodePosInQue[queId] + curCcuTask->startInstrIdInQue[queId];
        HCCL_VM_ERROR("{} Store data length must be a multiple of 64 bytes before writing HBM, "
            "rankId={}, dieId={}, queueId={}, instrId={}, hbmAddr={}, dataEntryCount={}, dataLengthBytes={}",
            MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(), static_cast<uint32_t>(rankId), dieId, queId,
            instrId, xdValue, entry.size(), entry.size() * sizeof(uint64_t));
        return HCCL_E_PARA;
    }
...
HcclResult TransformAddInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    if (parMode == 0) {
        ...
        HCCL_VM_DEBUG("Xn[{}]({}) + immediate[{}] -> Xn[{}]({})", xnId, xnValue, instr->v2.operate.xmId, xdId,
            xdValue);
    } else {
        // Xd = Xn + Xm
...
// Sub指令，实现算数“-”
HcclResult TransformSubInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
...
// Mul指令，实现算数“*”
HcclResult TransformMulInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t parMode = instr->v2.operate.parMode;
    ...
    if (parMode == 0) {
        ...
        HCCL_VM_DEBUG("low32(Xn[{}]({})) * immediate[{}] -> Xn[{}]({})",
            xnId, xnValue & 0xFFFFFFFF, instr->v2.operate.xmId, xdId, xdValue);
    } else {
        uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
        uint64_t xmValue = 0;
        CHK_GET_XN_V3(curCcuTask, queId, xmId, xmValue);
        ...
        HCCL_VM_DEBUG("low32(Xn[{}]({})) * low32(Xn[{}]({})) -> Xn[{}]({})",
            xnId, xnValue & 0xFFFFFFFF, xmId, xmValue & 0xFFFFFFFF, xdId, xdValue);
    }
...
// And指令，实现逻辑“&”
HcclResult TransformANDInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
...
// Or指令，实现逻辑“|”
HcclResult TransformORInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
...
// Not指令，实现逻辑“~”
HcclResult TransformNOTInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t ckeId = UpdateCKEId(instr->v2.operate.setCKEId, loopGroupParam);
...
// Xor指令，实现逻辑“^”
HcclResult TransformXORInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
...
// SHL指令，实现逻辑“<<”
HcclResult TransformSHLInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
    ...
    switch (shiftType) {
        case 0: // 逻辑移位
            xdValue = xnValue << xmValue;
            HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [logical] -> Xn[{}]({})", xnId, xnValue, xmId, xmValue, xdId,
                xdValue);
            break;
        case 1: // 算术移位
            xdValue = static_cast<int64_t>(xnValue) << xmValue;
            HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [arithmetic] -> Xn[{}]({})", xnId, xnValue, xmId, xmValue,
                xdId, xdValue);
            break;
        case 2: // 循环移位
            xdValue = (xnValue << (xmValue & 0x3F)) | (xnValue >> (64 - (xmValue & 0x3F)));
            HCCL_VM_DEBUG("Xn[{}]({}) << Xn[{}]({}) [rotate] -> Xn[{}]({})", xnId, xnValue, xmId, xmValue, xdId,
                xdValue);
            break;
        default:
...
// SHR指令，实现逻辑“>>”
HcclResult TransformSHRInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t xnId = GetXnId(instr->v2.operate.xnId, loopGroupParam);
    uint16_t xdId = GetXnId(instr->v2.operate.xdId, loopGroupParam);
    uint16_t xmId = GetXnId(instr->v2.operate.xmId, loopGroupParam);
...
HcclResult TransformPopcntInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId{0};
    curCcuTask->GetDieId(queId, dieId);
...
static void SetLoopParamAtIteration(const LoopGroupParamA6 &sampleParam, u32 curLoopCnt, LoopGroupParamA6 &iterParam)
{
    iterParam = sampleParam;
    iterParam.curLoopCnt = curLoopCnt;
}

static HcclResult CollectLoopCkeOpsA6(const CcuRep::CcuInstr *instr, const LoopGroupParamA6 &sampleParam,
    ...
    switch (instr->header.type) {
        case CTRL_TYPE:
            if (instr->header.code == A6_CLEARCKE_CODE) {
                waitOp = {UpdateCKEId(instr->v2.clearCKE.waitCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam)), instr->v2.clearCKE.waitCKEMask};
                return HCCL_SUCCESS;
            }
        ...
        case TRANS_TYPE:
            if (instr->header.code == A6_TRANSLOCMEMTOLOCMS_CODE) {
                setOp = {UpdateCKEId(instr->v2.transLocMemToLocMS.setCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam)), instr->v2.transLocMemToLocMS.setCKEMask};
                return HCCL_SUCCESS;
            }
            if (instr->header.code == A6_TRANSLOCMSTOLOCMEM_CODE) {
                setOp = {UpdateCKEId(instr->v2.transLocMSToLocMem.setCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam)), instr->v2.transLocMSToLocMem.setCKEMask};
                return HCCL_SUCCESS;
            }
            if (instr->header.code == A6_TRANSLOCMSTOLOCMS_CODE) {
                setOp = {UpdateCKEId(instr->v2.transLocMSToLocMS.setCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam)), instr->v2.transLocMSToLocMS.setCKEMask};
                return HCCL_SUCCESS;
            }
            if (instr->header.code == A6_TRANSLOCMEMTOLOCMEM_CODE) {
                setOp = {UpdateCKEId(instr->v2.transLocMemToLocMem.setCKEId,
                    const_cast<LoopGroupParamA6 *>(&sampleParam)), instr->v2.transLocMemToLocMem.setCKEMask};
                return HCCL_SUCCESS;
            }
            if (instr->header.code == A6_TRANSMEM_CODE) {
                ...
                setOp = {UpdateCKEId(instr->v2.transMem.setCKEId, const_cast<LoopGroupParamA6 *>(&sampleParam)),
        ...
        case REDUCE_TYPE:
            if (instr->header.code == A6_REDUCEADD_CODE || instr->header.code == A6_REDUCEMAX_CODE ||
                ...
                setOp = {UpdateCKEId(instr->v2.reduce.setCKEId, const_cast<LoopGroupParamA6 *>(&sampleParam)),
...
static HcclResult CollectTransMemGenericLoopInstrA6(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask,
    uint32_t queId, uint16_t instrId, const LoopGroupParamA6 &sampleParam, uint64_t loopCnt,
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop)
{
    ...
    if (reduceEn) {
        ...
        CHK_RET(GetHcclDataTypeFromCCUDataType(op.reduceDataType, ccuReduceTypeMap[op.reduceOpCode], hcclDataType));
...
static HcclResult CollectTransLoopInstrA6(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParamA6 &sampleParam, uint64_t loopCnt,
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop)
{
    if (instr->header.code == A6_TRANSMEM_CODE) {
        return CollectTransMemGenericLoopInstrA6(instr, curCcuTask, queId, instrId, sampleParam, loopCnt, instrInLoop);
    }
...
static HcclResult CollectReduceLoopInstrA6(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParamA6 &sampleParam, uint64_t loopCnt,
    std::shared_ptr<CcuLoopInstrV3> &instrInLoop)
{
    RankId rankId = curCcuTask->GetRankId();
...
static HcclResult CollectClearCkeLoopInstrA6(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    uint16_t instrId, const LoopGroupParamA6 &sampleParam, std::shared_ptr<CcuLoopInstrV3> &instrInLoop)
{
    ...
    clearInstr->clearCKEId = UpdateCKEId(instr->v2.clearCKE.clearCKEId,
...
static HcclResult CollectMergedLoopInstrsA6(CcuGraphStateV3 *curCcuTask, uint32_t queId, uint16_t startInstrId,
    uint16_t endInstrId, const LoopGroupParamA6 &sampleParam, uint64_t loopCnt, CcuLoopInstrsV3 &mergedInstrs)
{
    ...
    for (uint16_t insId = startInstrId; insId <= endInstrId; ++insId) {
        ...
        if (curInstr->header.type == CTRL_TYPE) {
            ...
            ret = CollectTransLoopInstrA6(curInstr, curCcuTask, queId, insId, sampleParam, loopCnt, instrInLoop);
        } else if (curInstr->header.type == REDUCE_TYPE) {
            ret = CollectReduceLoopInstrA6(curInstr, curCcuTask, queId, insId, sampleParam, loopCnt, instrInLoop);
        } else {
...
static HcclResult CollectLoopExpandA6(const CcuRep::CcuInstr *loopInstr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    const LoopGroupParamA6 &loopParam, CcuLoopInstrsV3 &mergedInstrs, uint64_t &loopCnt)
{
    if (loopInstr->header.type != CTRL_TYPE || loopInstr->header.code != A6_LOOP_CODE || loopInstr->v2.loop.mode == 1) {
        return HCCL_E_NOT_SUPPORT;
    }
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId = INVALID_DIE_ID;
    ...
    return CollectMergedLoopInstrsA6(curCcuTask, queId, loopInstr->v2.loop.startInstrId,
        loopInstr->v2.loop.endInstrId, sampleParam, loopCnt, mergedInstrs);
}

static HcclResult RecordMergedLoopParamA6(const CcuRep::CcuInstr *loopInstr, CcuGraphStateV3 *curCcuTask,
    uint32_t queId, LoopGroupParamA6 &loopParam)
{
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId = INVALID_DIE_ID;
    curCcuTask->GetDieId(queId, dieId);
...
static HcclResult TryProcessMergedLoopA6(const CcuRep::CcuInstr *loopInstr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool &isContinue, LoopGroupParamA6 &loopParam, uint32_t loopGroupIdx, uint32_t expandTimes, bool &merged)
{
    merged = false;
    ...
    HCCL_VM_INFO("Merged LoopGroup body successfully, loopInstructionId={}, loopCount={}, "
        "expandTimes={}, mergedInstructionCount={}", loopInstr->v2.loop.startInstrId, mergedLoopCnt, expandTimes,
...
HcclResult TransformLoopGroupInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    if (loopInsCnt > 512 || expandCnt > 511 || expandOffset > 511 || expandOffset > loopInsCnt) {
        HCCL_VM_ERROR("{} LoopGroup configuration is invalid, loopGroupXnValue={}, loopCount={}, "
    ...
    HCCL_VM_INFO("Parsed LoopGroup configuration, loopCount={}, expandOffset={}, expandCount={}",
    ...
    for (u32 curLoopIdx = 0; curLoopIdx < loopCnt; curLoopIdx++) {
        ...
        bool merged = false;
        CHK_RET(TryProcessMergedLoopA6(&microCodeQue.instrVec[insPos], curCcuTask, queId, isContinue,
    ...
    HCCL_VM_INFO("Finished LoopGroup expansion, startLoopInstructionId={}, loopGroupXnId={}, "
        "loopGroupXmId={}", startLoopInstrId, xnId, xmId);
    return HCCL_SUCCESS;
}

// SetCKBit指令
HcclResult TransformSetCKBitInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
...
HcclResult TransformJumpInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    ...
    switch (conditionType) {
        case 0: // 等于
            ...
            HCCL_VM_DEBUG("Jump if Xn[{}]({}) == Xn[{}]({}), jumpMode[{}], targetXn[{}]({})",
                conditionXnId, conditionValue, expectedXnId, expectedValue, jumpMode, relTarInstrXnId,
                relTarInstrValue);
            break;
        case 1: // 不等于
            updateNextInsIdx(conditionValue != expectedValue);
...
// Wait指令
HcclResult TransformWaitInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
...
// Fence指令
HcclResult TransformFenceInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    HCCL_VM_ERROR("{} Fence instruction is not supported by CheckerV3 graph expansion",
        MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str());
    return HCCL_E_PARA;
}

// TransLocMemToLocMS指令：将内存中的数据搬移到本端MS
HcclResult TransformTransLocMemToLocMSInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    ...
    if (len == 0) {
        ...
        HCCL_VM_ERROR("{} Transfer size is 0, instruction=TransLocMemToLocMS, rankId={}, dieId={}, "
            "queueId={}, instrId={}, transferSize={}", MakeErrorCodeText(ErrorCode::GRAPH_STRUCTURE_INVALID).c_str(),
            static_cast<uint32_t>(rankId), dieId, queId, instrId, len);
        return HCCL_E_INTERNAL;
    }
...
HcclResult TransformTransLocMSToLocMemInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
...
HcclResult TransformTransLocMSToLocMSInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
...
HcclResult TransformTransLocMemToLocMemInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
...
HcclResult TransformTransMemInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    // 参数校验
    uint16_t udfType = instr->v2.transMem.udfType;
    if (udfType != 0) {
        HCCL_VM_ERROR("{} This TransMem udfType is not supported by CheckerV3 graph expansion, "
            "udfType={}", MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(), udfType);
        return HCCL_E_PARA;
    }
    ...
    if(!IsExistRemoteDieInfo(rankId, dieId, xcValue)) {
        HCCL_VM_ERROR("{} Cannot find remote rank/die mapping for this channel, instruction=TransMem, "
    ...
    // 读数据 src为对端 dst为本端
        ...
        HCCL_VM_ERROR("{} This TransMem dmaOpCode is not supported by CheckerV3 graph expansion, "
            "dmaOpCode={}", MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str(), dmaOpCode);
        return HCCL_E_PARA;
    }
...
HcclResult TransformSyncXnWtxInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    RankId rankId = curCcuTask->GetRankId();
    uint32_t dieId {0};
    curCcuTask->GetDieId(queId, dieId);
    ...
    CHK_GET_XN_V3(curCcuTask, queId, xcId, xcValue);
    if(!IsExistRemoteDieInfo(rankId, dieId, xcValue)) {
        HCCL_VM_ERROR("{} Cannot find remote rank/die mapping for this channel, instruction=SyncXnWtx, "
            "localRankId={}, localDieId={}, channelValue={}",
            MakeErrorCodeText(ErrorCode::GRAPH_RESOURCE_NOT_FOUND).c_str(), static_cast<uint32_t>(rankId), dieId,
            static_cast<unsigned long long>(xcValue));
        return HCCL_E_PARA;
    ...
    if (ccuType == CcuComponerntType::XN_A6) {
        ...
        HCCL_VM_ERROR("{} SyncXnWtx destination address does not map to a supported remote target; "
...
HcclResult TransformSyncAtxInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    HCCL_VM_ERROR("{} SyncAtx instruction is not supported by CheckerV3 graph expansion",
        MakeErrorCodeText(ErrorCode::GRAPH_UNSUPPORTED).c_str());
    return HCCL_E_PARA;
}

// ReduceAdd指令
HcclResult TransformReduceAddInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    ...
    HCCL_VM_INFO("Completed local SUM reduce, msList={}, sourceCount={}, dataType={}, "
...
HcclResult TransformReduceMaxInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t count       = instr->v2.reduce.count;
    uint16_t dataType    = instr->v2.reduce.dataType;
    uint16_t setCKEId    = UpdateCKEId(instr->v2.reduce.setCKEId, loopGroupParam);
    ...
    HCCL_VM_INFO("Completed local MAX reduce, msList={}, sourceCount={}, dataType={}, "
...
// ReduceMin指令
HcclResult TransformReduceMinInstr(const CcuRep::CcuInstr *instr, CcuGraphStateV3 *curCcuTask, uint32_t queId,
    bool& isContinue, LoopGroupParamA6* loopGroupParam) {
    uint16_t count       = instr->v2.reduce.count;
    uint16_t dataType    = instr->v2.reduce.dataType;
    uint16_t setCKEId    = UpdateCKEId(instr->v2.reduce.setCKEId, loopGroupParam);
    ...
    HCCL_VM_INFO("Completed local MIN reduce, msList={}, sourceCount={}, dataType={}, "
        "setCkeId={}, setMask=0x{:04x}",
    ...
    uint32_t queId, bool& isContinue, LoopGroupParamA6* loopGroupParam, uint32_t loopGroupIdx)
{
    ...
    auto loopStart = AddLoopStartTask(queId, loopIdx, loopGroupIdx, curCcuTask,
        startInstrId, endInstrId, static_cast<uint64_t>(loopIterCnt), 1ULL); // loop前新增loopStart标记节点
    ...
    for (u32 curLoopCnt = 0; curLoopCnt < loopIterCnt; curLoopCnt++) {
        ...
        HCCL_VM_INFO("Running loop iteration, loopCount={}, currentIteration={}",
        ...
        for (uint16_t insId = startInstrId; insId <= endInstrId; insId++) {
            ...
            HCCL_VM_DEBUG("Expanding loop instruction, rankId={}, queueId={}, instructionId={}",
                curCcuTask->GetRankId(), queId, insId);
            if (transformA6InstrSqeMap.count(instrVec->header.header) == 0) {
                HCCL_VM_ERROR("{} This A6 CCU instruction type is not supported during loop "
    ...
    HCCL_VM_DEBUG("Finished loop expansion, startInstructionId={}, endInstructionId={}, "
        "loopCountRegisterId={}", startInstrId, endInstrId, xnId);
    return HCCL_SUCCESS;
}
...
HcclResult InstructMapA6::Transform(const CcuRep::CcuInstr* instr, CcuGraphStateV3 * curCcuTask, uint32_t queId,
        bool& isContinue, void* loopParam) {
    auto it = transformInstrSqeMap.find(instr->header.header);
    if (it == transformInstrSqeMap.end()) {
        HCCL_VM_ERROR("{} This A6 CCU instruction type is not supported by CheckerV3 graph "
