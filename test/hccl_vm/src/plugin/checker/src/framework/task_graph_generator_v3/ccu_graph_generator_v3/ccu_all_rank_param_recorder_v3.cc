...
HcclResult AllRankParamRecorder::GetMSIdByAddr(uint32_t dieId,  uint64_t addr, uint16_t& msId) {
    if (dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId = ccu_resource_base_addr_.empty() ? "null" :
            std::to_string(static_cast<uint32_t>(ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting address to MS id, dieId={}, "
            "maxDieId={}",
    ...
    if (ccuIndex == UINT64_MAX) {
        HCCL_VM_ERROR("{} Address does not fall into any known MS address range, localMsAddr={}, "
            "rawAddr={}", MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(), msAddr, addr);
        return HCCL_E_PARA;
    }
...
HcclResult AllRankParamRecorder::GetXnAndTypeIdByAddr(uint32_t dieId,  uint64_t xnAddr, CcuComponerntType& type, uint16_t& xnId) {
    if (dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId = ccu_resource_base_addr_.empty() ? "null" :
            std::to_string(static_cast<uint32_t>(ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting address to register id, dieId={}, "
            "maxDieId={}",
            MakeErrorCodeText(ErrorCode::GRAPH_OUT_OF_RANGE).c_str(), dieId, maxDieId);
        return HCCL_E_PARA;
    }
    ...
    if (type == CcuComponerntType::UNKNOWN) {
        HCCL_VM_ERROR("{} Address does not belong to any known CCU component range, addr={}, "
            "dieId={}, dieBaseAddr={}", MakeErrorCodeText(ErrorCode::GRAPH_ADDRESS_INVALID).c_str(), xnAddr,
...
HcclResult AllRankParamRecorder::GetAddrByXnId(uint32_t dieId, CcuComponerntType type, uint16_t xnId, uint64_t& xnAddr) {
    ...
    if(dieId >= ccu_resource_base_addr_.size()) {
        const std::string maxDieId = ccu_resource_base_addr_.empty() ? "null" :
            std::to_string(static_cast<uint32_t>(ccu_resource_base_addr_.size() - 1));
        HCCL_VM_ERROR("{} dieId is out of range when converting register id to address, dieId={}, "
            "maxDieId={}",
            MakeErrorCodeText(ErrorCode::GRAPH_OUT_OF_RANGE).c_str(), dieId, maxDieId);
        return HCCL_E_PARA;
    }
