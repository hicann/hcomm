...
HcclResult DecodeOpDetails(const sim::OpDetailTab &detailTab, OpDetails &details)
{
    if (detailTab.opDetail.size() < sizeof(OpDetails)) {
        HCCL_VM_ERROR("Operator detail payload is too small, rankId={}, actualSize={}, expectedSize={}",
...
HcclResult AppendTranslatedOperator(TaskGraphGeneratorV3::TaskMetaTranslatorV3 &translator,
    std::vector<std::unique_ptr<V3Node>> &nodes, TaskGraphGeneratorV3::AllRankNodeQueues &queues)
{
...
HcclResult BigGraphCheckerV3::TranslateTask()
{
    ...
    for (const OpParam &opParam : data_.operators) {
        for (const OperatorRankData &rankData : opParam.ranks) {
            HcclResult ret = storage_.LoadHcclVmSynthesisData(rankData.rankId, rankData.op.memInfo,
                data_.channels);
            if (ret != HCCL_SUCCESS) {
                return ret;
            }
    ...
    HCCL_VM_INFO("Translated big graph tasks, operatorCount={}, nodeCount={}, rankCount={}",
...
HcclResult BigGraphCheckerV3::GenerateBigGraph()
{
    ...
    HCCL_VM_INFO("Generated big graph, nodeCount={}, rankCount={}", graph_->GetNodes().size(),
        graph_->GetTaskQueues().size());
    return HCCL_SUCCESS;
}

HcclResult BigGraphCheckerV3::SingleTaskCheck()
{
...
HcclResult BigGraphCheckerV3::SyncCheck()
{
    if (graph_ == nullptr || graph_->GetMainStartNode() == nullptr) {
        HCCL_VM_ERROR("{} Cannot run big graph sync-conflict check before the graph is generated",
    ...
    HCCL_VM_INFO("Big graph sync-conflict check finished, status={}, originalNodeCount={}, copiedNodeCount={}, "
...
HcclResult BigGraphCheckerV3::MemConflictCheck()
{
    return HCCL_E_NOT_SUPPORT;
}

HcclResult BigGraphCheckerV3::SemanticCheck()
{
    return HCCL_E_NOT_SUPPORT;
}
