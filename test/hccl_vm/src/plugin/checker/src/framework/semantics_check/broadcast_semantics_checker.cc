...
namespace HcclSim {
HcclResult
TaskCheckBroadcastSemantics(std::map<RankId, RankMemorySemantics>& allRankMemSemantics, u64 dataSize, RankId root)
{
    ...
    for (RankId rankId = 0; rankId < rankSize; rankId++) {
        ...
        if (allRankMemSemantics.count(rankId) == 0) {
            HCCL_VM_ERROR(
                "{} Broadcast produced no result data for rank {}, but this rank is "
                "expected to receive the full root input from rank {} with expected size 0x{:x}.",
        ...
        for (auto& ele : allRankMemSemantics[rankId][BufferType::INPUT]) {
            ...
            if (ele.startAddr != totalSize) {
                HCCL_VM_ERROR(
                    "{} Broadcast output for rank {} should continue at 0x{:x}, "
            ...
            if (ele.srcBufs.size() != 1) {
                HCCL_VM_ERROR(
                    "{} Broadcast output range [0x{:x},0x{:x}) for rank {} should "
            ...
            if (srcBuf.rankId != root) {
                HCCL_VM_ERROR(
                    "{} Broadcast output range [0x{:x},0x{:x}) for rank {} should come "
        ...
        if (totalSize != dataSize) {
            HCCL_VM_ERROR(
                "{} Broadcast output for rank {} ends too early. The checker has "
