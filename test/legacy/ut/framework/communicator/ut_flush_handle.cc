...
TEST_F(FlushHandleTest, Init_WhenAllocateLocalMemoryFails_Expect_MemoryError)
{
    ...
    MOCKER_CPP(&RdmaHandleManager::GetByAddr)
        ...
        .with(mockcpp::any(), mockcpp::any(), mockcpp::any(), mockcpp::any())
        .will(returnValue(fakeRdmaHandle));
    // lbMax == 0 → flushOpcodeSupport_ = false → AllocateLocalMemory 走 malloc 分支
    int lbMax = 0;
    MOCKER(RaGetLbMax)
        .stubs()
        .with(mockcpp::any(), outBoundP(&lbMax, sizeof(lbMax)))
...
TEST_F(FlushHandleTest, Init_WhenHrtMallocThrows_Expect_MemoryError)
{
    GlobalMockObject::verify();
    MOCKER_CPP(&RdmaHandleManager::GetByAddr)
        .stubs()
        .with(mockcpp::any(), mockcpp::any(), mockcpp::any(), mockcpp::any())
        .will(returnValue(fakeRdmaHandle));
    // lbMax > 0 → flushOpcodeSupport_ = true → AllocateLocalMemory 走 HrtMalloc 分支
    int lbMax = 1;
    MOCKER(RaGetLbMax)
        .stubs()
        .with(mockcpp::any(), outBoundP(&lbMax, sizeof(lbMax)))
        .will(returnValue(0));
    // HrtMalloc 抛出异常 → AllocateLocalMemory 失败，异常向上传播
    MOCKER(HrtMalloc)
        .stubs()
        .with(mockcpp::any(), mockcpp::any())
    ...
    FlushHandle handle;
    IpAddress ip("192.168.1.1");
    EXPECT_ANY_THROW(handle.Init(ip, 0));
    EXPECT_FALSE(handle.flushIsInitialized);
}
