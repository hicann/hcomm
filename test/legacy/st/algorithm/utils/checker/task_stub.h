 ...
 * Description: task stub header file
 * Author: shenyutian
 * Create: 2024-07-27ExecPos
 */
...
MAKE_ENUM(
    ...
    WAIT_FLAG_SHADOW, AIV_START, BLOCK_START, AIV_END, VIRTUAL_RANK_START)
...
struct LinkInfoStub {
    ...
    LinkInfoStub(LinkProtoStub proto) { linkProto = proto; }
...
public:
    TaskStubLocalReduce(
        const DataSlice& srcSlice, const DataSlice& dstSlice, CheckerDataType dataType, CheckerReduceOp reduceOp,
...
public:
    TaskStubLocalBatchReduce(
        const std::vector<DataSlice>& srcSlices, const DataSlice& dstSlice, CheckerDataType dataType,
        CheckerReduceOp reduceOp)
...
public:
    TaskStubPost(
        const RankId remoteRank, const LinkInfoStub& link, u32 topicId,
...
public:
    TaskStubRead(
        const RankId remoteRank, const LinkInfoStub& link, const DataSlice& localSlice, const DataSlice& remoteSlice,
        bool isGenFromSync = false)
    ...
    const LinkInfoStub GetLinkInfo() const;
    bool IsGenFromSync();
...
public:
    TaskStubReadReduce(
        const RankId remoteRank, const LinkInfoStub& link, const DataSlice& localSlice, const DataSlice& remoteSlice,
        CheckerDataType dataType, CheckerReduceOp reduceOp, bool isGenFromSync = false)
