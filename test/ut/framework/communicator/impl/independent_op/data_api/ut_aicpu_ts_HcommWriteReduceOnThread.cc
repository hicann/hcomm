...
protected:
    ...
    virtual void TearDown() override
    {
        ...
        std::cout << "A Test case in UtAicpuTsHcommWriteReduceOnThread TearDown" << std::endl;
    }
...
TEST_F(UtAicpuTsHcommWriteReduceOnThread, Ut_HcommWriteReduceOnThread_When_Normal_Expect_ReturnIsHCCL_SUCCESS)
{
    res = HcommWriteReduceOnThread(thread, devHandle, dst, src, count, dataType, reduceOp);
    EXPECT_EQ(res, HCCL_SUCCESS);
}

TEST_F(UtAicpuTsHcommWriteReduceOnThread, Ut_HcommWriteReduceOnThread_When_Thread_IsNull_Expect_ReturnIsHCCL_E_PTR)
{
    res = HcommWriteReduceOnThread(0, devHandle, dst, src, count, dataType, reduceOp);
