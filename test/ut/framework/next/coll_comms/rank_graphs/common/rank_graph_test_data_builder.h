...
namespace Hccl {
namespace test {
inline AddressInfo MakeAddress(const std::string &ip, std::initializer_list<std::string> ports,
    const std::string &planeId = "0")
{
    AddressInfo address;
...
inline RankLevelInfo MakeRankLevel(u32 netLayer, const std::string &netInstId, NetType netType,
    const std::vector<AddressInfo> &rankAddrs)
{
    RankLevelInfo level;
...
inline NewRankInfo MakeRankInfo(u32 rankId, u32 deviceId, u32 localId,
    const std::vector<RankLevelInfo> &levels, u32 replacedLocalId = UINT32_MAX)
{
    NewRankInfo rank;
    rank.rankId = rankId;
...
inline EdgeInfo MakeEdge(u32 netLayer, LinkType linkType, u32 localA,
    std::initializer_list<std::string> localAPorts, u32 localB = 0,
...
{
    return MakeTopoInfo({0, 1}, {
        MakeEdge(0, LinkType::PEER2NET, 0, {"0/1", "0/2"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 0),
        MakeEdge(0, LinkType::PEER2NET, 0, {"1/1", "1/2"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 1),
        MakeEdge(0, LinkType::PEER2NET, 1, {"0/3", "0/4"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 0),
        MakeEdge(0, LinkType::PEER2NET, 1, {"1/3", "1/4"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 1),
    });
}
...
inline TopoInfo MakeFourPeerBuilderTopo()
{
    ...
    for (u32 localId = 0; localId < 4U; ++localId) {
        edges.emplace_back(MakeEdge(1, LinkType::PEER2NET, localId, {"0/7", "0/8"}, 0, {},
            LinkProtocol::UB_CTP, AddrPosition::DEVICE, 0));
        edges.emplace_back(MakeEdge(2, LinkType::PEER2NET, localId, {"1/7", "1/8"}, 0, {},
            LinkProtocol::UB_CTP, AddrPosition::DEVICE, 0));
    }
...
inline TopoInfo MakeTwoByTwoPlusBackupTopo()
{
    ...
    for (u32 localId : {0U, 1U, 8U, 9U}) {
        edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, localId, {"0/1"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 0));
        edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, localId, {"0/2"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 1));
        edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, localId, {"1/1"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 2));
        edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, localId, {"1/2"}, 0, {}, LinkProtocol::UB_CTP,
            AddrPosition::DEVICE, 3));
    }
    edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, 64, {"0/0", "0/1", "0/2", "0/3"}, 0, {},
        LinkProtocol::UB_CTP, AddrPosition::DEVICE, 0));
    edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, 64, {"0/4", "0/5", "0/6", "0/7"}, 0, {},
        LinkProtocol::UB_CTP, AddrPosition::DEVICE, 1));
    edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, 64, {"1/0", "1/1", "1/2", "1/3"}, 0, {},
        LinkProtocol::UB_CTP, AddrPosition::DEVICE, 2));
    edges.emplace_back(MakeEdge(0, LinkType::PEER2NET, 64, {"1/4", "1/5", "1/6", "1/7"}, 0, {},
        LinkProtocol::UB_CTP, AddrPosition::DEVICE, 3));

    for (u32 localId : {0U, 1U, 8U, 9U, 64U}) {
        edges.emplace_back(MakeEdge(1, LinkType::PEER2NET, localId, {"0/8"}, 0, {}, LinkProtocol::UB_TP,
            AddrPosition::DEVICE, 0));
    }
...
inline RankTableInfo MakeRankTable1p()
{
    return MakeRankTable({
        MakeRankInfo(0, 0, 0, {
            MakeRankLevel(0, "az0-rack0", NetType::TOPO_FILE_DESC, {
...
inline RankTableInfo MakeRankTable2p()
{
    return MakeRankTable({
        MakeRankInfo(0, 0, 0, {
            MakeRankLevel(0, "az0-rack0", NetType::CLOS, {
...
inline RankTableInfo MakeRankTable4pForBuilder()
{
    ...
    for (u32 rankId = 0; rankId < 4; ++rankId) {
        ...
        ranks.emplace_back(MakeRankInfo(rankId, rankId, rankId, {
            MakeRankLevel(0, "az0-rack0", NetType::TOPO_FILE_DESC, {
            ...
            MakeRankLevel(1, "az0", NetType::CLOS, {
                MakeAddress("192.168.101." + std::to_string(ipOffset), {"0/7", "0/8"}),
            }),
            MakeRankLevel(2, "all", NetType::CLOS, {
...
inline RankTableInfo MakeRankTable4p64Plus1(bool replaceRank1)
{
    ...
    for (u32 rankId = 0; rankId < 4; ++rankId) {
        ...
        ranks.emplace_back(MakeRankInfo(rankId, deviceId, localId, {
            MakeRankLevel(0, "az0-rack0", NetType::TOPO_FILE_DESC, {
...
inline std::vector<AddressInfo> MakeTwoByTwoLevel0Addrs(u32 ipThirdOctet, bool backupPorts)
{
    ...
    for (u32 plane = 0; plane < 2; ++plane) {
        for (u32 port = 0; port <= maxPort; ++port) {
            ...
            addrs.emplace_back(MakeAddress("192.168." + std::to_string(ipThirdOctet) + "." +
                std::to_string((plane == 0U ? 100U : 110U) + port), {std::to_string(plane) + "/" +
                std::to_string(port)}));
        }
    }
    return addrs;
...
inline RankTableInfo MakeRankTable2x2(bool replaceRank1)
{
    ...
    for (u32 rankId = 0; rankId < localIds.size(); ++rankId) {
        ...
        ranks.emplace_back(MakeRankInfo(rankId, deviceId, localId, {
            MakeRankLevel(0, "az0-rack0", NetType::TOPO_FILE_DESC, MakeTwoByTwoLevel0Addrs(level0IpOctet,
                backupPorts)),
            MakeRankLevel(1, "az0-layer1", NetType::CLOS, {
