...
TEST_F(HcclAlltoAllTest, Ut_HcclAlltoAll_When_SendBufIsNull_Expect_ReturnIsHCCL_E_PTR)
{
    ...
    HcclResult ret = HcclAlltoAllInner(
        sendBuf, sendCount, HCCL_DATA_TYPE_INT8, recvBuf, recvCount, HCCL_DATA_TYPE_INT8, comm, stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUF_COUNT(
        HCCL_COM_DATA_SIZE + 1, HCCL_COM_DATA_SIZE + 1, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
TEST_F(HcclAlltoAllTest, Ut_HcclAlltoAll_When_TypeIsNotEqual_Expect_ReturnIsHCCL_E_PARA)
{
    ...
    HcclResult ret = HcclAlltoAllInner(
        sendBuf, sendCount, HCCL_DATA_TYPE_INT32, recvBuf, recvCount, HCCL_DATA_TYPE_INT8, comm, stream);
    EXPECT_EQ(ret, HCCL_E_PARA);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUF_COUNT(
        HCCL_COM_BIG_DATA_SIZE, HCCL_COM_BIG_DATA_SIZE, HCCL_COM_BIG_DATA_SIZE, HCCL_COM_BIG_DATA_SIZE);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
