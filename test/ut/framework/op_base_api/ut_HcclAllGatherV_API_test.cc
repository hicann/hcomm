...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(0, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 1, HCCL_COM_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);

    HcclResult ret
        = HcclAllGatherVInner(sendBuf, sendCount, recvBuf, recvCounts, recvDispls, HCCL_DATA_TYPE_INT8, comm, stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(0, 0, HCCL_COM_DATA_SIZE, 1, HCCL_COM_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_DATA_SIZE, SYS_MAX_COUNT + 1, HCCL_COM_DATA_SIZE, 1, HCCL_COM_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 0, 1, HCCL_COM_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 0, 0, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 1, HCCL_COM_DATA_SIZE, 0, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 1, 0, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
TEST_F(HcclAllGatherVTest, Ut_HcclAllGatherV_When_GroupModeSuccess_Expect_ReturnIsHCCL_SUCCESS)
{
    MOCKER(taskAppend).stubs().with(mockcpp::any(), mockcpp::any()).will(returnValue(HCCL_SUCCESS));
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, 1, HCCL_COM_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
    hcclGroupDepth = 1;
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE + 1, 1, HCCL_COM_DATA_SIZE + 1, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_BIG_DATA_SIZE, HCCL_COM_BIG_DATA_SIZE, HCCL_COM_BIG_DATA_SIZE, 1, HCCL_COM_BIG_DATA_SIZE, 1, 0);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
...
TEST_F(HcclAllGatherVTest, Ut_HcclAllGatherV_When_Exec20times_Expect_ReturnIsHCCL_SUCCESS)
{
    ...
    for (u64 k = 0; k < LOOP_TIMES; k++) {
        HcclResult ret = HcclAllGatherVInner(
            sendBuf, sendCount, recvBuf, recvCounts, recvDispls, HCCL_DATA_TYPE_INT8, comm, stream);
        EXPECT_EQ(ret, HCCL_SUCCESS);
...
{
    UT_SET_SENDBUF_COUNT_RECVBUFV(
        HCCL_COM_DATA_SIZE, HCCL_COM_DATA_SIZE, rankNum * HCCL_COM_DATA_SIZE, rankNum, HCCL_COM_DATA_SIZE, rankNum,
        HCCL_COM_DATA_SIZE);
    UT_COMM_CREATE_DEFAULT(comm);
    UT_STREAM_CREATE_DEFAULT(stream);
