...
TEST_F(HcclReduceScatterTest, Ut_HcclReduceScatter_When_SendBufIsNull_Expect_ReturnIsHCCL_E_PTR)
{
    ...
    HcclResult ret
        = HcclReduceScatterInner(sendBuf, recvBuf, count, HCCL_DATA_TYPE_INT8, HCCL_REDUCE_SUM, comm, stream);
...
TEST_F(HcclReduceScatterTest, Ut_HcclReduceScatter_When_DeviceNotSupport_Expect_ReturnIsHCCL_E_NOT_SUPPORT)
{
    ...
    HcclResult ret = HcclReduceScatterInner(
...
TEST_F(HcclReduceScatterTest, Ut_HcclReduceScatter_When_GroupModeSuccess_Expect_ReturnIsHCCL_SUCCESS)
{
