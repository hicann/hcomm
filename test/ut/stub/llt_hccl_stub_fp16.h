...
static inline float fp16_ieee_to_fp32_value(uint16_t h)
{
     ...
     * - The exponent needs to be corrected by the difference in exponent bias between single-precision and
     * half-precision formats (0x7F - 0xF = 0x70)
     ...
     *     exponent adjustment by 0xE0 less the necessary exponent adjustment by 0x70 due to difference in exponent
     * bias. The floating-point multiplication hardware would ensure than Inf and NaN would retain their value on at
     * least partially IEEE754-compliant implementations.
     *
     ...
     * Therefore, when the biased exponent is 126, a unit change in the mantissa of the input denormalized
     * half-precision number causes a change of the constructud single-precision number by 2**(-24), i.e. the same
     * amount.
     *
     * The last step is to adjust the bias of the constructed single-precision number. When the input half-precision
     * number is zero, the constructed single-precision number has the value of FP32 = 1 * 2**(126 - 127) = 2**(-1) =
     * 0.5 Therefore, we need to subtract 0.5 from the constructed single-precision number to get the numerical
     ...
     * the input exponent. The variable two_w contains input exponent in bits 27-31, therefore if its smaller than
