...
protected:
    static void SetUpTestCase() { std::cout << "\033[36m--ManagedMemTest SetUP--\033[0m" << std::endl; }
    static void TearDownTestCase() { std::cout << "\033[36m--ManagedMemTest TearDown--\033[0m" << std::endl; }
...
TEST_F(ManagedMemTest, alloc_fail)
{
    ...
    /*构造rt_malloc异常*/
    .expects(atMost(1)).will(returnValue(1));
    ManagedMem mem = ManagedMem::alloc(8);
